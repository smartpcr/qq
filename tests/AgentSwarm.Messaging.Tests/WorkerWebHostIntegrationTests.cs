    ...
    : IClassFixture<WorkerWebHostIntegrationTests.WorkerFactory>
{
    ...
    public void HostBootstrap_RegistersTelegramUserAuthorizationService()
    {
        ...
        // TelegramUserAuthorizationService is the sole supported
        // Telegram IUserAuthorizationService -- the iter-5
        // ConfiguredOperatorAuthorizationService was deleted in
        // Stage 5.2 iter-4 (retire-from-supported-surface) so any
        // future DI registration that bypasses the registry-backed
        // model would have to author a brand-new implementation.
        ...
        auth!.Should().BeOfType<AgentSwarm.Messaging.Telegram.Auth.TelegramUserAuthorizationService>(
            "Stage 3.4 replaced the now-deleted ConfiguredOperatorAuthorizationService with TelegramUserAuthorizationService -- the registry-backed authz that reads OperatorBinding rows from persistence and onboards via Telegram:UserTenantMappings");
    }
