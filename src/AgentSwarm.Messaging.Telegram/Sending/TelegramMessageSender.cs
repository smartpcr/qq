...
/// the architecture.md §4.12 contract: <see cref="SendTextAsync"/>
...
/// <remarks>
/// <para>
/// <b>Render boundary (architecture.md §4.12).</b> For non-question
...
/// </para>
/// <para>
/// <b>Trace footer — unconditional (architecture.md §10.1 "renderer
...
/// and <see cref="ApiRequestException"/> with <c>ErrorCode ≥ 500</c>
...
public sealed class TelegramMessageSender : IMessageSender
{
    ...
    /// carries its trace id per architecture.md §10.1 "Renderer
    ...
    internal static TimeSpan ComputeTransientBackoff(int attempt)
    {
        ...
        // Deterministic jitter source seeded by the attempt index so
        // tests can predict the wait. The ±20% bound keeps the
        // schedule predictable for the operator SLO budget while
        // breaking lockstep retries.
        var jitter = 1.0 + (((attempt * 7919) % 41) - 20) / 100.0;
