...
### Implementation Steps
- [x] Create `SlackIntegrationTestFixture` that configures `WebApplicationFactory<Program>` with SQLite in-memory database, `ChannelBasedSlackQueue` for inbound/outbound queues, and mock `IAgentTaskService`
- [x] Create mock Slack API server using ASP.NET Core `TestServer` to simulate Slack Web API responses (`chat.postMessage`, `chat.update`, `views.open`, `auth.test`, `usergroups.users.list`)
- [x] Write integration test for AC-1: POST `/api/slack/commands` with a valid `/agent ask generate implementation plan for persistence failover` payload; verify `IAgentTaskService.CreateTaskAsync` was called, a thread root message was posted, and a `SlackAuditEntry` was persisted
- [x] Write integration test for AC-2: simulate orchestrator emitting a status update and an `AgentQuestion`; verify both are posted as threaded replies with correct Block Kit formatting
- [x] Write integration test for AC-3: simulate a Block Kit button click on an `AgentQuestion` message; verify a `HumanDecisionEvent` is published and the message buttons are updated to disabled state
- [x] Write integration test for AC-4: send the same event payload twice with identical `event_id`; verify only one `IAgentTaskService` call occurs and the second is recorded as `outcome = duplicate`
- [x] Write integration test for AC-5: send a slash command from a channel not in `AllowedChannelIds`; verify HTTP 200 ACK is returned to Slack, the request is rejected during async processing with an ephemeral error message to the user, and audit records `outcome = rejected_auth`. Covered by TWO companion tests, EACH against a real production transport (no test-only configuration on either):
    - **(a) HTTP-facing sync filter leg** (`SlackAuthorizationFilter`, Scenario 11.4 / architecture.md §2.4). Default fixture exercises this path via `POST /api/slack/commands` from `DeniedChannelId`; asserts (i) HTTP 200 ACK; (ii) the response body carries the literal `SlackAuthorizationOptions.DefaultRejectionMessage` text inside `{"response_type":"ephemeral",...}` (channel-specific text, not "any ephemeral"); (iii) `outcome = rejected_auth` audit row with `team_id` / `channel_id` / `user_id` / `command_text` (verbatim `/agent ask <args>`); (iv) `IAgentTaskService.CreateTaskAsync` was NOT invoked.
    - **(b) Socket Mode async-pipeline leg** — the brief's literal "during async processing" path. Production's `SlackSocketModeReceiver` resolves the workspace's `AppLevelTokenRef` via `ISecretProvider`, opens an `ISlackSocketModeConnection` through `ISlackSocketModeConnectionFactory`, pumps frames in a background loop, and for every `slash_commands` / `events_api` / `interactive` frame: (i) sends the envelope_id ACK FIRST (Slack's 5-second WebSocket budget), (ii) normalises via `SlackSocketModePayloadNormalizer`, (iii) pushes onto `ISlackInboundQueue` via `SlackInboundEnqueueScheduler.EnqueueWithRetryAsync`. The MVC sync filter is NEVER in the Socket Mode path. The iter-8 test exercises this entire production transport end-to-end: it constructs a real `SlackSocketModeReceiver` against the host's resolved `ISlackInboundQueue` + `ISlackInboundEnqueueDeadLetterSink` + `ILoggerFactory`, hands it a scripted `ISlackSocketModeConnection` through a scripted `ISlackSocketModeConnectionFactory` (both internal types are visible via `[InternalsVisibleTo("AgentSwarm.Messaging.Slack.Tests")]`), seeds the workspace's `AppLevelTokenRef` in the `InMemorySecretProvider`, pre-loads a `hello` + `slash_commands` frame pair onto the scripted connection, and calls `StartAsync`. The test then asserts: (1) the connection factory was invoked exactly once with the resolved `xapp-...` token (proves the production `ISecretProvider` seam was exercised, not bypassed); (2) the scripted connection captured a `SendAckAsync` call for the slash_commands envelope_id (proves the production WebSocket ACK contract was honoured through the real transport — NOT "by construction" as a direct-enqueue test would imply); (3) the pipeline-side `SlackInboundAuthorizer` rejected the denied-channel envelope and POSTed an ephemeral via `ISlackEphemeralResponder.SendEphemeralAsync` (captured by `MockSlackWebApi` as `{response_type=ephemeral, text=DefaultRejectionMessage, path=/mock/response_url/*}`); (4) the `rejected_auth` audit row carries `team_id` / `channel_id` / `user_id` plus the VERBATIM `command_text` (`/agent ask <args>`) extracted via `SlackInboundEnvelopeAuditFields.Extract` from the Socket Mode JSON `RawPayload` (proves the JSON-shape audit extraction path); (5) `IAgentTaskService.CreateTaskAsync` was NOT invoked; (6) no `inbound=success` row was written for the denied channel. `StopAsync` then cancels the receive loop and asserts `CloseAsync` was invoked on the scripted connection so the shutdown contract is pinned too. The pre-iter-7 `bypassMvcAuthorizationFilter` fixture knob and the prior `CommandText: null` shape on pipeline rejections remain removed; the iter-8 receiver-driven test exercises the production transport end-to-end without any fixture-side production-shape mutation, replacing the iter-7 direct `ISlackInboundQueue.EnqueueAsync` shortcut the iter-8 evaluator flagged.
- [x] Write integration test for AC-6: execute a full ask-question-approve flow and verify all exchanges are queryable by `CorrelationId` via `SlackAuditLogger.QueryAsync`

### Dependencies
- phase-connector-wiring-and-acceptance-validation/stage-slackconnector-facade-and-di-wiring

### Test Scenarios
- [x] Scenario: AC-1 slash command creates task and thread -- Given a signed `/agent ask` payload from an authorized channel, When the full pipeline processes it, Then a task is created via the orchestrator and a Slack thread root message is posted
- [x] Scenario: AC-3 button click maps to decision -- Given an `AgentQuestion` rendered with buttons in a thread, When a user clicks "Approve", Then a `HumanDecisionEvent` with `ActionValue = "approve"` is published and the originating message is updated
- [x] Scenario: AC-4 duplicate suppression -- Given two identical Events API payloads with the same `event_id`, When both are processed, Then only one orchestrator call is made and audit shows one `success` and one `duplicate`
- [x] Scenario: AC-6 correlation ID query -- Given a full command-response-decision exchange, When `SlackAuditLogger.QueryAsync` is called with the `CorrelationId`, Then all related audit entries (inbound command, outbound question, inbound decision) are returned