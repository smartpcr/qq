...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: all sibling documents' substantive content consistently uses the sidecar envelope model (see S-3, HC-3, Section 10). The connector reads `ProposedDefaultActionId` from `AgentQuestionEnvelope`, denormalizes it into `PendingQuestionRecord.DefaultActionId`, and `QuestionTimeoutService` applies it at expiry or emits `__timeout__` when absent. Remaining editorial contradictions (e2e-scenarios.md lines 64–66 and footer line 650 reference this spec as UNRESOLVED) must be reconciled before implementation — see Section 10. |
...
| P95 latency scoping (HC-4) | **RESOLVED.** `telegram.send.latency_ms` (primary) is the first-attempt, non-rate-limited metric; the **P95 ≤ 2 s** acceptance criterion applies to it. `telegram.send.all_attempts_latency_ms` (diagnostic) is the all-inclusive metric for capacity planning. | §10.4 (lines 668–692): defines `telegram.send.latency_ms` (primary) as **first-attempt, non-rate-limited** — "elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** waiting behind a 429 rate-limit hold." Defines `telegram.send.all_attempts_latency_ms` (diagnostic) as all-inclusive. Line 676: "The 2-second P95 target applies to the primary `telegram.send.latency_ms` metric, which covers first-attempt, non-rate-limited sends only." Line 692: confirms naming alignment with first-attempt definition. **Aligned.** | **Internal contradiction.** Stage 4.1 (line 367) and Stage 7.2 (line 550) describe `telegram.send.latency_ms` as "all-inclusive," directly contradicting architecture.md §10.4. However, Stage 7.2 (line 559) correctly describes `telegram.send.latency_ms` as "first-attempt, non-rate-limited successes." The line-559 definition matches architecture.md and e2e-scenarios.md; lines 367 and 550 are stale and require editorial correction. | Lines 264–279: defines `telegram.send.latency_ms` as first-attempt, non-rate-limited — "measured ONLY for messages that succeed on first attempt and are NOT waiting behind a 429 rate-limit hold. This first-attempt, non-rate-limited metric is the one the P95 ≤ 2s acceptance criterion applies to." Lines 270–280 assert P95 under this definition. **Aligned** with architecture.md and this spec. | **Architecture.md and e2e-scenarios.md aligned** on `telegram.send.latency_ms` = first-attempt, non-rate-limited (P95 target) and `telegram.send.all_attempts_latency_ms` = all-inclusive diagnostic. **Implementation-plan.md has an internal contradiction** at lines 367/550 (say "all-inclusive") vs. line 559 (says "first-attempt") — line 559 is authoritative; lines 367/550 require editorial correction before implementation. Per operator answer to `p95-latency-scope`: the P95 < 2 s acceptance gate applies to first-attempt, non-rate-limited sends only. |
...
| DefaultAction model (HC-3) | **Sidecar envelope model adopted.** `AgentQuestion` has no `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` on `AgentQuestionEnvelope`. | §3.1 (lines 167–190): models `AgentQuestion` **without** a `DefaultAction` property. Proposed default carried as `ProposedDefaultActionId` sidecar metadata on `AgentQuestionEnvelope`. Line 195: carries a "Cross-doc status — DefaultAction placement RESOLVED" label stating all siblings are aligned. The substantive content (sidecar model, behavioral contract) is consistent. | Stage 1.2: defines `AgentQuestion` **without** `DefaultAction`; creates `AgentQuestionEnvelope` with `ProposedDefaultActionId`. The substantive implementation approach is consistent. | Lines 57–63: explicitly adopts the sidecar envelope model. Lines 64–66: state "tech-spec.md Section 10 alignment table still marks DefaultAction placement as UNRESOLVED" — this is a **stale reference** from an earlier iteration of this spec; the current Section 10 does not mark it unresolved. Footer line 650: repeats the same stale reference. | **Substantive content aligned across all documents** on the sidecar envelope model. **Remaining editorial contradictions:** e2e-scenarios.md lines 64–66 and footer line 650 still reference this spec's Section 10 as marking the placement "UNRESOLVED" — that was true in earlier iterations but is no longer the case. **Pre-implementation blocker:** update e2e-scenarios.md lines 64–66 and footer line 650 to remove the stale "UNRESOLVED" reference, confirming alignment. |
