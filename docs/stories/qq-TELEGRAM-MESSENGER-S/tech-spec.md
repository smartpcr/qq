...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. **Default-action design (this spec's adopted approach):** The proposed default action is carried as sidecar metadata — a `ProposedDefaultActionId` field in the `AgentQuestionEnvelope` — per **e2e-scenarios.md** (lines 57–76), which defines the authoritative test contract. The Telegram connector reads `ProposedDefaultActionId` from the envelope and, when present, displays the proposed default in the message body and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling. **Cross-document note:** architecture.md §3.1 (lines 167–184) and §5.3 (line 424) both model `DefaultAction` as a first-class nullable property on `AgentQuestion`, while e2e-scenarios.md (lines 57–63) explicitly states the shared model does **not** include `DefaultAction` and instead carries it as `ProposedDefaultActionId` sidecar metadata on the envelope. This spec adopts the sidecar-metadata approach as the authoritative design because it keeps `AgentQuestion` connector-agnostic and matches the acceptance test contract. The behavioral contract is identical regardless of approach: the Telegram connector obtains the proposed default action ID, denormalizes it into `PendingQuestionRecord.DefaultActionId`, and applies it on timeout. See Section 10 alignment notes. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. This spec adopts the **sidecar-metadata approach** from **e2e-scenarios.md** (lines 57–76) as the authoritative test contract: the shared `AgentQuestion` does **not** include a `DefaultAction` property. Instead, the proposed default action is carried as a `ProposedDefaultActionId` field on the `AgentQuestionEnvelope`. The Telegram connector reads `ProposedDefaultActionId` from the envelope and, when present, denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling. When the question times out, `QuestionTimeoutService` reads `DefaultActionId`, resolves the full `HumanAction` from `IDistributedCache`, and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. **Note:** Architecture.md §3.1 (lines 167–184) and §5.3 (line 424) both define `DefaultAction` as a first-class nullable property on `AgentQuestion`. E2e-scenarios.md (lines 57–63) explicitly states the shared model does **not** include `DefaultAction` and uses `ProposedDefaultActionId` on the envelope instead. This spec follows the e2e-scenarios.md sidecar approach because it is the acceptance test contract and keeps `AgentQuestion` connector-agnostic. The behavioral contract is identical either way. See Section 10. | Story requirement + e2e-scenarios.md (lines 57–76, authoritative test contract) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. Per architecture.md §10.4 (lines 660–666), two complementary metrics are defined: **`telegram.send.latency_ms`** (primary) measures elapsed time from `OutboundMessage.CreatedAt` to Telegram Bot API HTTP 200 for **all messages regardless of attempt number or rate-limit holds**. This all-inclusive metric is the one the **P95 ≤ 2 s acceptance criterion** applies to. When a message encounters a 429 rate-limit wait, the wait duration is included in the elapsed time; when a message is retried, the latency reflects total time from original enqueue to eventual success. **`telegram.send.first_attempt_latency_ms`** (diagnostic) measures the same interval but scoped to messages that succeed on their first delivery attempt without encountering a rate-limit hold — this narrower metric isolates the system's inherent processing latency for capacity planning and regression detection. Additionally, time spent waiting during 429 backoff is tracked via a dedicated `telegram.send.rate_limited_wait_ms` histogram for operational diagnostics. Under normal operating conditions (no rate-limit hits, first-attempt success), the vast majority of sends comfortably meet the 2-second target. Under extreme burst conditions (100+ agents), the priority queuing design ensures Critical/High severity messages are dispatched first and meet the 2-second target, while Normal/Low severity messages may queue-delay beyond 2 seconds. | Story requirement + architecture.md §10.4 (lines 660–666, metric definition) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to the **all-inclusive primary `telegram.send.latency_ms` metric** (architecture.md §10.4 lines 660–666), which includes rate-limit waits and retries. A separate `telegram.send.first_attempt_latency_ms` diagnostic metric isolates first-attempt, non-rate-limited successes for capacity planning. Under normal conditions most sends meet the 2-second target; under extreme burst, priority queuing ensures Critical/High messages are dispatched first and meet the target, while Normal/Low may queue-delay beyond 2 seconds. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: this spec adopts the e2e-scenarios.md sidecar approach (lines 57–76) where the proposed default is carried as `ProposedDefaultActionId` on the `AgentQuestionEnvelope`; the Telegram connector reads it at render time, displays the proposed default in the message body, and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` (HC-3). `QuestionTimeoutService` reads this field at expiry to apply the default or emit `__timeout__`. |
...
| P95 latency scoping (HC-4) | < 2 s target applies to the **all-inclusive `telegram.send.latency_ms`** primary metric, which covers all messages regardless of attempt number or rate-limit holds. A separate `telegram.send.first_attempt_latency_ms` diagnostic metric isolates first-attempt, non-rate-limited successes for capacity planning. | §10.4 (lines 660–666): "`telegram.send.latency_ms` (primary) = … measured for ALL messages regardless of attempt number or rate-limit holds. This all-inclusive metric is the one the P95 ≤ 2s acceptance criterion applies to." `telegram.send.first_attempt_latency_ms` (diagnostic) = first-attempt, non-rate-limited successes only. **Aligned** with this spec. | Not specified numerically | Burst test asserts P95 < 2 s on `telegram.send.latency_ms` covering all sends (lines 263–279). Separate `telegram.send.first_attempt_latency_ms` diagnostic histogram also emitted (line 279). **Aligned.** | **All documents are aligned.** The P95 ≤ 2 s target applies to the all-inclusive `telegram.send.latency_ms` metric. `telegram.send.first_attempt_latency_ms` is a diagnostic metric for capacity planning. |
...
| DefaultAction model (HC-3) | This spec adopts the sidecar-metadata approach from e2e-scenarios.md: `ProposedDefaultActionId` on `AgentQuestionEnvelope`, not on `AgentQuestion`. Telegram connector reads from envelope and denormalizes into `PendingQuestionRecord.DefaultActionId`. | §3.1 (lines 167–184) and §5.3 (line 424) both model `DefaultAction` as a **first-class nullable property** on `AgentQuestion`. §3.1: "The shared `AgentQuestion` model includes a nullable `DefaultAction` property… This is a first-class shared field." §5.3: "The default action is a first-class nullable property on the `AgentQuestion` model." **Diverges** from this spec's sidecar approach. | Stage 1.2 (line 36): defines `AgentQuestion` with `DefaultAction` as a first-class property — aligns with architecture.md §3.1/§5.3 but not with this spec's sidecar approach. | Lines 57–63: explicitly states "The shared AgentQuestion model does NOT include a DefaultAction property" and carries the proposed default as `ProposedDefaultActionId` on the `AgentQuestionEnvelope` (line 75: `ProposedDefaultActionId = act-1`). **Aligned** with this spec's sidecar approach. | **This spec adopts the e2e-scenarios.md sidecar-metadata approach** as the authoritative test contract. Architecture.md §3.1/§5.3 define `DefaultAction` as first-class on `AgentQuestion`; e2e-scenarios.md defines it as sidecar `ProposedDefaultActionId` on the envelope. The behavioral contract is identical: the Telegram connector obtains the proposed default action ID, denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays it in the message body, and applies it on timeout. Only the read path differs (model property vs. envelope metadata). No implementation is blocked by this divergence. |
