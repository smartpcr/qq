...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. Architecture.md (§5, line 177) extends the shared `AgentQuestion` record with a `DefaultAction` property (nullable `string` matching one of `HumanAction.Value` entries in `AllowedActions`). Implementation-plan.md Stage 1.2 creates `AgentQuestion` with `DefaultAction` as a first-class property. E2e-scenarios.md uses `ProposedDefault` as a Gherkin table field name that maps to `AgentQuestion.DefaultAction`. This connector consumes `AgentQuestion.DefaultAction` directly — no Telegram-specific override or `default:` prefix convention is needed. When `DefaultAction` is present, the timeout handler applies it automatically; when absent (`null`), the question expires with a `__timeout__` action value. | Epic shared data model + story requirement + architecture.md §5 + implementation-plan.md Stage 1.2 |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued | Story requirement (tighter than epic's 3 s) |
| HC-5 | **Zero message loss** for Critical, High, and Normal severity messages under burst from 100+ agents (1 000+ queued events). Under extreme backpressure (queue depth exceeds `MaxQueueDepth`, default 5 000), low-severity messages that cannot be enqueued are immediately **dead-lettered** — not silently discarded, not shed. Dead-lettered messages are persisted in the dead-letter queue with full failure context, counted via a `telegram.messages.deadlettered_backpressure` metric, and trigger an ops-channel alert. Operators can replay dead-lettered messages from the DLQ once pressure subsides. This is consistent with architecture.md §10.4's priority-queuing design and the reliability model's "delivered-or-dead-lettered" contract. Critical and High severity messages are never subject to backpressure rejection — they are always enqueued regardless of queue depth. | Story + epic performance requirements + architecture.md §10.4 |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **(a) Transfer human oversight of a task to another operator.** The handler accepts `/handoff TASK-ID @operator-alias`, validates both arguments, updates the `OperatorRegistry` binding, and notifies both parties. **Cross-doc state (inconsistent — alignment needed):** This spec and architecture.md (line 445) agree on the decided transfer semantic. e2e-scenarios.md (line 336) defines full Gherkin scenarios for the transfer flow. However, **implementation-plan.md (line 206) still implements a not-yet-configured stub** returning "Handoff is not yet configured — awaiting policy decision (D-4)" and explicitly says D-4 is open. **architecture.md (line 445) says decided but may carry a stale open-decision note elsewhere.** The sibling doc set is not yet internally consistent on D-4 status. This spec asserts "Decided" based on the weight of architecture.md's full specification and e2e-scenarios.md's complete Gherkin coverage; implementation-plan.md should update its `HandoffCommandHandler` to implement the full transfer rather than a stub. | Decided (sibling inconsistency flagged) |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided: transfer human oversight of a task to another operator | Decided (line 445): full transfer flow with validation, notifications, and audit | **Inconsistent:** `HandoffCommandHandler` (line 206) implements a not-yet-configured stub, says D-4 is open, and logs for future replay | Decided (line 336): full Gherkin scenarios — valid transfer, invalid task ID, unknown operator alias | **Partially aligned.** This spec, architecture.md, and e2e-scenarios.md agree D-4 is decided with transfer semantics. implementation-plan.md still treats D-4 as open and implements a stub — it needs to update `HandoffCommandHandler` to the full transfer flow in its next iteration. |
...
| DefaultAction model (HC-3) | `AgentQuestion.DefaultAction` — shared model property consumed directly by Telegram connector | `AgentQuestion.DefaultAction` as nullable string (§5, line 177); extension note explains it extends the epic attachment's model | `AgentQuestion` with `DefaultAction` property (Stage 1.2, line 36) | `ProposedDefault` Gherkin field maps to `AgentQuestion.DefaultAction` | **Aligned** across all docs. `DefaultAction` is a first-class property on the shared `AgentQuestion` record. |
