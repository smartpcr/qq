...
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Metric definitions adopted by this spec follow architecture.md §8/§10.4 as the canonical source (see Section 10 for cross-document divergences).** Three metrics form the latency observability model: **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **enqueue instant** (`OutboundMessage.CreatedAt`, the moment the message is persisted to the outbound queue) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** delayed by a 429 rate-limit hold. This directly preserves the story requirement "P95 send latency under 2 seconds **after event is queued**" by starting the clock at enqueue, not dequeue. The **P95 ≤ 2 s** acceptance criterion applies to this metric. **`telegram.send.all_attempts_latency_ms`** (all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured for **all** messages regardless of attempt number or rate-limit holds — the all-inclusive monitoring metric for capacity planning. **`telegram.send.queue_dwell_ms`** (diagnostic) = enqueue to dequeue — monitors queue backlog; under normal operating conditions this is negligible (< 50 ms with 10 workers). Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends), `telegram.send.rate_limited_wait_ms` (429 backoff). Per operator answer to `p95-latency-scope`: the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately via `telegram.send.all_attempts_latency_ms`. Under normal operating conditions (the expected steady state), queue dwell is negligible, so `first_attempt_latency_ms` comfortably meets P95 ≤ 2 s across all severities. Under burst, priority queuing ensures Critical/High messages are dequeued first with minimal queue dwell, keeping their enqueue-to-200 latency within the 2 s window; Normal/Low first-attempt sends may exceed 2 s P95 under sustained burst due to accumulated queue dwell (per architecture.md §10.4). **Known cross-document divergence:** e2e-scenarios.md defines the acceptance gate as **dequeue**-to-HTTP-200 (using `DequeuedAt` as the start point) and names the all-inclusive metric `telegram.send.latency_ms` instead of `all_attempts_latency_ms`. This spec follows architecture.md §8/§10.4 (**enqueue**-to-200, `all_attempts_latency_ms`) as the canonical source. See Section 10 for details. | Story requirement; operator answer `p95-latency-scope` confirms first-attempt, non-rate-limited scope; follows architecture.md §8/§10.4 as canonical source; see Section 10 for sibling divergences |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to `telegram.send.first_attempt_latency_ms` — the acceptance gate metric covering first-attempt, non-rate-limited sends only, measured from enqueue (`OutboundMessage.CreatedAt`) to HTTP 200 (per architecture.md §8/§10.4 and operator answer to `p95-latency-scope`). The all-inclusive metric `telegram.send.all_attempts_latency_ms` tracks all sends including retries and rate-limit waits for capacity planning (per architecture.md §8/§10.4). See HC-4 and Section 10 for cross-document divergence notes. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| P95 latency scoping (HC-4) | **This spec follows architecture.md §8/§10.4 as canonical source.** `telegram.send.first_attempt_latency_ms` (acceptance gate) = **enqueue** (`OutboundMessage.CreatedAt`) to HTTP 200, first-attempt, non-rate-limited; P95 ≤ 2 s applies. `telegram.send.all_attempts_latency_ms` (all-inclusive) = enqueue to HTTP 200, all sends regardless of attempt or rate-limit — capacity planning. `telegram.send.queue_dwell_ms` (diagnostic) = enqueue to dequeue. Under normal load, queue dwell is negligible so the enqueue-based gate comfortably meets P95 ≤ 2 s. Per operator answer to `p95-latency-scope`, the P95 target applies to first-attempt, non-rate-limited sends only. | §8 Observability table: defines `telegram.send.first_attempt_latency_ms` as "enqueue — `OutboundMessage.CreatedAt` — to HTTP 200, first-attempt, non-rate-limited sends only — P95 ≤ 2 s target." All-inclusive metric: `telegram.send.all_attempts_latency_ms` (enqueue to HTTP 200 regardless of attempt or rate-limit). §10.4: same definitions — acceptance gate is enqueue-based, all-inclusive is `all_attempts_latency_ms`. **This spec is aligned with architecture.md.** | Requires verification — implementation-plan.md may have internal inconsistencies between sections on measurement start point and metric naming. Must align to architecture.md §10.4 before implementation. | E2e-scenarios.md **diverges** from architecture.md on two points: (1) defines `first_attempt_latency_ms` as **dequeue** (`DequeuedAt`) to HTTP 200 rather than enqueue-to-200; (2) names the all-inclusive metric `telegram.send.latency_ms` rather than `telegram.send.all_attempts_latency_ms`. E2e-scenarios.md's footer acknowledges this as a known editorial divergence with tech-spec.md. The scenarios are functionally valid (dequeue-to-200 is a subset of enqueue-to-200), but the metric names and measurement start points must be reconciled to architecture.md before implementation. | **Known divergence — reconciliation required before implementation.** Architecture.md and this spec use **enqueue-to-200** measurement + `all_attempts_latency_ms` naming. E2e-scenarios.md uses **dequeue-to-200** measurement + `latency_ms` naming. All four documents agree on the core semantics: (1) `first_attempt_latency_ms` is the acceptance gate, (2) P95 ≤ 2 s applies to first-attempt non-rate-limited sends only, (3) a separate all-inclusive metric captures all sends for capacity planning. The disagreements are on the measurement start point (enqueue vs. dequeue) and the all-inclusive metric's name (`all_attempts_latency_ms` vs. `latency_ms`). **Action required:** e2e-scenarios.md should update to use architecture.md §10.4 definitions (enqueue-to-200, `all_attempts_latency_ms`) before implementation begins. |
