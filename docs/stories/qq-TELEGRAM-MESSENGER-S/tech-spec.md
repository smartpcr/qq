...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. **Default-action design (this spec's adopted approach):** The proposed default action is carried as sidecar metadata — a `ProposedDefaultActionId` field in the event envelope / routing context — per **architecture.md §3.1** (lines 167–184). The Telegram connector reads `ProposedDefaultActionId` from context metadata and, when present, displays the proposed default in the message body and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling. **Internal inconsistency within architecture.md:** §3.1 (lines 167–184) specifies the sidecar-metadata approach and states the shared model does **not** include `DefaultAction`, while §5.3 (line 424) describes `DefaultAction` as "a first-class property of the shared `AgentQuestion` model." E2e-scenarios.md (lines 57–76) aligns with §5.3 by including `DefaultAction` directly on the `AgentQuestion` fixture (line 73: `DefaultAction = act-1`). This spec adopts the §3.1 sidecar-metadata approach as the authoritative shared-model definition because it keeps `AgentQuestion` connector-agnostic. The behavioral contract is identical regardless of which approach is chosen: the Telegram connector obtains the proposed default action ID, denormalizes it into `PendingQuestionRecord.DefaultActionId`, and applies it on timeout. See Section 10 alignment notes. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. This spec adopts **architecture.md §3.1** (lines 167–184) as the authoritative shared-model definition: the shared `AgentQuestion` does **not** include a `DefaultAction` property. Instead, the proposed default action is carried as **sidecar metadata** — a `ProposedDefaultActionId` field in the event envelope / routing context. The Telegram connector reads `ProposedDefaultActionId` from context metadata and, when present, denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling. When the question times out, `QuestionTimeoutService` reads `DefaultActionId`, resolves the full `HumanAction` from `IDistributedCache`, and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. **Note:** Architecture.md has an internal inconsistency — §5.3 (line 424) describes `DefaultAction` as a first-class `AgentQuestion` property, and e2e-scenarios.md (lines 57–76) also includes `DefaultAction` directly on the `AgentQuestion` fixture. This spec follows §3.1 as authoritative because it was explicitly designed for connector-agnostic separation. The behavioral contract is identical either way. See Section 10. | Story requirement + architecture.md §3.1 (authoritative shared-model definition) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. Per architecture.md §10.4 (lines 658–666), two complementary metrics are defined: **`telegram.send.latency_ms`** (primary) measures elapsed time from `OutboundMessage.CreatedAt` to Telegram Bot API HTTP 200 for messages that **succeed on their first delivery attempt** and are **not waiting behind a 429 rate-limit hold**. **`telegram.send.all_attempts_latency_ms`** (diagnostic) measures the same interval but includes all messages regardless of attempt number or rate-limit holds. The **P95 ≤ 2 s acceptance criterion applies to the primary `telegram.send.latency_ms` metric** — i.e., first-attempt, non-rate-limited successes only. Messages that are retried (attempts > 1) or rate-limited are tracked separately via `telegram.send.retry_latency_ms` and `telegram.send.rate_limited_wait_ms`. This scoping ensures the P95 target measures the system's inherent latency under normal operating conditions. Under extreme burst conditions (100+ agents), the priority queuing design ensures Critical/High messages are dispatched first and meet the 2-second target on first attempt, while lower-priority messages may queue-delay beyond 2 seconds. | Story requirement + architecture.md §10.4 (lines 658–666, metric definition) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to **first-attempt, non-rate-limited successes only** (architecture.md §10.4 lines 660–662). Messages delayed by rate-limit waits are excluded from the primary `telegram.send.latency_ms` metric and tracked separately via `telegram.send.rate_limited_wait_ms`. With priority queuing, Critical/High messages (~50 in a burst) are dispatched first and meet the 2-second target on first attempt; Normal/Low messages may queue-delay beyond 2 seconds under extreme burst. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: this spec adopts architecture.md §3.1 (lines 167–184) where the proposed default is carried as sidecar metadata (`ProposedDefaultActionId` in event envelope); the Telegram connector reads it at render time, displays the proposed default in the message body, and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` (HC-3). `QuestionTimeoutService` reads this field at expiry to apply the default or emit `__timeout__`. |
...
| P95 latency scoping (HC-4) | < 2 s target applies to **first-attempt, non-rate-limited successes** (`telegram.send.latency_ms` primary metric). Retries and rate-limit waits tracked separately via `telegram.send.retry_latency_ms` and `telegram.send.rate_limited_wait_ms`. | §10.4 (lines 660–666): "`telegram.send.latency_ms` (primary) = … measured only for messages that succeed on their first delivery attempt and are not waiting behind a 429 rate-limit hold. This is the metric the P95 ≤ 2s acceptance criterion applies to." **Aligned** with this spec. | Not specified numerically | Burst test asserts P95 < 2 s scoped to first-attempt successes (lines 257–281). **Aligned.** | **All documents are aligned.** The P95 ≤ 2 s target applies to first-attempt, non-rate-limited successes. Retries and rate-limit waits are diagnostic metrics. |
...
| DefaultAction model (HC-3) | This spec adopts §3.1 sidecar-metadata approach: `ProposedDefaultActionId` in event envelope, not on `AgentQuestion`. Telegram connector reads from context metadata and denormalizes into `PendingQuestionRecord.DefaultActionId`. | **Internal inconsistency in architecture.md:** §3.1 (lines 167–184) specifies sidecar-metadata approach — shared model does **not** include `DefaultAction`; proposed default carried as `ProposedDefaultActionId` in event envelope. §5.3 (line 424) contradicts this by describing `DefaultAction` as "a first-class property of the shared `AgentQuestion` model." | Stage 1.2 (line 36): defines `AgentQuestion` with `DefaultAction` as a first-class property — aligns with architecture.md §5.3 but not §3.1. | Lines 57–76: includes `DefaultAction` directly on the `AgentQuestion` fixture (line 57: "The shared AgentQuestion model includes a nullable DefaultAction property"; line 73: `DefaultAction = act-1`). Aligns with architecture.md §5.3 but not §3.1. | **This spec adopts architecture.md §3.1 as authoritative** because it is the dedicated shared-model definition section and its sidecar-metadata design keeps `AgentQuestion` connector-agnostic. The behavioral contract is identical regardless of approach: the Telegram connector obtains the proposed default action ID, denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays it in the message body, and applies it on timeout. Sibling documents that use the first-class property approach produce functionally equivalent behavior — only the read path differs (model property vs. context metadata). No implementation is blocked by this divergence. |
