...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. The shared `AgentQuestion` does **not** carry a `DefaultAction` property (see HC-3); instead, the Telegram connector determines the proposed default action from the agent/command context at render time and stores it in `PendingQuestionRecord.DefaultActionId` for timeout handling. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. Per the epic attachment and **architecture.md §5.2** (line 168), the shared `AgentQuestion` model does **not** include a `DefaultAction` property — it is unchanged from the epic's definition. The shared `AgentQuestion` includes: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions`, `ExpiresAt`, and `CorrelationId`. The "proposed default action" requirement is satisfied **Telegram-side**: the connector determines the proposed default from agent/command context at question-render time and stores it in `PendingQuestionRecord.DefaultActionId` (a nullable `string` matching a `HumanAction.ActionId` in `AllowedActions`). The `QuestionTimeoutService` reads `PendingQuestionRecord.DefaultActionId` at expiry: if present, it resolves the full `HumanAction` from `IDistributedCache` and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. This keeps the shared model stable across all messenger connectors. **Cross-doc note:** implementation-plan.md Stage 1.2 (line 36) currently lists `DefaultAction` as a first-class property on `AgentQuestion` — this is **inconsistent** with architecture.md §5.2 and this spec; implementation-plan.md should reconcile in its next iteration by removing `DefaultAction` from `AgentQuestion` and documenting it on `PendingQuestionRecord` instead. | Story requirement + architecture.md §5.2 (authoritative for shared model) |
...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering with Telegram-side default action via `PendingQuestionRecord.DefaultActionId`), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: the shared `AgentQuestion` does not carry `DefaultAction` (HC-3); the Telegram connector stores the proposed default in `PendingQuestionRecord.DefaultActionId` at render time; `QuestionTimeoutService` reads this field at expiry to apply the default or emit `__timeout__`. |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided — full oversight transfer with validation, notification, and audit | §5.5 (lines 486–498): full transfer with validation, notification, and audit — **aligned** with this spec | Stage 3.2 (line 207): `HandoffCommandHandler` performs full transfer with validation, notification, and audit — **aligned** with this spec | Lines 354–365: "Scenario: /handoff transfers oversight to another operator" describes full transfer — **aligned** with this spec | **All four documents are aligned on full oversight transfer for `/handoff`.** |
...
| DefaultAction model (HC-3) | Shared `AgentQuestion` does **not** include `DefaultAction`; default action is handled Telegram-side via `PendingQuestionRecord.DefaultActionId`, set by the connector at render time from agent/command context. | §5.2 (line 168): shared `AgentQuestion` does NOT include `DefaultAction`; default action handled Telegram-side via `PendingQuestionRecord.DefaultActionId` — **aligned** with this spec. | Stage 1.2 (line 36): currently lists `DefaultAction` as a first-class shared property on `AgentQuestion` — **inconsistent** with this spec and architecture.md §5.2. Implementation-plan should reconcile by removing `DefaultAction` from `AgentQuestion` and documenting it on `PendingQuestionRecord`. | e2e-scenarios should be verified for consistency with the Telegram-side model; if scenarios reference `AgentQuestion.DefaultAction` directly, they should be updated to reference `PendingQuestionRecord.DefaultActionId`. | **This spec and architecture.md are aligned.** Implementation-plan.md has a known inconsistency (flagged by architecture.md §5.2 line 173) and should reconcile in its next iteration. |
