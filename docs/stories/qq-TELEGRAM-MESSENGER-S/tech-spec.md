...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. The proposed default action is stored Telegram-side in `PendingQuestionRecord.DefaultActionId` when the question is rendered. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The epic attachment defines `AgentQuestion` as a shared record **without** a `DefaultAction` property — the shared model contains `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions`, `ExpiresAt`, and `CorrelationId`. The story requires "proposed default action" handling; this is implemented Telegram-side via `PendingQuestionRecord.DefaultActionId` (architecture.md §3.1, line 183–196). When the Telegram connector renders an `AgentQuestion` as an inline-keyboard message, it creates a `PendingQuestionRecord` and stores the proposed default action's `HumanAction.ActionId` in `DefaultActionId`. The `QuestionTimeoutService` reads `PendingQuestionRecord.DefaultActionId` at expiry: if present, it resolves the full `HumanAction` and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. This keeps the shared `AgentQuestion` model unchanged across all messenger connectors while allowing per-connector timeout handling. E2e-scenarios.md (lines 57–58, 71, 73) confirms this design: `AgentQuestion` is "unchanged" and the default action is stored in `PendingQuestionRecord.DefaultActionId`. | Epic shared data model (lines 831–841) + story requirement + architecture.md §3.1 (PendingQuestionRecord) + e2e-scenarios.md |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **(a) Transfer human oversight.** The handler accepts `/handoff TASK-ID @operator-alias`, validates that the task exists and the sending operator currently has oversight, validates that the target operator is registered via `OperatorRegistry`, transfers oversight by updating the binding, notifies both operators, and persists an audit record. Returns error for invalid task ID or unregistered operator. This aligns with implementation-plan.md Stage 3.2 (`HandoffCommandHandler` performs full transfer with validation, notification, and audit) and e2e-scenarios.md (lines 354–365: `/handoff transfers oversight to another operator`, both success and error scenarios). Architecture.md §5.5 (line 474) also defines the handler but includes a "pending policy decision (D-4)" note — with this decision resolved, that note should be updated in the next architecture iteration. | Decided |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided — full transfer of human oversight; handler validates task/operator, updates binding, notifies both operators, persists audit record | §5.5 (line 474): describes handler with "pending policy decision" note — should update to reflect decided status in next iteration | Stage 3.2 (line 207): full transfer flow with validation, notification, and audit — **aligned** with this spec | Lines 354–365: `/handoff transfers oversight to another operator` with success and error scenarios — **aligned** with this spec | **This spec, implementation-plan.md, and e2e-scenarios.md are aligned on full transfer semantics.** Architecture.md §5.5 has a stale "pending policy decision" note that should be updated in its next iteration to reflect this decided status. |
...
| DefaultAction model (HC-3) | Shared `AgentQuestion` does NOT include `DefaultAction` (per epic attachment). Default action handled Telegram-side via `PendingQuestionRecord.DefaultActionId`. | §3.1 (line 166): `AgentQuestion` table includes `DefaultAction` at line 177 — **inconsistent** with epic attachment (lines 831–841) which defines `AgentQuestion` without `DefaultAction`. §3.1 (line 183): `PendingQuestionRecord.DefaultActionId` stores default — **aligned** with this spec. | Stage 1.2 (line 36): adds `DefaultAction` to `AgentQuestion` — **inconsistent** with epic attachment. | Lines 57–58: "AgentQuestion is the shared model (unchanged)"; default stored in `PendingQuestionRecord.DefaultActionId` — **aligned** with this spec and epic attachment. | **This spec and e2e-scenarios.md are aligned with the epic attachment**: the shared `AgentQuestion` model is unchanged; the proposed default action is stored Telegram-side in `PendingQuestionRecord.DefaultActionId`. Architecture.md §3.1 and implementation-plan.md Stage 1.2 add `DefaultAction` directly to `AgentQuestion`, which contradicts the epic's shared model — those docs should reconcile in their next iterations. |
