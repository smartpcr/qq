...
| S-2 | **Command handling** | `/start`, `/status`, `/agents`, `/ask`, `/approve`, `/reject`, `/handoff`, `/pause`, `/resume` — parsed, validated, and dispatched to the Agent Swarm Orchestrator. `/handoff TASK-ID @operator-alias` semantics: all sibling documents are aligned on **full oversight transfer** (Decided). Architecture.md §5.5 (lines 527–540) defines the design; implementation-plan.md Stage 3.2 (lines 264–278) implements `HandoffCommandHandler` with full transfer — task validation, operator resolution via `IOperatorRegistry`, `TaskOversight` record creation, bidirectional notification, and audit; e2e-scenarios.md (lines 393–433) tests this flow end-to-end including success, error, and invalid-syntax scenarios. See D-4. |
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. **Default-action design — sidecar envelope model adopted (RESOLVED).** All sibling documents consistently use the sidecar approach: architecture.md §3.1 (lines 167–190) defines the shared `AgentQuestion` model **without** a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` sidecar metadata on `AgentQuestionEnvelope`. Implementation-plan.md Stage 1.2 follows the same sidecar approach. E2e-scenarios.md (lines 57–63) explicitly adopts the sidecar envelope model; lines 64–67 confirm all sibling documents mark DefaultAction placement as RESOLVED; footer (lines 707–708) reiterates the sidecar model. Architecture.md §3.1 (lines 167–193) implements the sidecar approach — the design is embodied in the model definition itself. **All documents aligned on DefaultAction — no remaining contradictions.** The Telegram connector reads `ProposedDefaultActionId` from the `AgentQuestionEnvelope`, denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays the default in the message body, and applies it on timeout. When the question times out, `QuestionTimeoutService` reads `DefaultActionId` and applies the action automatically; when `null`, the question expires with `ActionValue = "__timeout__"`. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. **Cross-document status — sidecar envelope model RESOLVED.** All sibling documents are aligned on the sidecar model: architecture.md §3.1 (lines 167–190) defines the shared `AgentQuestion` model **without** a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` on `AgentQuestionEnvelope`. Implementation-plan.md Stage 1.2 follows this approach. E2e-scenarios.md (lines 57–63) explicitly adopts the sidecar model; lines 64–67 confirm all sibling documents mark DefaultAction placement as RESOLVED; footer (lines 707–708) reiterates the sidecar model. Architecture.md §3.1 (lines 167–193) implements the sidecar approach — the design is embodied in the model definition. **All documents aligned on DefaultAction — no remaining editorial contradictions.** The behavioral contract: the Telegram connector reads `ProposedDefaultActionId` from the envelope, denormalizes it into `PendingQuestionRecord.DefaultActionId` for timeout polling, and applies it on timeout. When `null`, the question expires with `ActionValue = "__timeout__"`. | Story requirement; sidecar envelope model RESOLVED — all documents aligned (see Section 10) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Metric definitions adopted by this spec (see Section 10 for cross-document divergences).** Three metrics form the latency observability model: **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **dequeue instant** (`DequeuedAt`, when an `OutboundQueueProcessor` worker picks the message) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** delayed by a 429 rate-limit hold. This follows architecture.md §10.4 (line 679), which defines the acceptance gate as dequeue-to-HTTP-200. The **P95 ≤ 2 s** acceptance criterion applies to this metric. **`telegram.send.latency_ms`** (all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured for **all** messages regardless of attempt number, rate-limit holds, or queue dwell time — the all-inclusive monitoring metric for capacity planning. This follows architecture.md §10.4 (line 681), which names the all-inclusive metric `telegram.send.latency_ms`. **`telegram.send.queue_dwell_ms`** (diagnostic) = enqueue to dequeue — monitors queue backlog. Together, `queue_dwell_ms` + `first_attempt_latency_ms` ≈ `latency_ms` for first-attempt sends, so the story requirement "after event is queued" is covered by the combination of these metrics. Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends), `telegram.send.rate_limited_wait_ms` (429 backoff). Per operator answer to `p95-latency-scope`: the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately via `telegram.send.latency_ms`. Under normal operating conditions (the expected steady state), queue dwell is negligible (< 50 ms with 10 workers), so `first_attempt_latency_ms` (dequeue-to-200) effectively tracks the full enqueue-to-200 path and the P95 target is met. Under burst, priority queuing ensures Critical/High messages are dequeued first with minimal queue dwell, keeping their dequeue-to-200 latency within the 2 s window; Normal/Low first-attempt sends may exceed 2 s P95 under sustained burst due to accumulated queue dwell (per architecture.md §10.4). **Known cross-document divergence:** e2e-scenarios.md and parts of implementation-plan.md (line 566) describe the acceptance gate as enqueue-to-HTTP-200 and name the all-inclusive metric `telegram.send.all_attempts_latency_ms`. This spec follows architecture.md §8/§10.4 (dequeue-to-200, `latency_ms`) as the canonical source. See Section 10 for details. | Story requirement; operator answer `p95-latency-scope` confirms first-attempt, non-rate-limited scope; follows architecture.md §8/§10.4 as canonical source; see Section 10 for sibling divergences |
| HC-5 | **Zero message loss** for Critical, High, and Normal severity messages under burst from 100+ agents (1 000+ queued events). Under extreme backpressure (queue depth exceeds `MaxQueueDepth`, default 5 000), **only Low-severity** messages that cannot be enqueued are immediately **dead-lettered** — not silently discarded, not shed. Dead-lettered messages are persisted in the dead-letter queue with full failure context, counted via the `telegram.messages.backpressure_dlq` counter (canonical name per architecture.md §8 line 623 and §10.4 line 735), and trigger an ops-channel alert. Operators can replay dead-lettered messages from the DLQ once pressure subsides. This is consistent with architecture.md §10.4's priority-queuing design and the reliability model's "delivered-or-dead-lettered" contract. Critical, High, and Normal severity messages are never subject to backpressure rejection — they are always enqueued regardless of queue depth, consistent with architecture.md §10.4 which states "`Normal`, `High`, and `Critical` severity messages are always accepted." | Story + epic performance requirements + architecture.md §10.4 |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to `telegram.send.first_attempt_latency_ms` — the acceptance gate metric covering first-attempt, non-rate-limited sends only, measured from dequeue to HTTP 200 (per architecture.md §10.4 line 679 and operator answer to `p95-latency-scope`). The all-inclusive metric `telegram.send.latency_ms` tracks all sends including retries and rate-limit waits for capacity planning (per architecture.md §10.4 line 681). See HC-4 and Section 10 for cross-document divergence notes. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **(a) Full transfer of human oversight — Decided per architecture.md, aligned across all sibling documents.** Architecture.md §5.5 (lines 527–540) specifies full oversight transfer (Decided): the handler accepts `/handoff TASK-ID @operator-alias` syntax and performs: (1) validates syntax (two arguments: task ID and operator alias); (2) validates that the specified task exists and the sending operator currently has oversight; (3) validates that the target operator (`@operator-alias`) is registered via `OperatorRegistry`; (4) transfers oversight by creating/updating a `TaskOversight` record (architecture.md §5.5 line 534) mapping the task to the target operator; (5) notifies both operators — the sender receives confirmation, the target receives a handoff notification with task context; (6) persists an audit record with handoff details (task ID, source operator, target operator, timestamp, `CorrelationId`); (7) returns error for invalid task ID, unregistered target operator, or missing arguments with usage help. Implementation-plan.md Stage 3.2 (lines 264–278) implements `HandoffCommandHandler` with full oversight transfer matching this specification — including task validation, operator resolution via `IOperatorRegistry`, `TaskOversight` record creation/update, bidirectional notification, and audit (specifically at line 275). E2e-scenarios.md (lines 393–433) tests the full transfer flow end-to-end, including success, nonexistent task, unregistered target, and invalid syntax scenarios. **All documents are aligned.** | Decided per architecture.md §5.5 |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided — full oversight transfer per architecture.md §5.5 (lines 527–540), with validation, notification, and audit | §5.5 (lines 527–540): "Full oversight transfer (Decided)" — specifies full transfer with validation, `TaskOversight` record creation, notification, and audit. **Aligned** with this spec. | Stage 3.2 (lines 264–278): implements `HandoffCommandHandler` with **full oversight transfer** — task validation, operator resolution via `IOperatorRegistry`, `TaskOversight` record creation/update, bidirectional notification, and audit (line 275). **Aligned** with this spec and architecture.md. | Lines 393–433: six `/handoff` scenarios — success transfer, nonexistent task, unregistered target operator, invalid syntax (no args), invalid syntax (one arg). Tests full oversight transfer with task validation, operator resolution via `IOperatorRegistry`, `TaskOversight` record creation, bidirectional notification, and audit. **Aligned** with this spec and architecture.md. | **All documents aligned.** No reconciliation needed. |
...
| P95 latency scoping (HC-4) | **This spec follows architecture.md §10.4 as canonical source.** `telegram.send.first_attempt_latency_ms` (acceptance gate) = **dequeue** to HTTP 200, first-attempt, non-rate-limited; P95 ≤ 2 s applies. `telegram.send.latency_ms` (all-inclusive) = enqueue to HTTP 200, all sends — capacity planning. `telegram.send.queue_dwell_ms` (diagnostic) = enqueue to dequeue. Together, `queue_dwell_ms` + `first_attempt_latency_ms` ≈ `latency_ms` for first-attempt sends. Under normal load, queue dwell is negligible so the dequeue-based gate effectively tracks the full enqueue-to-200 path. Per operator answer to `p95-latency-scope`, the P95 target applies to first-attempt, non-rate-limited sends only. | §8 (line 626): defines `telegram.send.first_attempt_latency_ms` as "dequeue to HTTP 200, first-attempt, non-rate-limited sends only — P95 ≤ 2 s target." All-inclusive metric: `telegram.send.latency_ms` (enqueue to HTTP 200 regardless of attempt or rate-limit). §10.4 (lines 679–689): same definitions — acceptance gate is dequeue-based, all-inclusive is enqueue-based. **This spec is aligned with architecture.md.** | Line 369: measures `first_attempt_latency_ms` from **dequeue** to HTTP 200, uses `latency_ms` as all-inclusive — **aligned** with architecture.md and this spec. Line 566: measures `first_attempt_latency_ms` from **enqueue** and uses `all_attempts_latency_ms` — **diverges** from line 369, architecture.md, and this spec. **Internal inconsistency within implementation-plan.md.** | Lines 265–300 (normal/burst scenarios): uses `first_attempt_latency_ms` as acceptance gate measured from **enqueue** (diverges from architecture.md dequeue-based definition). Uses `telegram.send.all_attempts_latency_ms` as all-inclusive metric name (diverges from architecture.md `telegram.send.latency_ms`). Metrics table (lines 650–651) and footer (line 712) confirm enqueue-based measurement and `all_attempts_latency_ms` naming. Footer (line 712) explicitly notes this spec's prior use of dequeue-based measurement as "a known editorial divergence." | **Known divergence — not yet fully reconciled.** Architecture.md and this spec use **dequeue-to-200** measurement + `latency_ms` naming. E2e-scenarios.md uses **enqueue-to-200** measurement + `all_attempts_latency_ms` naming. Implementation-plan.md is internally inconsistent (line 369 follows architecture.md, line 566 follows e2e-scenarios.md). All four documents agree on: (1) `first_attempt_latency_ms` is the acceptance gate, (2) P95 ≤ 2 s applies to first-attempt non-rate-limited sends only, (3) a separate all-inclusive metric captures all sends for capacity planning. The disagreements are on the measurement start point (dequeue vs. enqueue) and the all-inclusive metric's name (`latency_ms` vs. `all_attempts_latency_ms`). **Reconciliation required before implementation:** the sibling documents should align on architecture.md §10.4 as the canonical definition. |
...
| DefaultAction model (HC-3) | **Sidecar envelope model RESOLVED.** `AgentQuestion` has no `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` on `AgentQuestionEnvelope`. | §3.1 (lines 167–190): models `AgentQuestion` **without** a `DefaultAction` property. Proposed default carried as `ProposedDefaultActionId` sidecar metadata on `AgentQuestionEnvelope`. Line 193: describes default action flow. Line 195: heading for `PendingQuestionRecord` — the persistence model that denormalizes `DefaultActionId` from the envelope. The sidecar design is embodied in the model definitions. **Aligned.** | Stage 1.2: defines `AgentQuestion` **without** `DefaultAction`; creates `AgentQuestionEnvelope` with `ProposedDefaultActionId`. **Aligned.** | Lines 57–63: explicitly adopts the sidecar envelope model. Lines 64–67: confirm all sibling documents mark DefaultAction placement as RESOLVED and consistently adopt the sidecar envelope model. Footer (lines 707–708): reiterates the sidecar model — "`AgentQuestion` model does not include a `DefaultAction` property. The proposed default action is carried as `ProposedDefaultActionId` on the `AgentQuestionEnvelope`." **Aligned.** | **All documents aligned** on the sidecar envelope model. No remaining contradictions. |
