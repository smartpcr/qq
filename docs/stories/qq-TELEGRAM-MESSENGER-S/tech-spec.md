...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. **Cross-document status — sidecar envelope model RESOLVED.** All sibling documents are aligned on the sidecar model: architecture.md §3.1 (lines 167–190) defines the shared `AgentQuestion` model **without** a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` on `AgentQuestionEnvelope`. Implementation-plan.md Stage 1.2 follows this approach. E2e-scenarios.md (lines 57–63) explicitly adopts the sidecar model; lines 64–66 confirm "All sibling documents mark DefaultAction placement as RESOLVED"; footer line 683 confirms the same. Architecture.md §3.1 (lines 167–193) implements the sidecar approach — the design is embodied in the model definition. **All documents aligned — no remaining editorial contradictions.** The behavioral contract: the Telegram connector reads `ProposedDefaultActionId` from the envelope, denormalizes it into `PendingQuestionRecord.DefaultActionId` for timeout polling, and applies it on timeout. When `null`, the question expires with `ActionValue = "__timeout__"`. | Story requirement; sidecar envelope model RESOLVED — all documents aligned (see Section 10) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Metric definitions adopted by this spec, aligned with architecture.md §8/§10.4 (canonical source), e2e-scenarios.md, and implementation-plan.md.** **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **enqueue instant** (`OutboundMessage.CreatedAt`) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** delayed by a 429 rate-limit hold. The **P95 ≤ 2 s** acceptance criterion applies to this metric. Measuring from the enqueue instant preserves the story requirement "P95 send latency under 2 seconds **after event is queued**." **`telegram.send.all_attempts_latency_ms`** (all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured for **all** messages regardless of attempt number, rate-limit holds, or queue dwell time — the all-inclusive monitoring metric for capacity planning. **`telegram.send.queue_dwell_ms`** (diagnostic) = enqueue to dequeue — monitors queue backlog. Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends), `telegram.send.rate_limited_wait_ms` (429 backoff). Per operator answer to `p95-latency-scope`: the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately via `telegram.send.all_attempts_latency_ms`. Under normal operating conditions (the expected steady state), queue dwell is negligible (< 50 ms with 10 workers), so the enqueue-to-200 acceptance gate is effectively met. Under burst, priority queuing ensures Critical/High messages have minimal dwell and meet the 2 s target; Normal/Low first-attempt sends may exceed 2 s P95 under sustained burst due to accumulated queue dwell (per architecture.md §10.4). **All sibling documents are aligned** on enqueue-to-HTTP-200 measurement and the metric names defined above (see Section 10). | Story requirement; operator answer `p95-latency-scope` confirms first-attempt, non-rate-limited scope; aligned with architecture.md §8/§10.4, e2e-scenarios.md, implementation-plan.md |
...
| P95 latency scoping (HC-4) | **This spec's position (aligned with all sibling docs):** `telegram.send.first_attempt_latency_ms` (acceptance gate) measures from **enqueue instant** (`OutboundMessage.CreatedAt`) to HTTP 200, first-attempt, non-rate-limited; P95 ≤ 2 s applies. `telegram.send.all_attempts_latency_ms` (all-inclusive) measures from enqueue to HTTP 200 for all sends regardless of attempt number or rate-limit holds — capacity planning. `telegram.send.queue_dwell_ms` (diagnostic) tracks enqueue to dequeue — queue backlog monitoring. Per operator answer to `p95-latency-scope`, the P95 target applies to first-attempt, non-rate-limited sends only. The story's "after event is queued" language is directly preserved by measuring from the enqueue instant. | §8 (line 623): defines `telegram.send.first_attempt_latency_ms` as "enqueue to HTTP 200, first-attempt, non-rate-limited sends only — P95 ≤ 2 s target." All-inclusive metric: `telegram.send.all_attempts_latency_ms` (enqueue to HTTP 200 regardless of attempt or rate-limit). §10.4: same definitions. **Aligned** with this spec. | Lines 367, 553: measures `first_attempt_latency_ms` from `OutboundMessage.CreatedAt` (**enqueue**) to HTTP 200. Uses `first_attempt_latency_ms` as acceptance gate and `all_attempts_latency_ms` as all-inclusive. **Aligned** with this spec and architecture.md. | Lines 265–283 (normal load), 287–300 (burst): uses `telegram.send.first_attempt_latency_ms` as acceptance gate (enqueue to HTTP 200, first-attempt, non-rate-limited), `telegram.send.all_attempts_latency_ms` as all-inclusive (enqueue to HTTP 200, all sends). Metrics table (line 625–627) and footer (line 688) confirm enqueue-based measurement and `all_attempts_latency_ms` naming. **Aligned** with this spec and architecture.md. | **All documents aligned.** All four documents agree on: (1) metric names (`first_attempt_latency_ms` = acceptance gate, `all_attempts_latency_ms` = all-inclusive, `queue_dwell_ms` = diagnostic); (2) measurement start point (enqueue instant — `OutboundMessage.CreatedAt`); (3) P95 ≤ 2 s applies to first-attempt, non-rate-limited sends only. No reconciliation needed. |
