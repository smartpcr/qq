...
| S-2 | **Command handling** | `/start`, `/status`, `/agents`, `/ask`, `/approve`, `/reject`, `/handoff`, `/pause`, `/resume` — parsed, validated, and dispatched to the Agent Swarm Orchestrator. `/handoff` accepts syntax `/handoff TASK-ID @operator-alias` and validates arguments, but V1 returns a stub acknowledgement pending policy decision (see D-4). |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. Architecture.md (§5, line 177) extends the shared `AgentQuestion` record with a `DefaultAction` property (nullable `string` matching one of `HumanAction.Value` entries in `AllowedActions`). Implementation-plan.md Stage 1.2 creates `AgentQuestion` with `DefaultAction` as a first-class property. E2e-scenarios.md uses `DefaultAction` as the Gherkin table field name (lines 66, 83), directly matching the `AgentQuestion.DefaultAction` property. This connector consumes `AgentQuestion.DefaultAction` directly — no Telegram-specific override or `default:` prefix convention is needed. When `DefaultAction` is present, the timeout handler applies it automatically; when absent (`null`), the question expires with a `__timeout__` action value. | Epic shared data model + story requirement + architecture.md §5 + implementation-plan.md Stage 1.2 |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **Open — stub implementation for V1.** The handler accepts `/handoff TASK-ID @operator-alias`, validates syntax, and returns a stub acknowledgement: "Handoff is not yet configured — awaiting policy decision (D-4)." No transfer, question reassignment, or agent-team change occurs. An audit record is persisted for the attempt. This aligns with e2e-scenarios.md (line 338: "/handoff returns stub response pending policy decision (D-4)", line 346: "no transfer, question reassignment, or agent-team change occurs") and with implementation-plan.md's alignment note (line 553: "Stage 3.2 HandoffCommandHandler returns stub pending D-4"). **Sibling inconsistency noted:** implementation-plan.md line 207 describes a full transfer flow for `HandoffCommandHandler` that contradicts both the e2e Gherkin stub scenario and its own alignment footer — it should update to match the stub behavior. Architecture.md §5.1 may also describe decided transfer semantics that should be reconciled. The full transfer semantic remains the likely future direction, but this spec defers the decision until cross-doc alignment is achieved and operator policy is confirmed. | Open (stub for V1) |
...
| **Gateway** | The host worker service that loads connectors. Planned namespace: `AgentSwarm.Messaging.Worker` (to be created per implementation-plan.md Stage 1.1). |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Open — stub for V1; no transfer occurs; audit record persisted for the attempt | Describes full transfer flow (line 445) — **needs update to stub** | **Inconsistent:** `HandoffCommandHandler` (line 207) describes full transfer, but alignment footer (line 553) says "returns stub pending D-4" — needs internal reconciliation to match stub behavior | Stub (line 338): "/handoff returns stub response pending policy decision (D-4)"; no transfer, reassignment, or agent-team change occurs (line 346); invalid-syntax variant returns usage help (line 349) | **This spec and e2e-scenarios.md are aligned on stub behavior.** architecture.md and implementation-plan.md body text describe full transfer semantics but are inconsistent with the e2e Gherkin scenarios and impl-plan's own alignment footer. Those docs should update to stub behavior in their next iterations. The full transfer semantic is the likely future direction once operator policy is confirmed. |
...
| DefaultAction model (HC-3) | `AgentQuestion.DefaultAction` — shared model property consumed directly by Telegram connector | `AgentQuestion.DefaultAction` as nullable string (§5, line 177); extension note explains it extends the epic attachment's model | `AgentQuestion` with `DefaultAction` property (Stage 1.2, line 36) | `DefaultAction` Gherkin field (lines 66, 83) maps directly to `AgentQuestion.DefaultAction` | **Aligned** across all docs. `DefaultAction` is a first-class property on the shared `AgentQuestion` record. |
