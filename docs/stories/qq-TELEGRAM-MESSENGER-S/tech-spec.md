...
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Metric definitions adopted by this spec follow architecture.md §8/§10.4 as the canonical source (see Section 10 for cross-document verification).** Three metrics form the latency observability model: **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **enqueue instant** (`OutboundMessage.CreatedAt`, the moment the message is persisted to the outbound queue) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** delayed by a 429 rate-limit hold. This directly preserves the story requirement "P95 send latency under 2 seconds **after event is queued**" by starting the clock at enqueue, not dequeue. The **P95 ≤ 2 s** acceptance criterion applies to this metric. **`telegram.send.all_attempts_latency_ms`** (all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured for **all** messages regardless of attempt number or rate-limit holds — the all-inclusive monitoring metric for capacity planning. **`telegram.send.queue_dwell_ms`** (diagnostic) = enqueue to dequeue — monitors queue backlog; under normal operating conditions this is negligible (< 50 ms with 10 workers). Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends), `telegram.send.rate_limited_wait_ms` (429 backoff). Per operator answer to `p95-metric-scope`: the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately via `telegram.send.all_attempts_latency_ms`. Under normal operating conditions (the expected steady state), queue dwell is negligible, so `first_attempt_latency_ms` comfortably meets P95 ≤ 2 s across all severities. Under burst, priority queuing ensures Critical/High messages are dequeued first with minimal queue dwell, keeping their enqueue-to-200 latency within the 2 s window; Normal/Low first-attempt sends may exceed 2 s P95 under sustained burst due to accumulated queue dwell (per architecture.md §10.4). **Cross-document status: ALL ALIGNED.** All sibling documents (architecture.md, implementation-plan.md, e2e-scenarios.md) now use **enqueue-to-HTTP-200** measurement and `telegram.send.all_attempts_latency_ms` naming for the all-inclusive metric, consistent with this spec. See Section 10 for verification details. | Story requirement; operator answer `p95-metric-scope` confirms first-attempt, non-rate-limited scope; follows architecture.md §8/§10.4 as canonical source; all sibling documents aligned (see Section 10) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to `telegram.send.first_attempt_latency_ms` — the acceptance gate metric covering first-attempt, non-rate-limited sends only, measured from enqueue (`OutboundMessage.CreatedAt`) to HTTP 200 (per architecture.md §8/§10.4 and operator answer to `p95-metric-scope`). The all-inclusive metric `telegram.send.all_attempts_latency_ms` tracks all sends including retries and rate-limit waits for capacity planning (per architecture.md §8/§10.4). See HC-4 and Section 10. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
The following items were identified as cross-document alignment points during iteration reviews. All items have been verified as aligned across sibling documents.
