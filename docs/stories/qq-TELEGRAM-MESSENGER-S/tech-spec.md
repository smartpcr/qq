...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. Per **architecture.md §3.1** (line 168), the shared `AgentQuestion` does **not** include a `DefaultAction` property — the story requirement for "proposed default action" is satisfied connector-side: the proposed default action is provided as metadata alongside the question (e.g., via the agent/command context or a sidecar payload) and stored in the Telegram-specific `PendingQuestionRecord.DefaultActionId` at render time. This keeps the shared model stable across all messenger connectors. **Cross-doc note:** implementation-plan.md Stage 1.2 (line 36) currently puts `DefaultAction` directly on `AgentQuestion` as a first-class property — this contradicts architecture.md §3.1 (line 168). This spec aligns with architecture.md as authoritative for shared model definitions; implementation-plan.md should reconcile by removing `DefaultAction` from `AgentQuestion` and handling it connector-side. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. Per **architecture.md §3.1** (line 168, authoritative for the shared model), the shared `AgentQuestion` does **not** include a `DefaultAction` property. The `AgentQuestion` properties are: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions`, `ExpiresAt`, and `CorrelationId`. The story requirement for "proposed default action" is satisfied **connector-side**: the proposed default action is provided as metadata alongside the question and stored in the Telegram-specific `PendingQuestionRecord.DefaultActionId` at render time. The `QuestionTimeoutService` reads `PendingQuestionRecord.DefaultActionId` at expiry: if present, it resolves the full `HumanAction` from `IDistributedCache` and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. This keeps the shared model connector-agnostic. **Cross-doc note:** implementation-plan.md Stage 1.2 (line 36) currently puts `DefaultAction` directly on `AgentQuestion` as a shared first-class property, and Stages 2.4 (line 142) and 3.5 (lines 261–262) reference `AgentQuestion.DefaultAction` as the source for `PendingQuestionRecord.DefaultActionId`. These implementation-plan references contradict architecture.md §3.1 (line 168). This spec aligns with architecture.md; implementation-plan.md should reconcile by removing `DefaultAction` from `AgentQuestion` and sourcing the default action from connector-side metadata. | Story requirement + architecture.md §3.1 (authoritative for shared model) |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **(a) Full transfer of human oversight — Decided per architecture.md.** Architecture.md §5.5 (lines 480–493) specifies full oversight transfer (Decided): the handler accepts `/handoff TASK-ID @operator-alias` syntax and performs: (1) validates syntax (two arguments: task ID and operator alias); (2) validates that the specified task exists and the sending operator currently has oversight; (3) validates that the target operator (`@operator-alias`) is registered via `OperatorRegistry`; (4) transfers oversight by creating/updating a `TaskOversight` record (architecture.md §5.5 line 491) mapping the task to the target operator; (5) notifies both operators — the sender receives confirmation, the target receives a handoff notification with task context; (6) persists an audit record with handoff details (task ID, source operator, target operator, timestamp, `CorrelationId`); (7) returns error for invalid task ID, unregistered target operator, or missing arguments with usage help. Implementation-plan.md Stage 3.2 (line 211) also specifies full transfer. **Cross-doc contradiction:** e2e-scenarios.md (lines 357–370) currently describes `/handoff` as a **V1 stub** that validates syntax only, returns a stub ack, persists an audit record, but does NOT validate task existence, resolve the target operator, mutate any `OperatorBinding` or `TaskOversight` record, or notify the target operator. The e2e-scenarios.md stub scenario contains a stale comment (line 358: "architecture.md §5.5 (line 481): 'V1 stub; full transfer deferred.'") that does not match architecture.md's current text, which says "Full oversight transfer (Decided)". **This spec aligns with architecture.md §5.5 (authoritative for architectural decisions); e2e-scenarios.md should reconcile by updating the /handoff scenario from V1 stub to full transfer.** | Decided per architecture.md §5.5 |
...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: per architecture.md §3.1 (line 168), the shared `AgentQuestion` does **not** include a `DefaultAction` property; the story requirement for "proposed default action" is satisfied connector-side via `PendingQuestionRecord.DefaultActionId` (HC-3). `QuestionTimeoutService` reads this field at expiry to apply the default or emit `__timeout__`. |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided — full oversight transfer per architecture.md §5.5, with validation, notification, and audit | §5.5 (lines 480–493): "Full oversight transfer (Decided)" — specifies full transfer with validation, `TaskOversight` record creation, notification, and audit — **aligned** with this spec | Stage 3.2 (line 211): implements `HandoffCommandHandler` performing **full oversight transfer** — **aligned** with this spec and architecture.md §5.5 | Lines 357–370: "Scenario: /handoff returns V1 stub acknowledgement" — validates syntax only, returns stub ack, persists audit record, but does NOT validate task existence, resolve the target operator, mutate any records, or notify the target operator — **not aligned** with this spec, architecture.md, or implementation-plan.md. Contains stale comment (line 358) referencing architecture.md as "V1 stub; full transfer deferred" which does not match architecture.md's current text. | **This spec, architecture.md, and implementation-plan.md are aligned on full oversight transfer.** E2e-scenarios.md is the outlier: it tests a V1 stub with a stale cross-reference to an older version of architecture.md. **E2e-scenarios.md should reconcile** by updating the /handoff scenario from V1 stub to full transfer per architecture.md §5.5. |
...
| DefaultAction model (HC-3) | Shared `AgentQuestion` does **not** include `DefaultAction` (per architecture.md §3.1 line 168). Story requirement for "proposed default action" is satisfied connector-side via `PendingQuestionRecord.DefaultActionId`. | §3.1 (line 168): explicitly states `AgentQuestion` does **not** include `DefaultAction`; the proposed default action is provided as metadata alongside the question and stored connector-side in `PendingQuestionRecord.DefaultActionId` — **aligned** with this spec. | Stage 1.2 (line 36): puts `DefaultAction` directly on `AgentQuestion` as a shared first-class property. Stage 2.4 (line 142) and Stage 3.5 (lines 261–262) reference `AgentQuestion.DefaultAction` as the source for `PendingQuestionRecord.DefaultActionId`. These **contradict** architecture.md §3.1 (line 168) and this spec. | Timeout/default-action flow uses `PendingQuestionRecord.DefaultActionId` — **functionally aligned** with this spec's timeout flow regardless of where `DefaultAction` originates. | **This spec aligns with architecture.md §3.1** (authoritative for shared model definitions). Implementation-plan.md is the outlier: Stage 1.2, 2.4, and 3.5 put `DefaultAction` on `AgentQuestion` as a shared property, contradicting architecture.md. **Implementation-plan.md should reconcile** by removing `DefaultAction` from `AgentQuestion` and sourcing the default action from connector-side metadata. E2e-scenarios.md is functionally aligned on the timeout/default-action flow. |
