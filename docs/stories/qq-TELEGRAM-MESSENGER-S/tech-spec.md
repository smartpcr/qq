...
The following items were identified as cross-document alignment points during iteration reviews. As of iteration 15, most sibling documents are aligned on key design decisions, with one notable inconsistency on `DefaultAction` ownership between architecture.md/implementation-plan.md and e2e-scenarios.md:
...
| P95 latency scoping (HC-4) | < 2 s, applies to **all sends** including rate-limit waits per architecture.md §10.4 | §10.4 (line 671): "Rate-limited wait time is included in the primary `telegram.send.latency_ms` metric"; (line 680): "The P95 target of 2 seconds applies to all sends." **Aligned** with this spec. | Not specified | Burst test asserts P95 < 2 s | **Aligned.** The primary `telegram.send.latency_ms` metric includes all sends and rate-limit waits. The P95 ≤ 2 s target applies to this all-inclusive metric. Priority queuing ensures Critical/High messages meet the target even under burst. |
...
| DefaultAction model (HC-3) | Shared `AgentQuestion` **includes** nullable `DefaultAction` (`string?`) per architecture.md §3.1. Telegram connector reads it at render time and denormalizes into `PendingQuestionRecord.DefaultActionId`. | §3.1 (lines 167–184): explicitly includes `DefaultAction` as a first-class shared field in the `AgentQuestion` field table (line 180). Describes the full default-action flow (line 184). **Aligned** with this spec. | Stage 1.2 (line 36): defines `AgentQuestion` with `DefaultAction` as a nullable `string` — the `ActionId` of the proposed default action from `AllowedActions`, matching architecture.md §3.1. Stage 2.4 (line 142): reads `AgentQuestion.DefaultAction` (the shared first-class property) at render time and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId`. Stage 3.5 (line 270): stores `DefaultActionId` denormalized from `AgentQuestion.DefaultAction` for timeout polling. **Aligned** with architecture.md §3.1 and this spec. | Lines 57–76: **⚠ NOT ALIGNED.** e2e-scenarios.md states "The shared AgentQuestion model does NOT include a DefaultAction property" (line 57) and treats the proposed default as **sidecar metadata** — `ProposedDefaultActionId` in agent/command context metadata (lines 59–62, 74). The Telegram connector reads `ProposedDefaultActionId` from context metadata, not from `AgentQuestion`. This directly contradicts architecture.md §3.1 (which includes `DefaultAction` on `AgentQuestion`) and implementation-plan.md (which reads `AgentQuestion.DefaultAction`). | **Cross-doc inconsistency requiring reconciliation.** architecture.md and implementation-plan.md agree that `DefaultAction` is a first-class property on `AgentQuestion`. e2e-scenarios.md disagrees and uses sidecar metadata. This must be resolved before implementation: either e2e-scenarios.md updates its test fixtures to use `AgentQuestion.DefaultAction`, or architecture.md and implementation-plan.md adopt the sidecar-metadata approach. This spec follows architecture.md §3.1 as the authoritative shared-model definition. |
