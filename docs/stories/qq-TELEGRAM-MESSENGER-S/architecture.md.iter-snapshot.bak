...
> **Cross-doc status — UNRESOLVED divergence on metric naming.** This architecture and tech-spec.md HC-4/R-1 define `telegram.send.latency_ms` as the **all-inclusive** primary metric (all sends, including retries and rate-limit waits; P95 ≤ 2 s applies here) and `telegram.send.first_attempt_latency_ms` as a narrower diagnostic metric (first-attempt, non-rate-limited successes only). However, e2e-scenarios.md (lines 232–237, 270–285, 593–594, footer lines 655–656) uses the **opposite** naming convention: it defines `telegram.send.latency_ms` as first-attempt/non-rate-limited only (primary, P95 ≤ 2 s applies) and `telegram.send.all_attempts_latency_ms` as the all-inclusive diagnostic metric. The behavioral intent is identical — both approaches define a primary metric the P95 target applies to and a secondary diagnostic metric — but the metric **names** and which population each name covers are swapped. This must be reconciled before implementation to avoid conflicting instrumentation. This architecture's position: the all-inclusive definition is the honest reading of the story requirement ("P95 send latency under 2 seconds after event is queued"), so the primary metric name `telegram.send.latency_ms` should cover all sends.
