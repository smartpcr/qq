...
_Metric naming: Per tech-spec.md HC-4, `telegram.send.latency_ms` (primary) measures elapsed time from `OutboundMessage.CreatedAt` (enqueue) to Telegram Bot API HTTP 200 **only** for messages that succeed on their **first delivery attempt** and are **not** waiting behind a 429 rate-limit hold — this is the **P95 ≤ 2s acceptance gate metric** (per operator answer to p95-metric-scope: first-attempt, non-rate-limited only). `telegram.send.all_attempts_latency_ms` (diagnostic) measures elapsed time for **all** sends regardless of attempt number or rate-limit holds — this is the **all-inclusive metric** for capacity planning and operational dashboards. Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends) and `telegram.send.rate_limited_wait_ms` (429 backoff duration). Backpressure dead-letter counter: `telegram.messages.backpressure_dlq` (per architecture.md §10.4 — canonical name). Queue depth and error counters: `telegram.queue.depth`, `telegram.dlq.depth`, `telegram.errors` (per implementation-plan.md Stage 6.1)._
_P95 metric scope: Per operator answer to p95-metric-scope, the P95 ≤ 2s acceptance criterion applies to first-attempt, non-rate-limited sends only (`telegram.send.latency_ms`). All sends (including retried and rate-limited) are tracked via the all-inclusive `telegram.send.all_attempts_latency_ms` for capacity planning, plus `telegram.send.rate_limited_wait_ms` for 429 backoff visibility. Under normal operating conditions (no rate-limit hits, low queue depth), all severities comfortably meet the 2-second target. Under extreme burst (100+ agents), priority queuing (architecture.md §5.2, §10.4) ensures Critical/High messages are dispatched first and meet the P95 ≤ 2s target; Normal/Low messages may queue-delay beyond 2 seconds transiently but are not lost._
