...
5. **`IDistributedCache` expiry — RESOLVED.** Architecture.md §5.2 invariant 2 and §11 now specify cache expiry at `AgentQuestion.ExpiresAt + 5 minutes` (grace window for late callbacks near expiry), matching implementation-plan.md Stage 2.3. `QuestionTimeoutService` does not depend on the cache.
