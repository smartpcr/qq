...
| **TelegramSender** | `AgentSwarm.Messaging.Telegram` (to be created) | Concrete `TelegramMessageSender` implementing `IMessageSender` (§4.12, defined in `AgentSwarm.Messaging.Core`). Wraps `ITelegramBotClient` from the `Telegram.Bot` library. Formats messages with MarkdownV2, builds `InlineKeyboardMarkup` for agent questions, enforces Telegram rate limits. `OutboundQueueProcessor` invokes the sender through the `IMessageSender` abstraction, keeping `AgentSwarm.Messaging.Worker` independent of the Telegram assembly. |
...
> **Cross-doc alignment (IPendingQuestionStore).** This document (§4.7) and implementation-plan.md Stage 1.3 should declare the same interface surface. The canonical interface defined here (§4.7) includes eight methods: `StoreAsync`, `GetAsync`, `GetByTelegramMessageAsync`, `MarkAnsweredAsync`, `MarkAwaitingCommentAsync`, `RecordSelectionAsync`, `GetAwaitingCommentAsync`, and `GetExpiredAsync`. The `RecordSelectionAsync` and `GetAwaitingCommentAsync` methods were added to the abstraction interface (iteration 9) to close an abstraction-boundary gap: previously, `CallbackQueryHandler` (in `AgentSwarm.Messaging.Telegram`) set `SelectedActionId`/`RespondentUserId` through the concrete `PersistentPendingQuestionStore` (in `AgentSwarm.Messaging.Persistence`), which violated implementation-plan.md Stage 3.5's stated constraint that "the Telegram project depends on Abstractions and Core but not Persistence." By promoting these operations to the abstraction interface, `CallbackQueryHandler` and the text-reply handler can operate through `IPendingQuestionStore` without a compile-time dependency on the Persistence assembly. **Implementation-plan.md Stage 1.3 should add `RecordSelectionAsync` and `GetAwaitingCommentAsync` to its `IPendingQuestionStore` definition in its next iteration.** The canonical method name `GetByTelegramMessageAsync(long telegramChatId, long telegramMessageId, CancellationToken)` accepts **two** identifiers because Telegram `message_id` is only unique within a chat. Implementation-plan.md Stage 1.3 uses this exact composite-key signature and method name (`GetByTelegramMessageAsync`, not `GetByTelegramMessageIdAsync`).
...
public interface IMessageSender
{
    ...
    Task SendQuestionAsync(long chatId, AgentQuestionEnvelope envelope, CancellationToken ct);
}
```

The concrete `TelegramMessageSender` (in `AgentSwarm.Messaging.Telegram`) implements this interface and wraps `ITelegramBotClient` from the `Telegram.Bot` library. It formats messages with MarkdownV2, builds `InlineKeyboardMarkup` for agent questions, writes `HumanAction` entries to `IDistributedCache`, and enforces Telegram rate limits via the dual-layer token-bucket limiter (§10.4). In component diagrams and sequence flows throughout this document, **`TelegramSender`** refers to this concrete `TelegramMessageSender` implementation.

> **Cross-doc alignment (IMessageSender).** Implementation-plan.md Stage 1.4 defines `IMessageSender` in Core with the same two-method contract: `SendTextAsync` and `SendQuestionAsync`. Stage 2.3 provides the concrete `TelegramMessageSender` implementing `IMessageSender`. Stage 4.1 (`OutboundQueueProcessor`) sends via `IMessageSender` (not the concrete type) so that Core/Worker assemblies do not depend on `AgentSwarm.Messaging.Telegram`. The component table in §2.2 lists `TelegramSender` as the short name for the concrete implementation — this is the same component as implementation-plan.md's `TelegramMessageSender`.
...
#### Outstanding discrepancies (require sibling-doc updates)

1. **`IMessageSender` denormalization wording — OUTSTANDING (implementation-plan.md Stage 1.4 line 96, Stage 2.3 line 153).** Implementation-plan.md Stage 1.4 line 96 describes `IMessageSender` as using `AgentQuestionEnvelope` "so the sender can read `ProposedDefaultActionId` from sidecar metadata and denormalize it into `PendingQuestionRecord.DefaultActionId`." Implementation-plan.md Stage 2.3 line 153 says `SendQuestionAsync` will "denormalize the `ActionId` into `PendingQuestionRecord.DefaultActionId`". Both contradict this architecture: the sender only renders/caches/enqueues; the `OutboundQueueProcessor` and `IPendingQuestionStore` own denormalization at persistence time (§5.2 invariant 1). Implementation-plan.md lines 359 and 403 correctly assign persistence to the processor. **Required fix in implementation-plan.md:** Stage 1.4 line 96 should say the sender reads `ProposedDefaultActionId` to *display* the proposed default in the message body, not to denormalize it into `PendingQuestionRecord`. Stage 2.3 line 153 should remove the denormalization clause ("and denormalize the `ActionId` into `PendingQuestionRecord.DefaultActionId`").

2. **`IPendingQuestionStore` interface surface — OUTSTANDING (implementation-plan.md Stage 1.3).** This architecture (§4.7, iteration 9) added `RecordSelectionAsync` and `GetAwaitingCommentAsync` to `IPendingQuestionStore` to close the abstraction-boundary gap where `CallbackQueryHandler` (Telegram assembly) previously required a compile-time dependency on the Persistence assembly to set `SelectedActionId`/`RespondentUserId`. Implementation-plan.md Stage 1.3 should add these two methods to its `IPendingQuestionStore` definition.

#### Resolved items (iterations 3–8)
...
8. **`PendingQuestionRecord` post-send persistence — PARTIALLY RESOLVED (processor ownership aligned; sender wording still contradicts in Stage 1.4 line 96 and Stage 2.3 line 153).** Architecture.md §5.2 invariant 1 assigns `PendingQuestionRecord` persistence to `OutboundQueueProcessor` as a post-send hook. Implementation-plan.md Stage 4.1 has been updated in iteration 5 to include the `IPendingQuestionStore.StoreAsync` call after `MarkSentAsync` for `SourceType = Question` messages. Implementation-plan.md Stage 3.5 has been updated to clarify that `TelegramMessageSender.SendQuestionAsync` only builds the inline keyboard and enqueues — it does not persist the `PendingQuestionRecord` directly. **However**, Stage 1.4 line 96 and Stage 2.3 line 153 still say the sender "denormalize[s] the `ActionId` into `PendingQuestionRecord.DefaultActionId`" — see Outstanding discrepancy #1 above.
