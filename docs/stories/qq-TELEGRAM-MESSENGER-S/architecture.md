...
> **⚠ OPERATOR-APPROVED SCOPE CLARIFICATION (burst-topology-envelope)**
>
...
>
> **Operator answer:** ≥ 10 operator chats **is the expected deployment topology** — 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats a natural consequence. This was explicitly confirmed by the operator (question ID `burst-topology-envelope`). Deployments with fewer than 10 operator chats must accept that the P95 SLO applies only under steady-state load, not during bursts — this is documented as a capacity planning note, not a hidden limitation. The story's "without message loss" guarantee holds unconditionally regardless of chat count.
...
All previously identified discrepancies have been resolved by aligned edits to both architecture.md and implementation-plan.md in iteration 3. No outstanding cross-document contradictions remain.
...
5. **`IDistributedCache` expiry — PARTIALLY RESOLVED (architecture.md aligned; tech-spec.md still diverges).** Architecture.md §5.2 invariant 2 and §11 now specify cache expiry at `AgentQuestion.ExpiresAt + 5 minutes` (grace window for late callbacks near expiry), and implementation-plan.md Stage 2.3 matches this. However, **tech-spec.md still diverges**: tech-spec.md R-3 (line 87) says "with entries expiring at `AgentQuestion.ExpiresAt`" (no `+ 5 minutes`), and tech-spec.md D-3 (line 116) says "expiring at `AgentQuestion.ExpiresAt`" (no `+ 5 minutes`). **Required update for tech-spec.md:** both R-3 and D-3 must be updated to say `AgentQuestion.ExpiresAt + 5 minutes` to match the canonical definition in architecture.md §5.2/§11. The `+ 5 minutes` grace window ensures `CallbackQueryHandler` can still resolve the cached `HumanAction` for late button taps near the `ExpiresAt` boundary. `QuestionTimeoutService` does not depend on the cache — it reads `DefaultActionId` directly from `PendingQuestionRecord`.
...
7. **Latency metric scope wording — OPEN (architecture.md canonical; sibling docs use ambiguous phrasing).** Architecture.md §10.4 (lines 1031, 1039, 1086) clearly specifies that `telegram.send.first_attempt_latency_ms` **includes** local token-bucket wait (proactive rate limiting before issuing the HTTP call) and **excludes** only sends that received a Telegram 429 response. However, **implementation-plan.md** Stage 4.1 (line 404) describes the metric as "first-attempt non-rate-limited sends only" and **e2e-scenarios.md** (line 269) says "without rate-limit waits." Both phrasings are ambiguous: they could be misread as excluding local token-bucket waiting from the metric, which contradicts architecture.md §10.4's explicit statement that local token-bucket wait is included. **Required update for implementation-plan.md and e2e-scenarios.md:** replace ambiguous "without rate-limit waits" / "non-rate-limited" language with the precise formulation: "first-attempt sends that did not receive a Telegram 429 response; local token-bucket wait (proactive rate limiting) is included in the measurement." Architecture.md §10.4 is the **single canonical source** for metric definitions; sibling documents should defer to it.

8. **`PendingQuestionRecord` post-send persistence — OPEN (architecture.md now assigns to `OutboundQueueProcessor`; implementation-plan.md Stage 4.1 needs update).** Architecture.md §5.2 invariant 1 (updated this iteration) explicitly assigns `PendingQuestionRecord` persistence to `OutboundQueueProcessor` as a post-send hook: after `MarkSentAsync` succeeds for `SourceType = Question` messages, the processor calls `IPendingQuestionStore.StoreAsync` using the `TelegramMessageId` from `MarkSentAsync` and the `SourceEnvelopeJson` from the `OutboundMessage`. **Implementation-plan.md Stage 4.1** (lines 403–404) defines `OutboundQueueProcessor` with dequeue, send, and mark-sent steps but does **not** include the `IPendingQuestionStore.StoreAsync` call. **Required update for implementation-plan.md:** add a step to Stage 4.1 `OutboundQueueProcessor` that, after `MarkSentAsync` for `SourceType = Question`, calls `IPendingQuestionStore.StoreAsync(envelope, chatId, telegramMessageId)` where `envelope` is deserialized from `OutboundMessage.SourceEnvelopeJson`.
