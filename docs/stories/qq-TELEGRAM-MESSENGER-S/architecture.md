...
The story requires "P95 send latency under 2 seconds after event is queued." This architecture defines the following **canonical** latency metrics. This document (architecture.md §10.4) is the **canonical source** for metric names and measurement semantics. E2e-scenarios.md is aligned: it uses `telegram.send.first_attempt_latency_ms` (enqueue to HTTP 200, first-attempt, non-rate-limited) and `telegram.send.all_attempts_latency_ms` (enqueue to HTTP 200, all-inclusive). Tech-spec.md currently has an editorial divergence — using `telegram.send.latency_ms` for the all-inclusive metric name and dequeue-to-HTTP-200 for `first_attempt_latency_ms` — which is noted here for convergence in a future iteration of tech-spec.md. The canonical definitions are:
...
- **P95 ≤ 2 s** applies to `telegram.send.first_attempt_latency_ms` — enqueue (`OutboundMessage.CreatedAt`) to HTTP 200, first-attempt, non-rate-limited sends only. Under normal operating conditions (steady state, queue depth < 100), queue dwell is negligible and this target holds across all severities. Under burst (1000+ simultaneous messages), priority queuing ensures Critical/High messages are dequeued first. **Bounded-volume assumption:** the P95 ≤ 2 s target for Critical/High under burst assumes ≤ 60 Critical+High messages in any burst window — at 30 msg/s with 10 workers, the top 60 priority messages drain in ≤ 2 s of queue dwell. If a burst contains more than 60 Critical/High messages, some may exceed the 2 s P95; in that scenario `telegram.send.queue_dwell_ms` and `telegram.send.all_attempts_latency_ms` provide visibility and the system still guarantees zero message loss. Normal/Low severity messages may exceed the 2-second P95 under sustained burst.
...
| **Burst load** (100+ agents, 1000+ messages) | P95 `telegram.send.first_attempt_latency_ms` ≤ 2 s for Critical/High severities **when ≤ 60 Critical+High messages are in the burst** (dequeued first via priority ordering, drain completes within 2 s at 30 msg/s). If a burst exceeds 60 Critical/High messages, some may breach 2 s P95; `queue_dwell_ms` provides visibility. Normal/Low first-attempt sends may exceed 2 s P95 under sustained burst due to accumulated queue dwell. Zero message loss. | Priority queuing dispatches Critical/High first. 10 concurrent workers at 30 msg/s sustained drain the top 60 priority messages within ~2 s of queue dwell. |
...
Under **extreme burst** (1000+ simultaneous messages), the priority queuing design ensures Critical/High messages are dequeued first. With the bounded-volume assumption of ≤ 60 Critical+High messages per burst, these priority messages drain within ~2 s of queue dwell at 30 msg/s, keeping their enqueue-to-200 latency within the 2-second P95 window. If a burst exceeds this volume of Critical/High messages, `telegram.send.queue_dwell_ms` surfaces the breach for capacity-planning response. Normal/Low messages are also delivered without loss but may experience longer queue dwell under sustained burst. The all-inclusive `telegram.send.all_attempts_latency_ms` and the diagnostic `telegram.send.queue_dwell_ms` provide visibility into the full delivery experience.
...
4. **Throughput**: At 30 msg/s sustained, a burst of 1 000 messages drains in ~34 seconds. With priority queuing, Critical/High messages are processed first and have minimal queue dwell (< 2 s for the top ~60 messages per the bounded-volume assumption). The acceptance metric `telegram.send.first_attempt_latency_ms` (enqueue to HTTP 200, first-attempt, non-rate-limited sends per §10.4) targets P95 ≤ 2 s under normal load; under burst, priority queuing ensures Critical/High messages meet this target when ≤ 60 are queued. If more than 60 Critical/High messages burst simultaneously, some may exceed the 2 s P95; `queue_dwell_ms` provides operational visibility. Sends that hit rate limits are tracked via `telegram.send.all_attempts_latency_ms` (all-inclusive, enqueue to 200) for capacity planning.
