...
>
> **Cross-doc status — UNRESOLVED divergence on DefaultAction placement.** This architecture defines the proposed default action as sidecar metadata (`ProposedDefaultActionId` on `AgentQuestionEnvelope`), keeping the shared `AgentQuestion` model free of it. Implementation-plan.md Stage 1.2 follows the same sidecar approach. However, the sibling documents are **internally contradictory** on this point. Within e2e-scenarios.md itself: lines 57–63 state "All sibling documents are aligned on the sidecar envelope model" and use `AgentQuestionEnvelope.ProposedDefaultActionId` throughout the scenario fixtures, yet the footer note at line 641 states "The sibling documents diverge on where the proposed default action lives and this divergence is **unresolved**" and notes that "e2e-scenarios.md models `DefaultAction` as a first-class nullable property on `AgentQuestion`; architecture.md §3.1 carries it as `ProposedDefaultActionId` sidecar metadata." Tech-spec.md S-3 and HC-3 also characterize the divergence as **unresolved** and do not declare either approach authoritative. The **behavioral contract** is identical under either approach: the Telegram connector obtains the proposed default action ID (from whichever source is adopted), denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays the default in the message body, and applies it on timeout. Resolution requires an explicit decision — either all documents adopt the sidecar envelope model (this architecture's position) or the shared model gains a first-class `DefaultAction` property. Until resolved, implementations should code to the behavioral contract rather than a specific structural assumption. The e2e-scenarios.md footer (line 641) should be reconciled with its own scenario body (lines 57–63) as a prerequisite.
...
The 2-second P95 target applies to the primary `telegram.send.latency_ms` metric, which covers first-attempt, non-rate-limited sends only. Per operator clarification, the P95 measurement includes only first-attempt, non-rate-limited sends — not retried sends or rate-limited wait time. This scoping ensures the acceptance criterion measures the system's intrinsic processing capability rather than external factors (Telegram rate limits, transient failures). The all-inclusive `telegram.send.all_attempts_latency_ms` diagnostic metric provides visibility into the operator's actual end-to-end experience including retries and rate-limit waits. This metric naming aligns with e2e-scenarios.md footer (line 645) and tech-spec.md HC-4.

**Acceptance interpretation under burst:** The story requires "P95 send latency under 2 seconds after event is queued" and "support burst alerts from 100+ agents without message loss." The P95 target is **unqualified** — it applies to all messages, all severities. The architecture meets this through a combination of priority queuing, concurrency, and capacity envelope design:

| Condition | P95 Guarantee | Mechanism |
|---|---|---|
| **Normal load** (queue depth < 100, no rate-limiting) | **≤ 2 s for all severities** | 10 concurrent workers drain the queue faster than it fills. Verified by steady-state P95 scenario (e2e-scenarios.md lines 260–275). |
| **Burst load** (100+ agents, 1000+ messages) | **≤ 2 s for all severities within the capacity envelope** | Priority queuing dispatches Critical/High first; at 30 msg/s sustained throughput with 10 workers, 1000 messages drain in ~34 s. The P95 of `telegram.send.latency_ms` (first-attempt, non-rate-limited sends) remains under 2 s because the metric excludes rate-limited waits and retried sends by definition. Messages that hit rate limits or require retries are tracked via `telegram.send.all_attempts_latency_ms` (diagnostic). |
| **Beyond capacity envelope** (sustained inflow > 30 msg/s for extended periods) | **Zero message loss; latency degrades gracefully** | Low-severity messages are backpressure-DLQ'd when queue depth exceeds `MaxQueueDepth` (5000). All other messages are delivered. Backpressure events are alerted. |

The capacity envelope where the unqualified P95 ≤ 2 s target is met: sustained inflow ≤ 30 msg/s (Telegram global rate limit) with queue depth < `MaxQueueDepth`. Within this envelope, all first-attempt, non-rate-limited sends — regardless of severity — meet the P95 target. Beyond this envelope, the system prioritizes zero message loss over latency guarantees.

Under **normal operating conditions** (queue depth < 100, no active rate-limiting), all severities comfortably meet the 2-second P95 target — verified by the steady-state P95 scenario in e2e-scenarios.md (lines 260–275: 100 messages, no 429s, P95 < 2 s).

Under **extreme burst** (1000+ simultaneous messages), the burst test in e2e-scenarios.md (lines 277–288) asserts: (a) zero message loss, (b) P95 `telegram.send.latency_ms` ≤ 2 s for Critical/High messages dispatched first via priority queuing, (c) Normal/Low messages are delivered without loss. Because `telegram.send.latency_ms` measures only first-attempt, non-rate-limited sends, the P95 target is achievable across all severities for sends that are not externally constrained. The `telegram.send.all_attempts_latency_ms` diagnostic metric captures the full end-to-end experience for capacity planning.

> **Cross-doc status — metric naming NOW ALIGNED.** This architecture now defines `telegram.send.latency_ms` as the **first-attempt, non-rate-limited** primary metric (P95 ≤ 2 s applies here) and `telegram.send.all_attempts_latency_ms` as the all-inclusive diagnostic metric. This aligns with e2e-scenarios.md footer (line 645) and tech-spec.md HC-4, which both define `telegram.send.latency_ms` as first-attempt/non-rate-limited only. The operator has confirmed that the P95 measurement should include only first-attempt, non-rate-limited sends. Note: e2e-scenarios.md has an internal contradiction — its scenario body (lines 260–275) still describes `telegram.send.latency_ms` as "all-inclusive" while its footer (line 645) defines it as first-attempt/non-rate-limited. The footer definition is authoritative per the operator's answer; the scenario body comments should be updated to match.
...
4. **Throughput**: At 30 msg/s sustained, a burst of 1 000 messages drains in ~34 seconds. With priority queuing, the ~50 Critical/High messages in the burst are processed in the first ~2 seconds, meeting the P95 ≤ 2 s target. The primary `telegram.send.latency_ms` metric covers first-attempt, non-rate-limited sends (see §10.4 P95 Metric Definition); the all-inclusive `telegram.send.all_attempts_latency_ms` diagnostic metric captures all sends including retries and rate-limit waits for capacity planning. Under normal conditions and within the capacity envelope (sustained inflow ≤ 30 msg/s), the P95 of `telegram.send.latency_ms` meets the 2-second target across all severities. The e2e-scenarios burst test asserts zero message loss, bounded drain time, and P95 < 2 s for Critical/High messages dispatched first via priority queuing.
