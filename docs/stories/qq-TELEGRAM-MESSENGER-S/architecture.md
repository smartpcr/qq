...
public interface IPendingQuestionStore
{
    ...
    Task RecordSelectionAsync(string questionId, string selectedActionId, long respondentUserId, CancellationToken ct);
...
> **Cross-doc alignment (IMessageSender).** Implementation-plan.md Stage 1.4 defines `IMessageSender` in Core with the same two-method contract: `SendTextAsync` and `SendQuestionAsync`. **Both methods return `Task<SendResult>`** (not bare `Task`), where `SendResult` carries the `TelegramMessageId` assigned by the Bot API. Stage 2.3 provides the concrete `TelegramMessageSender` implementing `IMessageSender`. Stage 4.1 (`OutboundQueueProcessor`) sends via `IMessageSender` (not the concrete type) and uses `SendResult.TelegramMessageId` for `MarkSentAsync` and `StoreAsync` calls, so that Core/Worker assemblies do not depend on `AgentSwarm.Messaging.Telegram`. The component table in §2.2 lists `TelegramSender` as the short name for the concrete implementation — this is the same component as implementation-plan.md's `TelegramMessageSender`.
...
All previously identified discrepancies have been resolved as of iteration 10. No outstanding discrepancies remain.
...
11. **`IMessageSender` return type and rendering boundary — RESOLVED (iteration 11).** Architecture.md §4.12 now defines `IMessageSender.SendTextAsync` and `SendQuestionAsync` as returning `Task<SendResult>` (where `SendResult` carries `TelegramMessageId`), enabling `OutboundQueueProcessor` to pass the Telegram message ID to `MarkSentAsync` and `StoreAsync`. The rendering boundary is now explicit: `TelegramMessageSender` is the sole owner of Telegram-specific rendering (question messages rendered at send time from `SourceEnvelopeJson`); non-question messages use pre-rendered `Payload` from enqueue time. Implementation-plan.md Stage 1.4 should adopt `Task<SendResult>` return types to match.
