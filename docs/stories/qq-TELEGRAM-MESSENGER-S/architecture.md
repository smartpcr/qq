...
> **Cross-doc alignment (MessengerMessage):** Implementation-plan.md Stage 1.2 defines `MessengerMessage` with these exact fields (`MessageId`, `CorrelationId`, `AgentId`, `TaskId`, `ConversationId`, `Timestamp`, `Text`, `Severity`, `Metadata`). `AgentId` is nullable (`string?`) because some outbound messages (e.g., `/ask` acknowledgements, `/start` welcome) exist before an agent is assigned; tech-spec HC-9 requires `AgentId` when an agent context exists. The `OutboundMessage` entity (below) stores the rendered Telegram payload in `Payload` and the original source envelope (when applicable) in `SourceEnvelopeJson`, adding queue-specific fields (`IdempotencyKey`, `Status`, `AttemptCount`, etc.) for durable delivery.
