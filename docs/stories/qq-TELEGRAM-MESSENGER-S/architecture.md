...
| `Value` | `string` | Machine-readable action value carried in `HumanDecisionEvent.ActionValue` when this action is selected. Typically matches `ActionId` but may differ when the consuming agent expects a different semantic value (e.g., `ActionId = "approve-btn"`, `Value = "approve"`). |
...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Stored here so that `QuestionTimeoutService` can apply the default action directly from this field without depending on `IDistributedCache` (whose entries expire at `AgentQuestion.ExpiresAt` per tech-spec D-3/R-3 and may be evicted when the timeout fires). When present, the timeout service publishes a `HumanDecisionEvent` with this value as the action. When `null`, the question expires with `ActionValue = "__timeout__"`. |
...
> **Cross-doc note (ISwarmCommandBus query signatures):** Implementation-plan.md Stage 1.3 currently declares `QueryStatusAsync(CancellationToken)` and `QueryAgentsAsync(CancellationToken)` without query parameter objects. This architecture document introduces `SwarmStatusQuery` and `SwarmAgentsQuery` to carry the workspace scope and optional filter/task-ID required by §5.7. **This is a known divergence, not a contradiction:** the architecture document is the canonical contract for `ISwarmCommandBus` method signatures; the implementation plan should be updated to adopt `QueryStatusAsync(SwarmStatusQuery, CancellationToken)` and `QueryAgentsAsync(SwarmAgentsQuery, CancellationToken)` before Stage 1.3 implementation begins. Until then, implementers must follow this architecture document's signatures.
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats so per-chat burst capacity is not exhausted — pending operator confirmation via open question `burst-topology-envelope`; see §10.4 Burst Scenario item 3) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
