...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Stored here so that `QuestionTimeoutService` can identify the default action without depending on `IDistributedCache` (whose entries expire at `AgentQuestion.ExpiresAt` per tech-spec D-3/R-3 and may be evicted when the timeout fires). When present, the timeout service publishes a `HumanDecisionEvent` with `DefaultActionId` directly as the `ActionValue` (consistent with e2e-scenarios.md line 114 — no JSON deserialization or cache lookup required). When `null`, the question expires with `ActionValue = "__timeout__"`. |
...
1. **`OutboundMessage.SourceEnvelopeJson` field missing from implementation-plan.md Stage 1.2.** Architecture.md §3.1 defines `OutboundMessage.SourceEnvelopeJson` (`string?`) as a required field — it stores the serialized original source envelope (e.g., `AgentQuestionEnvelope` JSON for question messages) and is essential for the Gap B recovery design (§3.1 `PendingQuestionRecord` crash-window analysis). The `QuestionRecoverySweep` component (§2.2) depends on this field to backfill missing `PendingQuestionRecord` rows after a crash between `MarkSentAsync` and `PendingQuestionRecord` persistence. Implementation-plan.md Stage 1.2 (line 48) defines the `OutboundMessage` record without `SourceEnvelopeJson` — this field must be added to the Stage 1.2 `OutboundMessage` definition.

2. **`ISwarmCommandBus` query signatures differ.** Architecture.md §4.6 defines `QueryStatusAsync(SwarmStatusQuery query, CancellationToken ct)` and `QueryAgentsAsync(SwarmAgentsQuery query, CancellationToken ct)` with query-parameter objects carrying `WorkspaceId` and optional filter/task-ID (required by §5.7 command handlers). Implementation-plan.md Stage 1.3 (line 76) defines parameterless signatures `QueryStatusAsync(CancellationToken)` and `QueryAgentsAsync(CancellationToken)`. The implementation-plan should adopt the parameterized signatures and define `SwarmStatusQuery` and `SwarmAgentsQuery` records alongside `SwarmCommand` in Abstractions.

3. **`QuestionRecoverySweep` has no implementation stage.** Architecture.md §2.2 defines `QuestionRecoverySweep` as a `BackgroundService` that detects `OutboundMessage` records with `SourceType = Question` and `Status = Sent` lacking a corresponding `PendingQuestionRecord`, and backfills them using `SourceEnvelopeJson`. This component is required by the Gap B crash-window mitigation (§3.1). Implementation-plan.md has no stage or test covering `QuestionRecoverySweep` — a new stage (e.g., Stage 3.6 or 4.4) should be added to implement and test this recovery sweep.

4. **Observability metric names differ.** Architecture.md §8 defines the canonical metric set: counters `telegram.messages.received`, `telegram.messages.sent`, `telegram.messages.dead_lettered`, `telegram.commands.processed`, `telegram.messages.backpressure_dlq`; histograms `telegram.send.first_attempt_latency_ms`, `telegram.send.all_attempts_latency_ms`, `telegram.send.queue_dwell_ms`, `telegram.send.retry_latency_ms`, `telegram.send.rate_limited_wait_ms`. Implementation-plan.md Stage 6.1 (line 500) configures a different metric set: `telegram.errors`, `telegram.queue.depth`, `telegram.dlq.depth`. The Stage 6.1 implementation steps should adopt the architecture.md canonical metric names. Specifically: replace `telegram.errors` with the counter/histogram set above (errors are captured via the retry and dead-letter counters), replace `telegram.queue.depth` with `telegram.send.queue_dwell_ms` (histogram) for queue monitoring, and replace `telegram.dlq.depth` with `telegram.messages.dead_lettered` (counter). The health check at Stage 6.2 may still inspect queue depth as a threshold, but the emitted OTel metric names should match architecture.md §8.
