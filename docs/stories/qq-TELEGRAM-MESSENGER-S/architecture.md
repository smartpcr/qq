...
| **TelegramSender** | `AgentSwarm.Messaging.Telegram` (to be created) | Concrete `TelegramMessageSender` implementing `IMessageSender` (§4.12, defined in `AgentSwarm.Messaging.Core`). All three methods (`SendTextAsync`, `SendMessageAsync`, `SendQuestionAsync`) return `Task<SendResult>` carrying the Telegram-assigned `message_id`. Wraps `ITelegramBotClient` from the `Telegram.Bot` library. **Sole owner of question rendering**: builds `InlineKeyboardMarkup`, formats question MarkdownV2 text, writes `HumanAction` entries to `IDistributedCache`, and displays the proposed default — all at send time from `SourceEnvelopeJson`, not from pre-rendered `Payload`. For non-question messages (`Alert`, `StatusUpdate`, `CommandAck`), the sender accepts the pre-rendered `Payload` text wrapped in a `MessengerMessage` (so the `CorrelationId` propagates through the trace footer and the durable `IMessageIdTracker` mapping). The sender also enforces Telegram rate limits and applies MarkdownV2 escaping for callers that pass raw human-readable text. `OutboundQueueProcessor` invokes the sender through the `IMessageSender` abstraction, keeping `AgentSwarm.Messaging.Worker` independent of the Telegram assembly. |
...
| `Payload` | `string` | Outbox payload whose semantics depend on `SourceType`. **For `CommandAck`, `StatusUpdate`, and `Alert`**: pre-rendered Telegram message content (MarkdownV2 text) ready for `IMessageSender.SendMessageAsync` (which wraps the payload + `CorrelationId` in a `MessengerMessage` so the trace id propagates through the chunked footer and the `IMessageIdTracker` mapping) — rendering is performed by `TelegramMessengerConnector` at enqueue time. **For `Question`**: a human-readable preview of the question (title, severity, body summary) stored for debugging, dead-letter inspection, and replay diagnostics — **not** the actual send content. For questions, the actual Telegram send is rendered at send time by `TelegramMessageSender.SendQuestionAsync`, which reads from `SourceEnvelopeJson` (below) to build the inline keyboard, write `IDistributedCache` entries, and format the full MarkdownV2 message. This split ensures that question rendering — which involves side effects (cache writes) and Telegram-specific layout (inline keyboards) — happens exactly once at send time, not at enqueue time. |
...
{
    // Legacy correlation-free overload: single-shot acks (e.g. /start)
    // where there is no upstream CorrelationId to propagate.
    Task<SendResult> SendTextAsync(long chatId, string text, CancellationToken ct);

    // Production path used by Stage 4.1 OutboundQueueProcessor for all
    // non-question messages (Alert, StatusUpdate, CommandAck). Carries
    // the CorrelationId through the rendered trace footer (one footer
    // per chunk for split messages) and the IMessageIdTracker mapping
...
The concrete `TelegramMessageSender` (in `AgentSwarm.Messaging.Telegram`) implements this interface and wraps `ITelegramBotClient` from the `Telegram.Bot` library. **Rendering boundary:** `TelegramMessageSender` is the **sole owner of question rendering** — it builds `InlineKeyboardMarkup`, formats question MarkdownV2 text, writes `HumanAction` entries to `IDistributedCache`, and displays the proposed default, all at send time from the deserialized `AgentQuestionEnvelope`. For non-question messages (`Alert`, `StatusUpdate`, `CommandAck`), rendering is performed earlier by `TelegramMessengerConnector` at enqueue time, and `TelegramMessageSender.SendMessageAsync` accepts the pre-rendered `Payload` wrapped in a `MessengerMessage` (so the `CorrelationId` flows into the trace footer and the durable `IMessageIdTracker` mapping). The sender also performs MarkdownV2 escaping on the `MessengerMessage.Text` so callers can supply raw human-readable text without manually escaping every metacharacter; the question-rendering path escapes its dynamic fields inline. `TelegramMessageSender` also enforces Telegram rate limits via the dual-layer token-bucket limiter (§10.4). In component diagrams and sequence flows throughout this document, **`TelegramSender`** refers to this concrete `TelegramMessageSender` implementation.
...
6. If no operator responds before `ExpiresAt`, a timeout handler reads `PendingQuestionRecord.DefaultActionValue` (the `HumanAction.Value` of the default action, resolved and denormalized at send time — see §3.1 `PendingQuestionRecord.DefaultActionValue`). **Two branches:** (a) when `DefaultActionValue` is non-null, the handler fires a `HumanDecisionEvent` with `DefaultActionValue` as the `ActionValue` — consistent with the interactive button-tap path where `HumanAction.Value` is always the canonical `ActionValue` — and updates the Telegram message to "⏰ Timed out — default action applied: {action}"; (b) when `DefaultActionValue` is null, the handler fires a `HumanDecisionEvent` with `ActionValue = "__timeout__"` (no automatic action is taken on behalf of the operator), and updates the Telegram message to "⏰ Timed out — no default action". In both branches, `PendingQuestionRecord.Status` transitions to `TimedOut` and an audit record is written (see §10.3 for the full timeout flow).
...
1. Reads `PendingQuestionRecord.DefaultActionValue` (the `HumanAction.Value` of the default action, resolved and denormalized at send time from the `HumanAction` in `AllowedActions` whose `ActionId` matches `ProposedDefaultActionId` — see §3.1 `PendingQuestionRecord.DefaultActionValue`). If present, the timeout service publishes a `HumanDecisionEvent` with `DefaultActionValue` as the `ActionValue` — consistent with the interactive button-tap path where `HumanAction.Value` is always the canonical `ActionValue` (see §3.1 `HumanAction.Value`, §5.2 invariant 6). No `IDistributedCache` lookup or `AgentQuestion` JSON deserialization is required. If `DefaultActionValue` is absent (`null`), publishes a `HumanDecisionEvent` with `ActionValue = "__timeout__"` so the agent is notified of timeout without an automatic decision.
...
11. **`IMessageSender` return type — RESOLVED (iteration 13).** Implementation-plan.md Stage 1.4 and Stage 2.3 now specify `Task<SendResult>` return types for the three sender methods (`SendTextAsync` legacy correlation-free overload, `SendMessageAsync` production correlation-carrying path, and `SendQuestionAsync` rendering path), with `SendResult` carrying `TelegramMessageId` (long) — matching the canonical contract in architecture.md §4.12. The rendering boundary is explicit: `TelegramMessageSender` is the sole owner of question rendering; non-question rendering (`Alert`, `StatusUpdate`, `CommandAck`) is performed by `TelegramMessengerConnector` at enqueue time and dispatched at send time via `SendMessageAsync` so the `MessengerMessage.CorrelationId` propagates into the rendered chunked footer and the durable `IMessageIdTracker` mapping.
