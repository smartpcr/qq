...
> **Cross-doc alignment (IPendingQuestionStore).** This document (§4.7) is the **canonical source** for the `IPendingQuestionStore` interface contract. The canonical interface includes eight methods: `StoreAsync`, `GetAsync`, `GetByTelegramMessageAsync`, `MarkAnsweredAsync`, `MarkAwaitingCommentAsync`, `RecordSelectionAsync`, `GetAwaitingCommentAsync`, and `GetExpiredAsync`. **Outstanding:** `RecordSelectionAsync` now accepts four parameters (`questionId`, `selectedActionId`, `selectedActionValue`, `respondentUserId`) to persist the resolved `HumanAction.Value` at button-tap time; implementation-plan.md Stage 1.3 should adopt this updated signature. `StoreAsync` now denormalizes both `DefaultActionId` and `DefaultActionValue`; implementation-plan.md Stages 1.3 and 3.5 should reflect this. `CallbackQueryHandler` and the text-reply handler operate through `IPendingQuestionStore` without a compile-time dependency on the Persistence assembly. The canonical method name `GetByTelegramMessageAsync(long telegramChatId, long telegramMessageId, CancellationToken)` accepts **two** identifiers because Telegram `message_id` is only unique within a chat. Implementation-plan.md Stage 1.3 uses this exact composite-key signature and method name (`GetByTelegramMessageAsync`, not `GetByTelegramMessageIdAsync`).
...
> **Cross-doc alignment (IMessageSender).** This document (§4.12) is the **canonical source** for the `IMessageSender` interface contract. Both methods return `Task<SendResult>` (not bare `Task`), where `SendResult` carries the `TelegramMessageId` assigned by the Bot API. **Outstanding:** implementation-plan.md Stage 1.4 defines `IMessageSender` with the same two methods (`SendTextAsync`, `SendQuestionAsync`) but does not yet specify `Task<SendResult>` return types — it should adopt them to match this canonical contract. Implementation-plan.md Stage 4.1 references the concrete `TelegramMessageSender` instead of the `IMessageSender` abstraction — it should use `IMessageSender` so that Core/Worker assemblies do not depend on `AgentSwarm.Messaging.Telegram`. See Appendix A items 11 and 14 for tracking. The component table in §2.2 lists `TelegramSender` as the short name for the concrete implementation — this is the same component as implementation-plan.md's `TelegramMessageSender`.
...
>
> **Decision (operator-directed, persisted as D-BURST in §11.1):** ≥ 10 operator chats **is the expected deployment topology** — 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats a natural consequence. Deployments with fewer than 10 operator chats must accept that the P95 SLO applies only under steady-state load, not during bursts — this is documented as a capacity planning note, not a hidden limitation. The story's "without message loss" guarantee holds unconditionally regardless of chat count.
>
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats — formal decision D-BURST per §11.1; see §10.4 Burst Scenario item 3) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
...
- **Decision:** The P95 ≤ 2 s burst SLO assumes ≤ 50 Critical+High messages **distributed across ≥ 10 operator chats**. This is the expected deployment topology: 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats a natural consequence. Deployments with fewer than 10 operator chats must accept that the P95 SLO applies only under steady-state load, not during bursts.
- **Rationale:** The per-chat rate limit (20 msg/min) becomes the binding constraint when messages concentrate in fewer chats. With ≥ 10 chats, the global rate limit (30 msg/s) is the binding constraint and the burst math (§10.4) holds.
...
The following discrepancies remain outstanding and require updates in the sibling implementation-plan.md. Items are tracked here so the sibling document author can resolve them in their next iteration.

#### Outstanding items

11. **`IMessageSender` return type — OUTSTANDING (since iteration 11).** Architecture.md §4.12 defines `IMessageSender.SendTextAsync` and `SendQuestionAsync` as returning `Task<SendResult>` (where `SendResult` carries `TelegramMessageId`). Implementation-plan.md Stage 1.4 (line 96) and Stage 2.3 (line 152) list `SendTextAsync(...)` / `SendQuestionAsync(...)` with no `Task<SendResult>` return type specified. `grep -nF "Task<SendResult>" docs/stories/qq-TELEGRAM-MESSENGER-S/implementation-plan.md` returns no hits. The sibling document should adopt `Task<SendResult>` return types to match the canonical contract in §4.12.

12. **`RecordSelectionAsync` signature update — OUTSTANDING (since iteration 12).** Architecture.md §4.7 now defines `RecordSelectionAsync(string questionId, string selectedActionId, string selectedActionValue, long respondentUserId, CancellationToken)` with the additional `selectedActionValue` parameter. Implementation-plan.md Stage 1.3 should adopt this updated four-parameter signature.

13. **`StoreAsync` denormalization scope — OUTSTANDING (since iteration 12).** Architecture.md §3.1 and §5.2 now specify that `IPendingQuestionStore.StoreAsync` denormalizes both `DefaultActionId` and `DefaultActionValue` (resolving `HumanAction.Value` from `AllowedActions`). Implementation-plan.md Stages 1.3 and 3.5 should reflect this dual denormalization.

14. **`OutboundQueueProcessor` dependency boundary — OUTSTANDING (since iteration 12).** Architecture.md §2.2, §4.12, and §5.2 specify that `OutboundQueueProcessor` sends via `IMessageSender` (the abstraction defined in Core), not the concrete `TelegramMessageSender`. Implementation-plan.md Stage 4.1 (line 403) says "sends via `TelegramMessageSender`" — it should say "sends via `IMessageSender`" so that the Worker assembly does not depend on `AgentSwarm.Messaging.Telegram`.

15. **Timeout `ActionValue` semantics — OUTSTANDING (since iteration 12).** Architecture.md now specifies that the timeout path publishes `DefaultActionValue` (the resolved `HumanAction.Value`) as `ActionValue`, not `DefaultActionId` directly. E2e-scenarios.md line 114 currently expects `ActionValue` to be the `ActionId` (`act-1`); it should expect the `HumanAction.Value` (e.g., `approve`) to be consistent with the data model. Implementation-plan.md timeout-related descriptions should also reflect `DefaultActionValue`.

#### Resolved items (iterations 3–10)
...
11. **`IMessageSender` return type and rendering boundary — PARTIALLY RESOLVED (iteration 11), OUTSTANDING remainder tracked above.** Architecture.md §4.12 now defines `IMessageSender.SendTextAsync` and `SendQuestionAsync` as returning `Task<SendResult>` (where `SendResult` carries `TelegramMessageId`), enabling `OutboundQueueProcessor` to pass the Telegram message ID to `MarkSentAsync` and `StoreAsync`. The rendering boundary is now explicit: `TelegramMessageSender` is the sole owner of Telegram-specific rendering (question messages rendered at send time from `SourceEnvelopeJson`); non-question messages use pre-rendered `Payload` from enqueue time. **Remaining:** Implementation-plan.md Stage 1.4 does not yet specify `Task<SendResult>` return types; Stage 4.1 references the concrete `TelegramMessageSender` instead of `IMessageSender`. See outstanding items 11 and 14 above.

12. **`OutboundQueueProcessor` in component table and assembly map — RESOLVED (iteration 11).** Architecture.md §2.2 component table now includes `OutboundQueueProcessor` with assembly (`AgentSwarm.Messaging.Worker`), responsibilities, and dependencies. §6 assembly map now lists it under Worker.

13. **P95 formal decision records — RESOLVED (iteration 11).** §11.1 now contains formal decision records D-P95 and D-BURST persisting the operator-directed answers on metric scope and burst topology. All §10.4 callouts now reference §11.1 instead of stating "not persisted as a formal decision record."
