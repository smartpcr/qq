...
> **Cross-doc alignment (ISwarmCommandBus query signatures):** The canonical signatures for query methods are `QueryStatusAsync(SwarmStatusQuery query, CancellationToken ct)` and `QueryAgentsAsync(SwarmAgentsQuery query, CancellationToken ct)`, carrying the workspace scope and optional filter/task-ID required by §5.7. Implementation-plan.md Stage 1.3 currently shows parameterless overloads (`QueryStatusAsync(CancellationToken)`, `QueryAgentsAsync(CancellationToken)`); both documents agree on the intent — the parameterized form here is the refined version that the implementation will adopt when Stage 1.3 is implemented. The stub `StubSwarmCommandBus` (implementation-plan.md Stage 6.3) will accept the query-parameter signatures and return empty collections scoped to the query objects' workspace.
