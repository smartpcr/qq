...
> **Cross-doc alignment (MessengerMessage):** Implementation-plan.md Stage 1.2 defines `MessengerMessage` with these exact fields (`MessageId`, `CorrelationId`, `AgentId`, `TaskId`, `ConversationId`, `Timestamp`, `Text`, `Severity`, `Metadata`). Tech-spec HC-9 requires all messages to carry `CorrelationId`, `AgentId`, `TaskId`, `ConversationId`, and `Timestamp`. The `OutboundMessage` entity (below) stores the rendered Telegram payload in `Payload` and the original source envelope (when applicable) in `SourceEnvelopeJson`, adding queue-specific fields (`IdempotencyKey`, `Status`, `AttemptCount`, etc.) for durable delivery.
...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Stored here so that `QuestionTimeoutService` can apply the default action directly from this field without depending on `IDistributedCache` (whose entries expire at `AgentQuestion.ExpiresAt + 5 minutes` per implementation-plan Stage 2.3 and may be evicted when the timeout fires). When present, the timeout service publishes a `HumanDecisionEvent` with this value as the action. When `null`, the question expires with `ActionValue = "__timeout__"`. |
...
| `RespondentUserId` | `long?` | Telegram user ID of the operator who tapped the inline button. Set at the same time as `SelectedActionId`. Used together with `TelegramChatId` and `Status = AwaitingComment` to correlate the follow-up text reply to the correct pending question — the text-reply handler matches on `(TelegramChatId, RespondentUserId, Status = AwaitingComment)`, which is unambiguous even when multiple questions are awaiting comments in the same chat from different operators. When only one question is `AwaitingComment` for a given `(chatId, userId)` pair, the match is exact; if the same operator has multiple `AwaitingComment` questions in the same chat (an unlikely but possible race), the handler matches the **oldest** by `StoredAt` and warns the operator about the remaining pending questions. |
...
- Index on `(TelegramChatId, RespondentUserId, Status)` — used by the text-reply handler to match a follow-up comment to the correct `AwaitingComment` question. The triple `(TelegramChatId, RespondentUserId, Status = AwaitingComment)` uniquely identifies the question awaiting a comment from a specific operator in a specific chat.
...
public interface IPendingQuestionStore
{
    ...
    Task MarkAwaitingCommentAsync(string questionId, string selectedActionId, long respondentUserId, CancellationToken ct);
...
6. If no operator responds before `ExpiresAt`, a timeout handler reads `PendingQuestionRecord.DefaultActionId` (denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at send time). **Two branches:** (a) when `DefaultActionId` is non-null, the handler fires a `HumanDecisionEvent` with `DefaultActionId` as the action value (no `IDistributedCache` lookup — the cache entry expires at `AgentQuestion.ExpiresAt + 5 minutes` per implementation-plan Stage 2.3, but `QuestionTimeoutService` does not depend on it; the `DefaultActionId` string is sufficient for the consuming agent to resolve semantics from its `AllowedActions` list), and updates the Telegram message to "⏰ Timed out — default action applied: {action}"; (b) when `DefaultActionId` is null, the handler fires a `HumanDecisionEvent` with `ActionValue = "__timeout__"` (no automatic action is taken on behalf of the operator), and updates the Telegram message to "⏰ Timed out — no default action". In both branches, `PendingQuestionRecord.Status` transitions to `TimedOut` and an audit record is written (see §10.3 for the full timeout flow).
...
1. Reads `PendingQuestionRecord.DefaultActionId` (denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at send time). If present, the timeout service publishes a `HumanDecisionEvent` with the `DefaultActionId` as the action value — it does **not** look up the full `HumanAction` from `IDistributedCache`, because the cache entry expires at `AgentQuestion.ExpiresAt + 5 minutes` (per implementation-plan Stage 2.3) and may already be evicted when the timeout fires (the timeout service polls at `ExpiresAt`, which is before the cache grace window ends, but eviction is not guaranteed). The `DefaultActionId` string is sufficient to construct the `HumanDecisionEvent`; the consuming agent resolves the full action semantics from its own `AllowedActions` list using this ID. If `DefaultActionId` is absent (`null`), publishes a `HumanDecisionEvent` with `ActionValue = "__timeout__"` so the agent is notified of timeout without an automatic decision.
...
>
> **Operator answer:** The P95 measurement covers **first-attempt, non-rate-limited sends only**. Retried sends and Telegram 429-rate-limited sends are excluded from the acceptance gate and tracked separately. "Non-rate-limited" refers to sends that did not receive a Telegram 429 response; local token-bucket wait (proactive rate limiting before issuing the HTTP call) **is included** in the metric as it is part of the normal send path. This was explicitly confirmed by the operator (question ID `p95-metric-scope`).
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats so per-chat burst capacity is not exhausted) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
...
3. **Multi-chat fan-out and per-chat constraint**: In production, 100+ agents typically span multiple tenants/workspaces, routing to multiple operator chats. The per-chat rate limit (20 msg/min ≈ 0.33 msg/s) constrains individual operators, while the global rate limit (30 msg/s) applies across all chats. With messages distributed across N operator chats, effective throughput is `min(30, N × 20/60)` msg/s. **The burst math assumes multi-chat distribution across ≥ 10 operator chats.** For 10+ operator chats, the global limit of 30 msg/s is the binding constraint and the ≤ 50 priority messages drain in ~1,900 ms (P95). The per-chat limiter (20 msg/min) is sized for sustained flow, not burst — during a burst, each per-chat token bucket has its own burst capacity (default 5 tokens, configurable via `Telegram:RateLimits:PerChatBurstCapacity`) that allows the first 5 messages per chat to drain without per-chat wait. With ≤ 50 messages spread across ≥ 10 operator chats (~5 messages per chat), the per-chat burst capacity absorbs the burst and the global 30 msg/s limit is the binding constraint — matching the burst math. **For fewer operator chats, the per-chat limit becomes the binding constraint and the P95 ≤ 2 s SLO cannot be met during bursts:** a single operator chat can sustain only ~0.33 msg/s, so 50 messages to a single chat would take ~150 s. Even 5 chats (each receiving 10 messages, 5 absorbed by burst capacity, 5 rate-limited) would exceed 2 s. Deployments with fewer than 10 operator chats must accept that the P95 SLO applies only under steady-state load, not during bursts, and individual chat delivery times will exceed 2 s during burst conditions. The ≥ 10 chat requirement is a natural consequence of 100+ agents spanning multiple workspaces with distinct operator assignments — it is a deployment topology constraint, not an implementation limitation.
