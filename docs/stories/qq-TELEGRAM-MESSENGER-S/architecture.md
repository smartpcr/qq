...
> **Cross-doc alignment note (IOperatorRegistry):** This document (§4.3) defines the **canonical** `IOperatorRegistry` interface contract: `GetByAliasAsync(string operatorAlias, string tenantId, CancellationToken)` with tenant-scoped resolution, `RegisterAsync(OperatorRegistration, CancellationToken)` accepting the `OperatorRegistration` value object, and a `UNIQUE (OperatorAlias, TenantId)` index on `OperatorBinding`. Implementation-plan.md Stage 1.3 (line 80) correctly defines the interface with these exact signatures and the tenant-scoped unique constraint. However, implementation-plan.md Stage 3.4 has three divergences that should be aligned to this canonical definition: (1) Stage 3.4 line 333 specifies `unique index on OperatorAlias` without `TenantId` — should be `UNIQUE (OperatorAlias, TenantId)` to prevent cross-tenant alias collisions; (2) Stage 3.4 line 335 describes `GetByAliasAsync` as querying by `OperatorAlias` only — the concrete `PersistentOperatorRegistry.GetByAliasAsync` should also filter by `TenantId`; (3) Stage 3.4 line 346 test scenario calls `GetByAliasAsync("@operator-1")` with one argument — should be `GetByAliasAsync("@operator-1", "t-1")` with both alias and tenantId.
