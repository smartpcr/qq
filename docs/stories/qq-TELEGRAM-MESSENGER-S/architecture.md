...
| **OutboundQueueProcessor** | `AgentSwarm.Messaging.Worker` (to be created) | `BackgroundService` with configurable concurrency (`OutboundQueue:ProcessorConcurrency`, default 10 workers). Each worker independently dequeues the highest-severity pending `OutboundMessage`, dispatches to `IMessageSender` based on `SourceType` (question → `SendQuestionAsync`; all others → `SendTextAsync` with pre-rendered `Payload`), calls `IOutboundQueue.MarkSentAsync` with the `SendResult.TelegramMessageId`. For question messages, `IPendingQuestionStore.StoreAsync` is invoked **directly by `TelegramMessageSender.SendQuestionAsync`** after a successful Telegram send (per Stage 3.5 workstream brief — see §5.2 invariant 1); `OutboundQueueProcessor`'s residual responsibility for pending-question persistence is solely the `PendingQuestionPersistenceException` recovery path (retry the `StoreAsync` upsert without re-sending the already-delivered Telegram message). On failure, increments `AttemptCount` and schedules retry or dead-letters. Depends on `IOutboundQueue`, `IMessageSender`, `IPendingQuestionStore`. See §10.4 for concurrency and burst analysis. |
...
| **QuestionRecoverySweep** | `AgentSwarm.Messaging.Telegram` (to be created) | `BackgroundService` that runs on startup and periodically (configurable interval, default 60 s). Queries for `OutboundMessage` records with `SourceType = Question` and `Status = Sent` that lack a corresponding `PendingQuestionRecord`, then backfills the missing records using the stored `TelegramMessageId`, `SourceId` (as `QuestionId`), and `SourceEnvelopeJson` (deserialized to extract `AgentQuestion` fields, `ProposedDefaultActionId`, and the resolved `DefaultActionValue`). This component owns the Gap B mitigation — see §3.1 `PendingQuestionRecord` for the crash-window analysis. |
...
| `Payload` | `string` | Outbox payload whose semantics depend on `SourceType`. **For `CommandAck`, `StatusUpdate`, and `Alert`**: pre-rendered Telegram message content (MarkdownV2 text) ready for `IMessageSender.SendTextAsync` — rendering is performed by `TelegramMessengerConnector` at enqueue time. **For `Question`**: a human-readable preview of the question (title, severity, body summary) stored for debugging, dead-letter inspection, and replay diagnostics — **not** the actual send content. For questions, the actual Telegram send is rendered at send time by `TelegramMessageSender.SendQuestionAsync`, which reads from `SourceEnvelopeJson` (below) to build the inline keyboard, write `IDistributedCache` entries, and format the full MarkdownV2 message. This split ensures that question rendering — which involves side effects (cache writes) and Telegram-specific layout (inline keyboards) — happens exactly once at send time, not at enqueue time. |
...
| `MaxAttempts` | `int` | Maximum number of delivery attempts before dead-lettering. Default value is sourced from `OutboundQueue:MaxRetries` configuration (default 5, consistent with §5.3 and implementation-plan.md Stage 4.1 `RetryPolicy.MaxAttempts`). Stored per-message so that the retry processor can evaluate `AttemptCount >= MaxAttempts` without a configuration lookup, and so that individual messages can be assigned a non-default retry budget at enqueue time if needed (e.g., time-sensitive alerts with fewer retries). |
...
| Field | Type | Description |
|---|---|---|
| `ActionId` | `string` | Unique identifier for this action within the question's `AllowedActions` list. Constrained to **printable ASCII only**, max **30 characters**, with **no `':'`** (the `QuestionId:ActionId` separator) and **no ASCII control characters**. ASCII enforcement guarantees the UTF-8 byte count equals the character count, so a 30-character `ActionId` is provably ≤ 30 bytes on the wire — keeping the joined `QuestionId:ActionId` payload ≤ 61 bytes within Telegram's 64-byte `callback_data` limit. Used as the value in `callback_data`, in `PendingQuestionRecord.SelectedActionId`, and in `PendingQuestionRecord.DefaultActionId` (denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId`). For interactive resolution paths (button tap / text reply), `HumanDecisionEvent.ActionValue` carries `HumanAction.Value` (resolved via cache or the `DefaultActionValue`/`SelectedActionValue` denormalized columns on the record). For the **timeout** path, `ActionValue` carries the `DefaultActionId` string verbatim (the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list, per §10.3). See `Value` below. |
...
> **Default action flow.** When the Telegram connector renders an `AgentQuestionEnvelope` as an inline-keyboard message, the sender (`TelegramMessageSender.SendQuestionAsync`) reads `ProposedDefaultActionId` from the envelope and, when present, displays the proposed default in the message body (e.g., "Default action if no response: Approve"). After a successful Telegram send, the sender calls `IPendingQuestionStore.StoreAsync` directly (per Stage 3.5 workstream brief step 5), and the store denormalises `ProposedDefaultActionId` into both `PendingQuestionRecord.DefaultActionId` (for the timeout-service emission AND for display/audit) and `PendingQuestionRecord.DefaultActionValue` (for the callback / RequiresComment text-reply path's cache-miss fallback). The store resolves `DefaultActionValue` by finding the `HumanAction` in `AgentQuestion.AllowedActions` whose `ActionId` matches `ProposedDefaultActionId` and reading its `Value`. At timeout, `QuestionTimeoutService` reads `PendingQuestionRecord.DefaultActionId` and publishes a `HumanDecisionEvent` with that string **verbatim** as the `ActionValue` — the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list (§10.3). No `IDistributedCache` lookup or `AgentQuestion` JSON deserialization is needed at timeout. When `ProposedDefaultActionId` is `null`, both `DefaultActionId` and `DefaultActionValue` are `null`, the question expires with `ActionValue = "__timeout__"`, and no automatic decision is applied. (A persistence failure after a successful Telegram send is propagated as `PendingQuestionPersistenceException` so the Stage 4.1 `OutboundQueueProcessor` can retry ONLY the `StoreAsync` upsert without re-sending the already-delivered Telegram message — see §5.2 invariant 1.)
...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Identifies which `HumanAction` in `AllowedActions` is the proposed default. **Read by `QuestionTimeoutService` at timeout and published verbatim as the `HumanDecisionEvent.ActionValue`** (the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list — see §10.3); also used for display/audit and for `QuestionRecoverySweep` backfill correlation. When `null`, the question has no proposed default action and the timeout fires with `ActionValue = "__timeout__"`. |
| `DefaultActionValue` | `string?` | The `HumanAction.Value` of the default action, resolved and denormalized at question-send time by `IPendingQuestionStore.StoreAsync`: the store looks up the `HumanAction` in `AgentQuestion.AllowedActions` whose `ActionId` matches `ProposedDefaultActionId` and persists its `Value` here. Read by the callback / RequiresComment text-reply path on `IDistributedCache` miss (architecture.md §5.2 invariant 3) so the resolved `ActionValue` survives cache eviction between button tap and text reply. `QuestionTimeoutService` does NOT read this column — at timeout it publishes `DefaultActionId` verbatim, leaving action-value resolution to the consuming agent (§10.3). When `null` (either because `ProposedDefaultActionId` is null or the matching `HumanAction` was not found in `AllowedActions`), the callback path falls back to deserialising `AgentQuestion` JSON. |
...
### 4.4 IOutboundQueue (defined in `AgentSwarm.Messaging.Abstractions`)
...
> **Layering note.** Both `IOutboundQueue` and `OutboundMessage` live in `AgentSwarm.Messaging.Abstractions` (per the Stage 1.4 brief). `OutboundMessage` was relocated from `Core` to `Abstractions` during Stage 1.4 implementation so the interface placement could be honored without forcing an `Abstractions → Core` project reference (which the established layering rule forbids). The record's only non-primitive dependencies (`MessageSeverity`, `CorrelationIdValidation`, `AgentQuestionEnvelope`) are all already in Abstractions, so the relocation introduces no new cross-assembly references.

`DequeueAsync` returns the highest-severity (`Critical > High > Normal > Low`) pending message, oldest-first within a severity, and returns `null` when the queue has no pending work. The `messageId` parameter on `MarkSentAsync` / `MarkFailedAsync` / `DeadLetterAsync` corresponds to `OutboundMessage.MessageId`; `telegramMessageId` is `long` per §3.1's canonical-type convention for Telegram identifiers.
...
Platform-agnostic outbound sending contract used by `OutboundQueueProcessor` (§10.4) to send messages without depending on the `AgentSwarm.Messaging.Telegram` assembly directly.
...
/// so the caller can pass it to MarkSentAsync (in OutboundQueueProcessor) and StoreAsync
/// (called directly inside TelegramMessageSender.SendQuestionAsync per Stage 3.5).
/// </summary>
public sealed record SendResult(long TelegramMessageId);
```

Both methods return `SendResult` containing the `TelegramMessageId` assigned by the Telegram Bot API. The `OutboundQueueProcessor` (§5.2, §10.4) uses `SendResult.TelegramMessageId` to call `IOutboundQueue.MarkSentAsync(messageId, telegramMessageId)` to persist the Telegram message ID on the `OutboundMessage` record. For question messages, `IPendingQuestionStore.StoreAsync(envelope, chatId, telegramMessageId)` is invoked **directly inside `TelegramMessageSender.SendQuestionAsync`** after the Telegram send succeeds (per Stage 3.5 workstream brief — see §5.2 invariant 1); the only persistence work `OutboundQueueProcessor` performs for question messages is the `PendingQuestionPersistenceException` recovery path (retry the `StoreAsync` upsert without re-sending the Telegram message). `SendResult` is defined in `AgentSwarm.Messaging.Core` alongside `IMessageSender`.
...
**Key invariants:**
1. The question includes `Severity`, `ExpiresAt`, `AllowedActions` rendered as inline keyboard buttons, and the proposed default action (if any). The default action is carried as sidecar metadata in `AgentQuestionEnvelope.ProposedDefaultActionId` (see §3.1). **Rendering boundary:** The `OutboundQueueProcessor` dispatches to `IMessageSender` based on `SourceType`: for `Question` messages, the processor deserializes `OutboundMessage.SourceEnvelopeJson` to recover the `AgentQuestionEnvelope` and calls `IMessageSender.SendQuestionAsync(chatId, envelope)` — `TelegramMessageSender` is the **sole owner** of question rendering (extracting `AgentQuestion` fields, building the `InlineKeyboardMarkup`, writing `HumanAction` entries to `IDistributedCache`, and displaying the proposed default in the message body). For all other `SourceType` values (`Alert`, `StatusUpdate`, `CommandAck`), the processor calls `IMessageSender.SendTextAsync(chatId, payload)` using the pre-rendered `OutboundMessage.Payload` (rendered by `TelegramMessengerConnector` at enqueue time — see §3.1 `Payload` field). **Stage 3.5 contract — sender owns post-send persistence directly.** After a successful `SendQuestionAsync` Telegram call, the `TelegramMessageSender` itself calls `IPendingQuestionStore.StoreAsync(envelope, chatId, telegramMessageId)` (per the Stage 3.5 workstream brief step 5 and §3.1 *Default action flow*). The store implementation denormalizes `ProposedDefaultActionId` into `PendingQuestionRecord.DefaultActionId` (read by `QuestionTimeoutService` and published verbatim as `HumanDecisionEvent.ActionValue` — see §10.3) and into `PendingQuestionRecord.DefaultActionValue` (read by the callback / RequiresComment text-reply path on cache miss only — see invariant 3 below). The record is created with `Status = Pending`. **Persistence-failure recovery contract:** if `StoreAsync` throws after the Telegram message has already been delivered, the sender wraps the failure in a typed `PendingQuestionPersistenceException` (carrying `QuestionId`, `TelegramChatId`, `TelegramMessageId`, `CorrelationId`) so that the Stage 4.1 `OutboundQueueProcessor` can pattern-match it and take a SPECIALIZED recovery path — calling `IPendingQuestionStore.StoreAsync` directly with the recovered envelope, NOT re-running `SendQuestionAsync` (which would re-send the message to Telegram, since Telegram has no idempotency on outbound sends). For non-question messages, no `PendingQuestionRecord` is created — `MarkSentAsync` is the only post-send call.
>
> **Cross-doc alignment (implementation-plan.md Stage 2.3 + Stage 3.5).** Implementation-plan.md Stage 1.4 and Stage 2.3 state that the sender reads `ProposedDefaultActionId` to *display* the proposed default in the message body. Implementation-plan.md Stage 3.5 step 5 ("Integrate store call site") additionally requires that the sender persist the `PendingQuestionRecord` **directly** after a successful Telegram send (per the workstream brief: "after successfully sending a question to Telegram, persist the question with its Telegram message ID for later lookup by `CallbackQueryHandler`"). Denormalisation of `ProposedDefaultActionId` → `DefaultActionId` / `DefaultActionValue` is performed inside `IPendingQuestionStore.StoreAsync` itself (in either invocation path — sender-direct in Stage 3.5 or `OutboundQueueProcessor`-driven if Stage 4.1 later moves the call). The typed `PendingQuestionPersistenceException` carries every key the Stage 4.1 `OutboundQueueProcessor` needs to recover a persistence-after-send failure by retrying `StoreAsync` directly (not by re-running `SendQuestionAsync`, which would re-send the Telegram message). Both methods on `IMessageSender` return `Task<SendResult>` (see §4.12).
...
6. If no operator responds before `ExpiresAt`, the timeout handler reads `PendingQuestionRecord.DefaultActionId` (the `HumanAction.ActionId` string denormalized at send time — see §3.1 `PendingQuestionRecord.DefaultActionId`). **Two branches:** (a) when `DefaultActionId` is non-null, the handler atomically claims the row via `MarkTimedOutAsync` then fires a `HumanDecisionEvent` with the `DefaultActionId` string verbatim as the `ActionValue` (the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list), and updates the Telegram message to "⏰ Timed out — default action applied: {DefaultActionId}"; (b) when `DefaultActionId` is null, the handler fires a `HumanDecisionEvent` with `ActionValue = "__timeout__"` (no automatic action is taken on behalf of the operator), and updates the Telegram message to "⏰ Timed out — no default action". In both branches, on publish failure the claim is reverted via `TryRevertTimedOutClaimAsync(priorStatus)` so the next sweep retries (at-least-once delivery); on publish success, `PendingQuestionRecord.Status` remains `TimedOut` and an audit record is written (see §10.3 for the full timeout flow).
...
│   │                                            IOutboundQueue, OutboundMessage,
│   │                                            OutboundSourceType, OutboundMessageStatus
│   │
│   ├── AgentSwarm.Messaging.Core← IOperatorRegistry,
│   │                                            IMessageSender, SendResult,
...
│                                                DI registration,
│                                                OutboundQueueProcessor (BackgroundService),
...
1. Atomically claims the row via `IPendingQuestionStore.MarkTimedOutAsync` — a conditional `UPDATE` filtered on `Status IN (Pending, AwaitingComment)` that returns `true` only when THIS sweeper transitioned the row. On `false`, another sweeper (or a late callback) won the race — skip silently. The claim happens FIRST, before publish, so no two sweepers can ever both publish a `HumanDecisionEvent` for the same row.
2. Reads `PendingQuestionRecord.DefaultActionId` (the `HumanAction.ActionId` string denormalized at send time from `AgentQuestionEnvelope.ProposedDefaultActionId` — see §3.1 `PendingQuestionRecord.DefaultActionId`). If present, the timeout service publishes a `HumanDecisionEvent` with the `DefaultActionId` string verbatim as the `ActionValue`; the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list. No `IDistributedCache` lookup or `AgentQuestion` JSON deserialization is required (the cache entry may already be evicted at timeout). If `DefaultActionId` is absent (`null`), publishes a `HumanDecisionEvent` with `ActionValue = "__timeout__"` so the agent is notified of timeout without an automatic decision.
3. Updates the original Telegram message (using `PendingQuestionRecord.TelegramMessageId`) to indicate the timeout ("⏰ Timed out — default action applied: *skip*" or "⏰ Timed out — no default action").
4. Writes an audit record noting the timeout.

On publish failure between the claim (step 1) and the audit (step 4), the timeout service calls `IPendingQuestionStore.TryRevertTimedOutClaimAsync(questionId, priorStatus)` — a conditional `UPDATE` filtered on `Status == TimedOut` that puts the row back to its pre-claim status (`Pending` or `AwaitingComment`) so the next sweep retries. The combined guarantee is **at-least-once delivery** of the timeout decision AND **single-claimant cross-process atomicity** per sweep iteration. The Telegram message edit (step 3) is best-effort and does NOT trigger revert — an edit failure leaves the row `TimedOut` with the `HumanDecisionEvent` already emitted (consistent with at-least-once-with-best-effort-display semantics).
...
|---|---|---|
| **P95 ≤ 2 s** (acceptance gate metric: `telegram.send.first_attempt_latency_ms`) | **Operator-scoped, bounded burst**: applies to first-attempt, non-429 sends (formal decision D-P95 per §11.1) under steady state (queue depth < 100) and bounded bursts (≤ 50 Critical+High messages **distributed across ≥ 10 operator chats with no single chat receiving more than 5 messages** — formal decision D-BURST per §11.1; deployments with fewer chats or skewed routing accept degraded burst P95 — see per-chat constraint below). This is the acceptance criterion that is tested and measured. | Priority queuing + token-bucket burst capacity + 10 concurrent workers. The 48th message of 50 completes in ~1,900 ms (see burst math below). |
...
> **⚠ SCOPE CLARIFICATION — OPERATOR-DIRECTED DECISION (burst-topology-envelope) — see §11.1 D-BURST**
>
> The story says: *"Support burst alerts from 100+ agents without message loss"* alongside *"P95 send latency under 2 seconds."*
>
> The P95 ≤ 2 s SLO under burst assumes a deployment topology where ≤ 50 Critical+High messages are **distributed across ≥ 10 operator chats with no single chat receiving more than 5 messages** (matching per-chat burst capacity), so the per-chat rate limit of 20 msg/min is not the binding constraint. This is narrower than the story's unconstrained "100+ agents" language.
>
> **Decision (operator-directed, persisted as D-BURST in §11.1):** ≥ 10 operator chats **is the expected deployment topology** — 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats with balanced routing (≤ 5 messages per chat during burst) a natural consequence. Deployments with fewer than 10 operator chats, or with skewed routing that concentrates >5 messages in any single chat, must accept that the P95 SLO applies only under steady-state load, not during bursts — this is documented as a capacity planning note, not a hidden limitation. The story's "without message loss" guarantee holds unconditionally regardless of chat count or distribution.
>
> **Risk:** If the deployment topology changes (e.g., consolidation to fewer operator chats) or agent-to-operator routing becomes skewed, the burst P95 envelope narrows and may no longer hold. Monitor `telegram.send.queue_dwell_ms` per chat to detect topology-driven or skew-driven degradation.
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats with max 5 per chat — formal decision D-BURST per §11.1; see §10.4 Burst Scenario item 3) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
...
3. **Multi-chat fan-out and per-chat constraint**: In production, 100+ agents typically span multiple tenants/workspaces, routing to multiple operator chats. The per-chat rate limit (20 msg/min ≈ 0.33 msg/s) constrains individual operators, while the global rate limit (30 msg/s) applies across all chats. With messages distributed across N operator chats, effective throughput is `min(30, N × 20/60)` msg/s. **Formal decision D-BURST (§11.1):** The burst math assumes ≤ 50 Critical+High messages distributed across ≥ 10 operator chats **with no single chat receiving more than 5 messages** (matching the per-chat burst capacity). The operator decided during story planning that ≥ 10 operator chats with balanced routing is the expected deployment topology — 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making this a natural consequence, not an arbitrary threshold. **Per-chat burst capacity is the load-bearing constraint:** each per-chat token bucket has burst capacity of 5 tokens (configurable via `Telegram:RateLimits:PerChatBurstCapacity`). When a chat receives ≤ 5 messages during a burst, all drain from burst tokens without per-chat wait — the global 30 msg/s limit is the binding constraint. When a chat receives >5 messages, the excess messages queue behind the per-chat rate limit at 0.33 msg/s sustained, adding ~3 s per excess message — this **violates the 2 s P95 for that chat's messages** regardless of global throughput headroom. Therefore, the burst SLO formally requires `max_messages_per_chat ≤ PerChatBurstCapacity` (default 5). With ≤ 50 messages across ≥ 10 chats and max 5 per chat, the per-chat burst capacity absorbs the burst and the global 30 msg/s limit is the binding constraint — matching the burst math above. **Skewed routing analysis:** If routing is skewed (e.g., 15 messages to one chat, 5 to each of 7 others), the 10 excess messages on the overloaded chat face per-chat rate limiting: messages 6–15 drain at 0.33 msg/s, taking ~30 s total. The P95 of the 50-message set would be dominated by the slowest chat, far exceeding 2 s. **For deployments with fewer than 10 operator chats or skewed routing**, the per-chat limit becomes the binding constraint and the P95 ≤ 2 s SLO cannot be met during bursts; such deployments must accept that the P95 SLO applies only under steady-state load. For 10+ chats with max 5 per chat, the global limit of 30 msg/s is the binding constraint and the ≤ 50 priority messages drain in ~1,900 ms (P95).
...
#### D-BURST — Burst Topology Envelope

- **Question ID:** `burst-topology-envelope`
- **Decision:** The P95 ≤ 2 s burst SLO assumes ≤ 50 Critical+High messages **distributed across ≥ 10 operator chats with no single chat receiving more than 5 messages** (matching the per-chat burst capacity `Telegram:RateLimits:PerChatBurstCapacity`, default 5). This is the expected deployment topology: 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats with balanced routing a natural consequence. Deployments with fewer than 10 operator chats, or deployments where skewed agent-to-operator routing causes any single chat to receive more than 5 burst messages, must accept that the P95 SLO applies only under steady-state load, not during bursts.
- **Rationale:** The per-chat rate limit (20 msg/min) becomes the binding constraint when messages concentrate in fewer chats. With ≥ 10 chats and ≤ 5 messages per chat, each chat's per-chat burst capacity (default 5 tokens) absorbs the burst without triggering per-chat rate-limit waits, and the global rate limit (30 msg/s) is the sole binding constraint — matching the burst math in §10.4. If any single chat receives >5 messages, the excess messages queue behind the per-chat rate limit (0.33 msg/s sustained), adding ~3 s per excess message — violating the 2 s P95. The max-5-per-chat constraint is therefore load-bearing for the SLO.
- **Impact:** If the deployment topology consolidates to fewer operator chats, or if agent-to-operator routing becomes skewed (e.g., many agents assigned to a single operator), the burst P95 envelope narrows and may no longer hold. Monitor `telegram.send.queue_dwell_ms` per chat to detect topology-driven or skew-driven degradation.
- **Status:** Decided (operator answer, iteration 11).
...
2. **`ISwarmCommandBus` query signatures — RESOLVED (iteration 4).** Implementation-plan.md Stage 1.3 now defines `QueryStatusAsync(SwarmStatusQuery query, CancellationToken)` and `QueryAgentsAsync(SwarmAgentsQuery query, CancellationToken)` with query-parameter records, matching architecture.md §4.6.

3. **`QuestionRecoverySweep` implementation stage — RESOLVED (iteration 5).** Implementation-plan.md now includes Stage 3.6 (`QuestionRecoverySweep`) with implementation steps covering Gap B detection, `SourceEnvelopeJson` deserialization, `PendingQuestionRecord` backfill, and three test scenarios.

4. **Observability metric names — RESOLVED (iteration 5).** Implementation-plan.md Stage 6.1 now configures the canonical metric set from architecture.md §8.

5. **`IDistributedCache` expiry — RESOLVED (iteration 5).** All documents now aligned on `ExpiresAt + 5 minutes`.

6. **`InboundRecoverySweep` assembly/configuration — RESOLVED (iteration 6).** Architecture.md §2.2 and implementation-plan.md Stage 2.4 both place `InboundRecoverySweep` in `AgentSwarm.Messaging.Worker`.

7. **Latency metric scope wording — RESOLVED (iteration 5).** All documents now use precise 429-only exclusion wording.

8. **`PendingQuestionRecord` post-send persistence — RE-RESOLVED (Stage 3.5 iter-3/iter-5).** Iteration 10 settled the call site at `OutboundQueueProcessor` (post-send hook). Stage 3.5 implementation revisited this and committed to the workstream brief's wording: `TelegramMessageSender.SendQuestionAsync` calls `IPendingQuestionStore.StoreAsync` **directly** after the Telegram send succeeds (per architecture.md §5.2 invariant 1, the new contract). `IPendingQuestionStore.StoreAsync` denormalizes `ProposedDefaultActionId` into `PendingQuestionRecord.DefaultActionId` (and `DefaultActionValue`) at persistence time. Persistence failures after a successful Telegram send are wrapped in `PendingQuestionPersistenceException` carrying QuestionId / chatId / messageId / correlationId so the Stage 4.1 `OutboundQueueProcessor` can recover by retrying ONLY the `StoreAsync` upsert (NOT a generic re-send, which would duplicate the Telegram message). The Stage 3.5 test scenario "Question stored on send" now targets `TelegramMessageSender.SendQuestionAsync` directly.

9. **`IPendingQuestionStore` interface surface — RESOLVED (iteration 10).** Implementation-plan.md Stage 1.3 now includes all eight methods from architecture.md §4.7: `StoreAsync`, `GetAsync`, `GetByTelegramMessageAsync`, `MarkAnsweredAsync`, `MarkAwaitingCommentAsync`, `RecordSelectionAsync`, `GetAwaitingCommentAsync`, and `GetExpiredAsync`.

10. **`IOutboundQueue` assembly placement — RESOLVED (Stage 1.4 implementation).** Architecture.md (§4.4 + §6 assembly map) and implementation-plan.md (Stage 1.4) place `IOutboundQueue` in `AgentSwarm.Messaging.Abstractions` per the Stage 1.4 brief. To avoid a forbidden `Abstractions → Core` project reference (every method on `IOutboundQueue` names `OutboundMessage`), `OutboundMessage` and its companion enums (`OutboundSourceType`, `OutboundMessageStatus`) were relocated from `Core` to `Abstractions` during Stage 1.4 — the record's only dependencies (`MessageSeverity`, `CorrelationIdValidation`, `AgentQuestionEnvelope`) are already in Abstractions, so the relocation introduces no new cross-assembly references. `TaskOversight` and `ITaskOversightRepository` continue to live in `Core` because `OperatorBinding` is a Core type.
...
14. **`OutboundQueueProcessor` dependency boundary — RESOLVED (iteration 13).** Implementation-plan.md Stage 4.1 now specifies that `OutboundQueueProcessor` sends via `IMessageSender` (the abstraction defined in Core), not the concrete `TelegramMessageSender`. The `TelegramMessageId` flows from `SendResult` (returned by `IMessageSender`) to `MarkSentAsync` (which consumes it) and `StoreAsync` — the message-ID flow is now consistent with architecture.md §4.12 and §5.2.

15. **Timeout `ActionValue` semantics — RE-RESOLVED (Stage 3.5 iter-3/iter-4).** Iteration 13 settled the contract on `DefaultActionValue` (the resolved `HumanAction.Value`) as the published `ActionValue`. Stage 3.5 implementation revisited this and flipped the source to `DefaultActionId` (the `HumanAction.ActionId` string); see Decision 17 below for the full rationale. The timeout path now publishes `DefaultActionId` verbatim as `HumanDecisionEvent.ActionValue`; the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list.
16. **`PendingQuestion` DTO completeness — RESOLVED (iteration 15).** Implementation-plan.md Stage 1.3 `PendingQuestion` DTO now includes all fields required by the architecture's timeout and RequiresComment flows: `DefaultActionId` (the primary timeout source, used by `QuestionTimeoutService.ApplyTimeoutAsync` per §10.3), `DefaultActionValue` (denormalized `HumanAction.Value`, retained for callback / comment-fallback paths and audit/display), `SelectedActionId` (for tracking operator's button selection), `SelectedActionValue` (for emitting `HumanDecisionEvent.ActionValue` from durable storage on interactive paths), `RespondentUserId` (for correlating follow-up text replies), and `StoredAt` (for deterministic tie-breaking in `GetAwaitingCommentAsync`). These fields match the `PendingQuestionRecord` persistence entity defined in architecture.md §3.1.
...
17. **Timeout semantics `DefaultActionId` → `DefaultActionValue` — RE-RESOLVED (Stage 3.5 iter-3/iter-4).** Iteration 15 settled the denormalisation contract on `DefaultActionValue` as the timeout-emission source. Stage 3.5 implementation revisited this and flipped the source-of-truth back to `DefaultActionId`: `QuestionTimeoutService` now reads `PendingQuestionRecord.DefaultActionId` (the `HumanAction.ActionId` string) and publishes that string verbatim as `HumanDecisionEvent.ActionValue` (the consuming agent resolves the full `HumanAction.Value` semantics from its own `AllowedActions` list, per §10.3). `DefaultActionValue` is still denormalised at send time but is now used only by callback / comment-fallback paths (when the `IDistributedCache` entry has been evicted) and for audit / display. Tech-spec.md S-3 (line 25), HC-3 (line 67), R-9 (line 98), §8 Acceptance Criteria (line 126), and e2e-scenarios.md scenario at line 114 have all been updated to match this resolution.
