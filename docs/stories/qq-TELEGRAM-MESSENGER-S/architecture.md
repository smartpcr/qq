...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Stored here so that `QuestionTimeoutService` can apply the default action directly from this field without depending on `IDistributedCache` (whose entries expire at `AgentQuestion.ExpiresAt` per tech-spec D-3 and may be evicted when the timeout fires). When present, the timeout service publishes a `HumanDecisionEvent` with this value as the action. When `null`, the question expires with `ActionValue = "__timeout__"`. |
...
> **Cross-doc canonical status (IOperatorRegistry).** This document (§4.3) is the **canonical source** for the `IOperatorRegistry` interface contract. The contract has **six** methods: `GetBindingsAsync`, `GetAllBindingsAsync`, `GetByAliasAsync`, `GetByWorkspaceAsync`, `RegisterAsync`, and `IsAuthorizedAsync`. All signatures are specified above in the code block. Implementation-plan.md Stage 2.7 alert fallback routing (line 242) already calls `IOperatorRegistry.GetByWorkspaceAsync(event.WorkspaceId)` to resolve the workspace-default operator when no `TaskOversight` record exists. Implementation-plan.md Stage 1.3 must define all six methods with matching signatures — including `GetByWorkspaceAsync(string workspaceId, CancellationToken)` returning `IReadOnlyList<OperatorBinding>` — and Stage 3.4 `PersistentOperatorRegistry` must implement it by querying active `OperatorBinding` rows filtered by `WorkspaceId`. Any implementation-plan iteration that omits `GetByWorkspaceAsync` from Stage 1.3 is out of alignment with this canonical contract and must be corrected.
...
3. **Multi-chat fan-out and per-chat constraint**: In production, 100+ agents typically span multiple tenants/workspaces, routing to multiple operator chats. The per-chat rate limit (20 msg/min ≈ 0.33 msg/s) constrains individual operators, while the global rate limit (30 msg/s) applies across all chats. With messages distributed across N operator chats, effective throughput is `min(30, N × 20/60)` msg/s. **The burst math assumes multi-chat distribution.** For 10+ operator chats, the global limit of 30 msg/s is the binding constraint and the ≤ 50 priority messages drain in ~1,900 ms (P95). For fewer chats, the per-chat limit becomes binding: a single operator chat can sustain only ~0.33 msg/s, so 50 messages to a single chat would take ~150 s — far exceeding the 2 s SLO. **The P95 ≤ 2 s SLO therefore requires that burst messages route to at least 2 operator chats** (2 chats × 20 msg/min = ~0.67 msg/s; 50 messages / 0.67 ≈ 75 s — still too slow) — **or more precisely, that the per-chat message count stays within the per-chat token-bucket burst capacity.** In practice, 100+ agents span multiple workspaces with distinct operator assignments, producing natural multi-chat distribution. The per-chat limiter (20 msg/min) is sized for sustained flow, not burst — during a burst, the per-chat token bucket has its own burst capacity (default 5 tokens, configurable via `Telegram:RateLimits:PerChatBurstCapacity`) that allows the first 5 messages per chat to drain without per-chat wait. With ≤ 50 messages spread across ≥ 10 operator chats (~5 messages per chat), the per-chat burst capacity absorbs the burst and the global 30 msg/s limit is the binding constraint — matching the burst math. If a deployment has only 1–2 operator chats, the operator must accept that the P95 SLO applies only to the global throughput capacity, not per-chat throughput, and individual chat delivery times will exceed 2 s during bursts.
