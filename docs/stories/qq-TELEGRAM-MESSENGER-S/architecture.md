...
| `DefaultActionId` | `string?` | Denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at question-send time. Stored here so that `QuestionTimeoutService` can identify the default action without depending on `IDistributedCache` (whose entries expire at `AgentQuestion.ExpiresAt` per tech-spec D-3/R-3 and may be evicted when the timeout fires). When present, the timeout service uses this `ActionId` to look up the corresponding `HumanAction` from the stored `AgentQuestion` JSON and publishes a `HumanDecisionEvent` with `HumanAction.Value` as the `ActionValue` — consistent with the button-press path. When `null`, the question expires with `ActionValue = "__timeout__"`. |
...
> **Cross-doc note (ISwarmCommandBus query signatures — mandatory correction):** Implementation-plan.md Stage 1.3 currently declares `QueryStatusAsync(CancellationToken)` and `QueryAgentsAsync(CancellationToken)` without query parameter objects. This architecture document is the **canonical contract** for `ISwarmCommandBus` and introduces `SwarmStatusQuery` and `SwarmAgentsQuery` to carry the workspace scope and optional filter/task-ID required by §5.7. **This is a mandatory correction:** implementation-plan.md Stage 1.3 must be updated to adopt `QueryStatusAsync(SwarmStatusQuery query, CancellationToken ct)` and `QueryAgentsAsync(SwarmAgentsQuery query, CancellationToken ct)` before implementation begins. Until the implementation plan is corrected, implementers must follow the signatures defined in this section. The stub `StubSwarmCommandBus` (implementation-plan.md Stage 6.3) must also adopt the query-parameter signatures, returning empty collections from the query objects' workspace scope.
...
6. If no operator responds before `ExpiresAt`, a timeout handler reads `PendingQuestionRecord.DefaultActionId` (denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at send time). **Two branches:** (a) when `DefaultActionId` is non-null, the handler deserializes `PendingQuestionRecord.AgentQuestion` (the full `AgentQuestion` JSON), finds the `HumanAction` entry whose `ActionId` matches `DefaultActionId`, and fires a `HumanDecisionEvent` with that `HumanAction.Value` as the `ActionValue` — consistent with the button-press path (no `IDistributedCache` lookup — the cache entry expires at `AgentQuestion.ExpiresAt` per tech-spec D-3/R-3, but `QuestionTimeoutService` does not depend on it; the stored `AgentQuestion` JSON contains the full `AllowedActions` array), and updates the Telegram message to "⏰ Timed out — default action applied: {action}"; (b) when `DefaultActionId` is null, the handler fires a `HumanDecisionEvent` with `ActionValue = "__timeout__"` (no automatic action is taken on behalf of the operator), and updates the Telegram message to "⏰ Timed out — no default action". In both branches, `PendingQuestionRecord.Status` transitions to `TimedOut` and an audit record is written (see §10.3 for the full timeout flow).
...
1. Reads `PendingQuestionRecord.DefaultActionId` (denormalized from `AgentQuestionEnvelope.ProposedDefaultActionId` at send time). If present, the timeout service deserializes `PendingQuestionRecord.AgentQuestion` (the full `AgentQuestion` JSON stored at record creation), finds the `HumanAction` entry whose `ActionId` matches `DefaultActionId`, and publishes a `HumanDecisionEvent` with that `HumanAction.Value` as the `ActionValue` — consistent with the button-press path, which also resolves `Value` from the `HumanAction` rather than using `ActionId` directly. It does **not** look up the full `HumanAction` from `IDistributedCache`, because the cache entry expires at `AgentQuestion.ExpiresAt` (per tech-spec D-3/R-3) and may already be evicted when the timeout fires; the stored `AgentQuestion` JSON in `PendingQuestionRecord` contains the complete `AllowedActions` array, making the cache unnecessary. If `DefaultActionId` is absent (`null`), publishes a `HumanDecisionEvent` with `ActionValue = "__timeout__"` so the agent is notified of timeout without an automatic decision.
...
|---|---|---|
| **P95 ≤ 2 s** (acceptance gate metric: `telegram.send.first_attempt_latency_ms`) | **Operator-scoped, bounded burst**: applies to first-attempt, non-429 sends under steady state (queue depth < 100) and bounded bursts (≤ 50 Critical+High messages **distributed across multiple operator chats** such that no single chat receives more than 20 messages per minute — the ≤ 50 message count and ≥ 10 chat distribution are a resolved architecture constraint: 100+ agents inherently span multiple tenants/workspaces with distinct operator assignments, making ≥ 10 operator chats the expected deployment topology; deployments with fewer chats accept degraded burst P95 — see per-chat constraint below). This is the acceptance criterion that is tested and measured. | Priority queuing + token-bucket burst capacity + 10 concurrent workers. The 48th message of 50 completes in ~1,900 ms (see burst math below). |
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats so per-chat burst capacity is not exhausted — resolved architecture constraint; see §10.4 Burst Scenario item 3) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
