...
> **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **enqueue instant** (`OutboundMessage.CreatedAt`, the moment the message is persisted to the outbound queue) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** delayed by a 429 rate-limit hold. This measurement preserves the story requirement "P95 send latency under 2 seconds **after event is queued**" by starting the clock at enqueue, not dequeue. **Operator decision (`p95-metric-scope`):** The operator was asked whether the P95 measurement should include retried sends and rate-limited wait time, or only first-attempt, non-rate-limited sends. The operator answered: **first-attempt, non-rate-limited sends only.** This decision is recorded here in architecture.md §10.4 as the authoritative source. This refinement materially narrows the original story wording ("P95 send latency under 2 seconds after event is queued") by excluding retry and rate-limit overhead from the acceptance gate; the narrowing is justified because retries and rate-limit holds are Telegram-side constraints outside the system's control, and the operator explicitly accepted this scope. This is the metric the **P95 ≤ 2 s** acceptance criterion applies to.
>
...
- **P95 ≤ 2 s** applies to `telegram.send.first_attempt_latency_ms` — enqueue (`OutboundMessage.CreatedAt`) to HTTP 200, first-attempt, non-rate-limited sends only. **Operator decision (`p95-metric-scope`), recorded in this document (§10.4):** the operator confirmed that the P95 measurement scope covers first-attempt, non-rate-limited sends only — not retried sends or rate-limited wait time. This materially narrows the original story wording ("P95 send latency under 2 seconds after event is queued") by excluding retry and rate-limit overhead from the acceptance gate; the narrowing is justified because retries and rate-limit holds are Telegram-side constraints outside the system's control. Retried/rate-limited sends are tracked separately via `telegram.send.all_attempts_latency_ms`. **Under normal operating conditions** (steady state, queue depth < 100), queue dwell is negligible and this target holds across all severities. **Under burst** (1000+ simultaneous messages), the P95 ≤ 2 s target holds for Critical+High messages only when the burst contains ≤ 60 Critical+High messages total (the **bounded-volume assumption**). At 30 msg/s, the top 60 priority-queued messages drain within ~2 s of queue dwell; any Critical+High messages beyond position 60 incur proportionally longer queue dwell and may exceed the 2 s P95. For reference, e2e-scenarios.md defines two burst profiles: a bounded burst (30 Critical + 30 High + 440 Normal + 500 Low = 1000 messages, 60 Critical+High within bound) and an exceeding burst (200 Critical + 200 High + 300 Normal + 300 Low = 1000 messages, 400 Critical+High exceeding bound). Normal/Low severity messages will exceed the 2-second P95 under any significant burst. **In summary, P95 ≤ 2 s is a steady-state SLO** that also holds for bounded Critical+High bursts (≤ 60), and degrades gracefully beyond that bound while guaranteeing zero message loss and priority ordering (Critical before High before Normal before Low).
- **Zero message loss** — every message is either delivered to Telegram or dead-lettered with a traceable reason and available for manual replay. **Reconciliation with story wording:** the story requires "support burst alerts from 100+ agents **without message loss**." This architecture interprets "without message loss" as "no silent discard" — every message that enters the outbound queue is either (a) delivered to Telegram successfully, or (b) moved to the dead-letter queue with a recorded reason (`retry_exhaustion` or `backpressure:queue_depth_exceeded`) and retained for operator-initiated manual replay. Backpressure dead-lettering (Low-severity only, when queue depth exceeds `MaxQueueDepth`) does **not** achieve eventual Telegram delivery automatically; it preserves the message durably and alerts the operator, who can replay after the burst subsides. This is an explicit trade-off: under extreme sustained load, the system prioritizes delivery of higher-severity messages (Critical/High/Normal are never backpressure-DLQ'd) while ensuring no Low-severity message is silently lost. The `telegram.messages.backpressure_dlq` counter and dead-letter queue provide full visibility and recovery capability.
...
| **Burst load** (100+ agents, 1000+ messages) | P95 `telegram.send.first_attempt_latency_ms` ≤ 2 s for Critical/High when burst contains ≤ 60 Critical+High messages (bounded-volume assumption; e2e-scenarios.md bounded burst: 30 Critical + 30 High = 60). **Not guaranteed** when Critical+High exceeds 60 (e2e-scenarios.md exceeding burst: 200 Critical + 200 High = 400). Normal/Low severities experience longer dwell under any burst. Zero message loss guaranteed. | Priority queuing + `queue_dwell_ms` / `all_attempts_latency_ms` for visibility. |
| **Beyond capacity envelope** (sustained inflow > 30 msg/s) | No silent message loss; queue dwell increases for lower severities. Low-severity messages are backpressure-DLQ'd (dead-lettered with reason, retained for manual replay) when queue depth exceeds `MaxQueueDepth` (5000) — this is not automatic Telegram delivery, but preserves recoverability. Critical/High/Normal messages are always accepted and delivered. | Backpressure DLQ + `telegram.messages.backpressure_dlq` counter + operator alert. |
...
Under **extreme burst** (1000+ simultaneous messages), the Telegram rate limit of 30 msg/s is the binding constraint. Priority queuing ensures Critical/High messages are dequeued before Normal/Low, reducing their queue dwell relative to lower severities. E2e-scenarios.md defines two burst profiles: a **bounded burst** (30 Critical + 30 High + 440 Normal + 500 Low = 1000 messages) where the 60 Critical+High messages drain in ~2 s at 30 msg/s and meet the P95 target; and an **exceeding burst** (200 Critical + 200 High + 300 Normal + 300 Low = 1000 messages) where 400 Critical+High messages drain in ~13 s, causing most to exceed the 2 s P95. The P95 ≤ 2 s target is therefore a **steady-state SLO** that extends to bounded Critical+High bursts (≤ 60) and degrades gracefully beyond. Under burst the architecture guarantees **no silent message loss** (every message is delivered or dead-lettered with a traceable reason and retained for replay — see reconciliation in §10.4) and **priority ordering**, with `telegram.send.queue_dwell_ms` surfacing queue depth for capacity-planning response and `telegram.send.all_attempts_latency_ms` providing the all-inclusive delivery latency view.
