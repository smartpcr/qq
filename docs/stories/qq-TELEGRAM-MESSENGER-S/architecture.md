...
| **InboundRecoverySweep** | `AgentSwarm.Messaging.Telegram` (to be created) | `BackgroundService` that runs on startup and periodically (configurable interval, default 60 s). Queries `IInboundUpdateStore` for `InboundUpdate` records with `IdempotencyStatus` in `Received`, `Processing`, or `Failed` where `AttemptCount < MaxRetries` (configurable via `InboundRecovery:MaxRetries`, default 3). Deserializes each record's `RawPayload` and re-feeds it into `ITelegramUpdatePipeline.ProcessAsync` for idempotent re-processing. Increments `AttemptCount` on failure. This component owns the no-command-loss guarantee across process restarts — see §3.1 `InboundUpdate` and §5.1 for the recovery flow. |
...
The Telegram connector publishes commands (task creation, approvals, pauses) via `PublishCommandAsync` and queries swarm state via `QueryStatusAsync` and `QueryAgentsAsync`. The `/status` handler constructs `SwarmStatusQuery { WorkspaceId, TaskId? }` from the operator's binding and optional command argument; the `/agents` handler constructs `SwarmAgentsQuery { WorkspaceId, Filter? }` similarly (see §5.7 command table). These four outbound methods align with implementation-plan.md Stage 1.3.
...
> **Cross-doc alignment:** Implementation-plan.md Stage 1.3 defines all four methods on `ISwarmCommandBus`: `PublishCommandAsync`, `QueryStatusAsync(SwarmStatusQuery, CancellationToken)`, `QueryAgentsAsync(SwarmAgentsQuery, CancellationToken)`, and `SubscribeAsync(string tenantId, CancellationToken)` returning `IAsyncEnumerable<SwarmEvent>`. The query parameter objects (`SwarmStatusQuery`, `SwarmAgentsQuery`) carry workspace scope and optional filter/task-ID, enabling the `/status` and `/agents` commands to pass the operator's workspace and command arguments through to the orchestrator. All sibling documents are aligned on this interface contract.
>
> **Cross-doc note:** Implementation-plan.md Stage 1.3 currently declares `QueryStatusAsync(CancellationToken)` and `QueryAgentsAsync(CancellationToken)` without query parameter objects. This architecture document introduces `SwarmStatusQuery` and `SwarmAgentsQuery` to carry the workspace scope and optional filter/task-ID required by §5.7. The implementation plan should be updated to adopt these parameter objects; until then, the architecture document is the canonical contract for query method signatures.
...
| **Bounded burst** (≤ 50 Critical+High in a 1000-msg burst, distributed across ≥ 10 operator chats so per-chat burst capacity is not exhausted — **architecture assumption**, not operator-approved; see §10.4 Burst Scenario item 3) | **Yes**, Critical+High only; Normal/Low exceed 2 s | At-least-once delivered or dead-lettered |
