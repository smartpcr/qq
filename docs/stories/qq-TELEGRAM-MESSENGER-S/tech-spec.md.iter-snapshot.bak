...
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. **Default-action design — RESOLVED, sidecar envelope model.** All sibling documents are now aligned: architecture.md §3.1 (lines 167–190) defines the shared `AgentQuestion` model **without** a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` sidecar metadata on `AgentQuestionEnvelope`. Implementation-plan.md Stage 1.2 (line 36) follows the same sidecar approach. E2e-scenarios.md (lines 57–63, line 75, footer line 641) now explicitly states "All sibling documents are aligned on the sidecar envelope model" and tests accordingly. **Note:** architecture.md line 195 still carries a stale "UNRESOLVED" label in its cross-doc status note, and implementation-plan.md line 36 still carries a stale cross-doc note saying e2e declares the divergence unresolved — both are editorial artifacts from earlier iterations that have not yet been cleaned up; the substantive content in all three documents is consistent. The behavioral contract: the Telegram connector reads `ProposedDefaultActionId` from the `AgentQuestionEnvelope`, denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays the default in the message body, and applies it on timeout. When the question times out, `QuestionTimeoutService` reads `DefaultActionId` and applies the action automatically; when `null`, the question expires with `ActionValue = "__timeout__"`. See Section 10 alignment notes. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. **Cross-document alignment — RESOLVED, sidecar envelope model.** All sibling documents are now aligned on the sidecar approach: architecture.md §3.1 (lines 167–190) defines the shared `AgentQuestion` model **without** a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` on `AgentQuestionEnvelope`. Implementation-plan.md Stage 1.2 (line 36) follows this approach. E2e-scenarios.md (lines 57–63, line 75, footer line 641) now explicitly states "All sibling documents are aligned on the sidecar envelope model" and tests the sidecar approach directly. **Stale labels:** architecture.md line 195 still carries an "UNRESOLVED" cross-doc status note, and implementation-plan.md line 36 still carries a cross-doc note referencing an earlier e2e-scenarios.md state — both are editorial holdovers; the substantive modeling content is consistent across all documents. The behavioral contract: the Telegram connector reads `ProposedDefaultActionId` from the envelope, denormalizes it into `PendingQuestionRecord.DefaultActionId` for timeout polling, and applies it on timeout. When `null`, the question expires with `ActionValue = "__timeout__"`. | Story requirement; resolved — sidecar envelope model adopted across all sibling docs (see Section 10) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Cross-document alignment — RESOLVED, all-inclusive metric.** All sibling documents are now aligned on the all-inclusive interpretation: `telegram.send.latency_ms` (primary) measures elapsed time from `OutboundMessage.CreatedAt` (enqueue) to Telegram API HTTP 200 for **all** messages regardless of attempt number or rate-limit holds. The P95 ≤ 2 s acceptance criterion applies to this all-inclusive metric. Architecture.md §10.4 (lines 668–691) defines the all-inclusive metric; implementation-plan.md (lines 357, 540) follows architecture.md; e2e-scenarios.md (lines 260–288, footer line 645) now uses the same all-inclusive definition — line 272 asserts "P95 telegram.send.latency_ms across ALL 100 messages" and line 274 explicitly states "telegram.send.latency_ms is the all-inclusive primary metric covering all sends regardless of attempt number or rate-limit holds." A separate diagnostic metric `telegram.send.first_attempt_latency_ms` (first-attempt, non-rate-limited successes only) is emitted for capacity planning. Additional diagnostics: `telegram.send.rate_limited_wait_ms` for 429 backoff visibility. Under normal conditions (queue depth < 100, no active rate-limiting), all severities comfortably meet the 2-second target. Under extreme burst (100+ agents, 1000+ messages), priority queuing ensures Critical/High messages are dispatched first; Normal/Low may experience queue delays. The operator has confirmed (answer to `p95-latency-scope`) that the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately. This is compatible with the all-inclusive metric design: `telegram.send.latency_ms` remains the primary all-inclusive metric for monitoring, but the acceptance gate uses `telegram.send.first_attempt_latency_ms` for pass/fail evaluation. | Story requirement; resolved — all-inclusive `telegram.send.latency_ms` for monitoring, `telegram.send.first_attempt_latency_ms` for acceptance gate (see Section 10) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) is now **resolved** across all sibling documents: `telegram.send.latency_ms` is the all-inclusive primary metric (all messages, all attempts, including rate-limit waits); `telegram.send.first_attempt_latency_ms` is the diagnostic metric for first-attempt, non-rate-limited successes used as the acceptance gate per operator guidance. See HC-4 and Section 10 for details. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
