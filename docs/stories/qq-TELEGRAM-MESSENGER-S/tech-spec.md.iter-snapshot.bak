...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. **All sibling documents are aligned on the sidecar envelope model.** The shared `AgentQuestion` model does **not** include a `DefaultAction` property (e2e-scenarios.md lines 57–62, architecture.md §3.1 lines 167–190, implementation-plan.md Stage 1.2). The proposed default action is carried as sidecar metadata via `ProposedDefaultActionId` on the `AgentQuestionEnvelope`. The Telegram connector reads `ProposedDefaultActionId` from the envelope, denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling, and applies it on timeout. When the question times out, `QuestionTimeoutService` reads `DefaultActionId`, resolves the full `HumanAction` from `IDistributedCache`, and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. | Story requirement; aligned across all sibling docs |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. Per architecture.md §10.4 (lines 664–691) and e2e-scenarios.md (lines 265–280), the following latency metrics are defined: **`telegram.send.latency_ms`** (primary) measures elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API HTTP 200, measured for **all** messages regardless of attempt number or rate-limit holds. This all-inclusive metric is the one the **P95 ≤ 2 s acceptance criterion** applies to — it captures the true end-to-end experience from the perspective of the queued event, including any retries and rate-limit waits. This is the honest interpretation of the story requirement ("P95 send latency under 2 seconds after event is queued"). **`telegram.send.first_attempt_latency_ms`** (diagnostic) measures the same interval but covers **only** messages that succeed on their first delivery attempt and are **not** waiting behind a 429 rate-limit hold — this narrower metric isolates the system's inherent processing latency for capacity planning. Additionally, `telegram.send.retry_latency_ms` (diagnostic) tracks latency for retried messages, and `telegram.send.rate_limited_wait_ms` (diagnostic) tracks time spent in 429 backoff. Under normal operating conditions (queue depth < 100, no active rate-limiting), all severities comfortably meet the 2-second target. Under extreme burst conditions (100+ agents, 1000+ messages), priority queuing ensures Critical/High severity messages are dispatched first and meet the target; Normal/Low severity messages are delivered without loss but may experience queue delays that transiently push the all-inclusive P95 above 2 seconds for the burst window. | Story requirement + architecture.md §10.4 (lines 664–691) + e2e-scenarios.md (lines 265–280) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) applies to the **all-inclusive primary `telegram.send.latency_ms` metric** (architecture.md §10.4 lines 664–691; e2e-scenarios.md lines 265–280), which measures elapsed time from enqueue to HTTP 200 for **all** messages regardless of attempt number or rate-limit holds. Rate-limit waits and retries are **included** in this primary metric. Diagnostic metrics — `telegram.send.first_attempt_latency_ms` (first-attempt, non-rate-limited successes only), `telegram.send.retry_latency_ms` (retried messages), and `telegram.send.rate_limited_wait_ms` (429 backoff time) — provide operational visibility. Under normal conditions most sends succeed on first attempt and comfortably meet the 2-second target; under extreme burst, priority queuing ensures Critical/High messages are dispatched first and meet the target, while Normal/Low may experience queue delays that transiently push the all-inclusive P95 above 2 seconds for the burst window. | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| Agent asks blocking question → Telegram user answers from mobile | S-3 (question rendering), S-4 (decision events), S-11 (inline buttons). Question timeout/default action: all sibling documents are aligned — the shared `AgentQuestion` model does not include a `DefaultAction` property; the proposed default is carried as `ProposedDefaultActionId` sidecar metadata on the `AgentQuestionEnvelope` (e2e-scenarios.md lines 57–77, architecture.md §3.1, implementation-plan.md Stage 1.2). The connector reads `ProposedDefaultActionId` from the envelope and denormalizes it into `PendingQuestionRecord.DefaultActionId` (HC-3). `QuestionTimeoutService` reads this field at expiry to apply the default or emit `__timeout__`. |
...
The following items were identified as cross-document alignment points during iteration reviews. All items below are now aligned across sibling documents.
...
| P95 latency scoping (HC-4) | < 2 s target applies to `telegram.send.latency_ms` (primary), which measures elapsed time from enqueue to HTTP 200 for **all** messages regardless of attempt number or rate-limit holds — the all-inclusive metric. A separate `telegram.send.first_attempt_latency_ms` (diagnostic) covers only first-attempt, non-rate-limited successes for capacity planning. | §10.4 (lines 664–691): "`telegram.send.latency_ms` (primary) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200 (acceptance), measured for **all** messages regardless of attempt number or rate-limit holds. This all-inclusive metric is the one the P95 ≤ 2 s acceptance criterion applies to." `telegram.send.first_attempt_latency_ms` (diagnostic) = first-attempt, non-rate-limited successes only. **Aligned** with this spec. | Not specified numerically | Lines 265–280: "`telegram.send.latency_ms` (primary) covers all sends regardless of attempt number or rate-limit holds; P95 ≤ 2s applies to this all-inclusive metric." `telegram.send.first_attempt_latency_ms` (diagnostic) covers only first-attempt, non-rate-limited successes (line 280). **Aligned** with this spec and architecture.md. | **All documents are aligned.** The P95 ≤ 2 s target applies to the all-inclusive `telegram.send.latency_ms` primary metric. The narrower `telegram.send.first_attempt_latency_ms` is a diagnostic metric for capacity planning. |
...
| DefaultAction model (HC-3) | **All sibling documents are aligned on the sidecar envelope model.** The shared `AgentQuestion` model does not include a `DefaultAction` property. The proposed default action is carried as `ProposedDefaultActionId` on the `AgentQuestionEnvelope`. The connector reads `ProposedDefaultActionId` from the envelope, denormalizes it into `PendingQuestionRecord.DefaultActionId`, displays it in the message body, and applies it on timeout. | §3.1 (lines 167–190): models `AgentQuestion` **without** a `DefaultAction` property. The proposed default action is carried as sidecar metadata via `ProposedDefaultActionId` on the `AgentQuestionEnvelope` (lines 183–190). §3.1 line 169: "The shared `AgentQuestion` model does **not** include a `DefaultAction` property; the proposed default action is carried as sidecar metadata." **Aligned.** | Stage 1.2 (line 36): defines `AgentQuestion` **without** a `DefaultAction` property — "the shared model does **not** include a `DefaultAction` property; per e2e-scenarios.md (lines 57–62), the proposed default action is provided as sidecar metadata via `ProposedDefaultActionId` in the `AgentQuestionEnvelope`." Creates `AgentQuestionEnvelope` with `ProposedDefaultActionId` (line 37). **Aligned.** | Lines 57–62: "The shared AgentQuestion model does NOT include a DefaultAction property … is carried as sidecar metadata via ProposedDefaultActionId on the AgentQuestionEnvelope." Line 75: `ProposedDefaultActionId = act-1`. Line 77: "the Telegram connector reads ProposedDefaultActionId 'act-1' from the AgentQuestionEnvelope." Line 618: confirms sidecar model. **Aligned.** | **All documents are aligned** on the sidecar envelope model. No divergence exists. |
