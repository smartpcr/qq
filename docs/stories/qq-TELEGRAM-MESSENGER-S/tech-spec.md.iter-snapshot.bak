...
| S-2 | **Command handling** | `/start`, `/status`, `/agents`, `/ask`, `/approve`, `/reject`, `/handoff`, `/pause`, `/resume` — parsed, validated, and dispatched to the Agent Swarm Orchestrator. `/handoff TASK-ID @operator-alias` semantics: architecture.md §5.5 (lines 483–496) and e2e-scenarios.md (lines 360–376) specify full oversight transfer (Decided) with validation, notification, and audit. Implementation-plan.md Stage 3.2 (line 211) currently implements a **V1 stub** that validates syntax only, returns a stub acknowledgement, persists an audit record, but does not validate task existence, resolve the target operator, create/update a `TaskOversight` record, or notify the target operator. See D-4 and §10 for the cross-doc reconciliation note. |
| S-3 | **Agent-to-human questions** | Render `AgentQuestion` as Telegram messages with inline keyboard buttons for each `HumanAction`. Include context, severity, timeout, and proposed default action in the message body. Per **architecture.md §3.1** (lines 167–184), the shared `AgentQuestion` model **includes** a nullable `DefaultAction` property (`string?`) as a first-class shared field — the `ActionId` of the proposed default from `AllowedActions`. At render time, the Telegram connector reads `AgentQuestion.DefaultAction` and, when present, displays the proposed default in the message body and denormalizes the `ActionId` into `PendingQuestionRecord.DefaultActionId` for efficient timeout polling. **Cross-doc note:** implementation-plan.md Stage 1.2 (line 36) currently defines `AgentQuestion` **without** `DefaultAction`, stating the story requirement is satisfied connector-side via metadata. This contradicts architecture.md §3.1 (lines 167–184) which explicitly includes `DefaultAction` in the shared model table. This spec aligns with architecture.md as authoritative for shared model definitions; implementation-plan.md should reconcile by adding `DefaultAction` to `AgentQuestion`. |
...
| HC-3 | Must use the shared data model: `AgentQuestion`, `HumanAction`, `HumanDecisionEvent`. The story requires "proposed default action" handling. Per **architecture.md §3.1** (lines 167–184, authoritative for the shared model), the shared `AgentQuestion` **includes** a nullable `DefaultAction` property (`string?`) — the `ActionId` of the proposed default action from `AllowedActions`. When the question times out, the connector reads `DefaultAction` (denormalized into `PendingQuestionRecord.DefaultActionId` at send time) and, if present, resolves the full `HumanAction` from `IDistributedCache` and publishes a `HumanDecisionEvent`; if absent (`null`), it publishes a timeout event with `ActionValue = "__timeout__"`. **Cross-doc note:** implementation-plan.md Stage 1.2 (line 36) currently defines `AgentQuestion` **without** `DefaultAction`, stating the story requirement is satisfied connector-side via metadata. Stages 2.4 (line 142) and 3.5 (line 266) also describe `DefaultAction` as connector-side metadata rather than a shared model property. These implementation-plan references contradict architecture.md §3.1 (lines 167–184) which explicitly includes `DefaultAction` in the `AgentQuestion` field table. This spec aligns with architecture.md; implementation-plan.md should reconcile by adding `DefaultAction` to `AgentQuestion` as a shared property. | Story requirement + architecture.md §3.1 (authoritative for shared model) |
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued — **scoped to first-attempt, non-rate-limited sends** per architecture.md §10.4 (lines 628–634). The metric `telegram.send.latency_ms` measures elapsed time from `OutboundMessage.CreatedAt` to Telegram Bot API HTTP 200, counted only for messages that succeed on their first delivery attempt and are not waiting behind a 429 rate-limit hold. Messages that are retried (attempts > 1) or rate-limited are tracked separately via `telegram.send.retry_latency_ms` and `telegram.send.rate_limited_wait_ms`. **This narrowing is a deliberate acceptance-test interpretation** of the broader story requirement ("P95 send latency under 2 seconds after event is queued") and **requires product/operator agreement** before acceptance tests are finalized, since a stricter reading of the story would include all sends regardless of attempt number or rate-limit holds (architecture.md §10.4 line 634). | Story requirement + architecture.md §10.4 (metric definition) |
...
|----|------|-----------|--------|------------|
| R-1 | **Telegram Bot API rate limits** — `sendMessage` is limited to ~30 msg/s globally per bot and ~20 msg/min per individual chat (per architecture.md §10.4). Under 100-agent burst, we may hit 429 errors. The P95 < 2 s target (HC-4) is scoped to first-attempt, non-rate-limited sends; messages delayed by 429 back-off are excluded from the P95 metric and tracked separately. With priority queuing, the ~50 Critical/High messages in a burst are processed in the first ~2 seconds; Normal/Low messages queue-delayed beyond 2 seconds are not failures under this metric scoping. This interpretation requires operator agreement (see HC-4). | High | High | Implement a dual token-bucket rate limiter in the outbound queue: one global bucket (30 msg/s) and per-chat buckets (20 msg/min). Back-off on 429 with `retry_after`. Spread alerts across operator chats when possible. |
...
| D-4 | `/handoff` command semantics | (a) Transfer human oversight of a task to another operator, (b) Reassign the question to another operator, (c) Transfer task to a different agent team | **(a) Full transfer of human oversight — Decided per architecture.md.** Architecture.md §5.5 (lines 483–496) specifies full oversight transfer (Decided): the handler accepts `/handoff TASK-ID @operator-alias` syntax and performs: (1) validates syntax (two arguments: task ID and operator alias); (2) validates that the specified task exists and the sending operator currently has oversight; (3) validates that the target operator (`@operator-alias`) is registered via `OperatorRegistry`; (4) transfers oversight by creating/updating a `TaskOversight` record (architecture.md §5.5 line 494) mapping the task to the target operator; (5) notifies both operators — the sender receives confirmation, the target receives a handoff notification with task context; (6) persists an audit record with handoff details (task ID, source operator, target operator, timestamp, `CorrelationId`); (7) returns error for invalid task ID, unregistered target operator, or missing arguments with usage help. E2e-scenarios.md (lines 360–376) is aligned: it tests full oversight transfer with task validation, operator resolution, `TaskOversight` record creation, and bidirectional notification. **Cross-doc contradiction:** implementation-plan.md Stage 3.2 (line 211) currently implements a **V1 stub** that validates syntax only, returns a stub acknowledgement ("🔜 Handoff for TASK-ID to @operator-alias noted — full transfer will be implemented in a future iteration"), persists an audit record, but does NOT validate task existence, resolve the target operator via `IOperatorRegistry`, mutate any `OperatorBinding` or `TaskOversight` record, or notify the target operator. Architecture.md §5.5 (line 496) explicitly notes this: "Implementation-plan.md Stage 3.2 currently implements a V1 stub… Implementation-plan.md should reconcile by upgrading `HandoffCommandHandler` to perform full transfer as specified here." **This spec aligns with architecture.md §5.5 and e2e-scenarios.md (authoritative for architectural decisions and acceptance tests); implementation-plan.md should reconcile by upgrading the V1 stub to full transfer.** | Decided per architecture.md §5.5 |
...
|------|-----------|-----------------|----------------------|-----------------|------------|
| `/handoff` semantics (D-4) | Decided — full oversight transfer per architecture.md §5.5, with validation, notification, and audit | §5.5 (lines 483–496): "Full oversight transfer (Decided)" — specifies full transfer with validation, `TaskOversight` record creation, notification, and audit. Explicitly notes implementation-plan.md is the outlier (line 496). **Aligned** with this spec. | Stage 3.2 (line 211): implements `HandoffCommandHandler` as a **V1 stub** — validates syntax only, returns stub ack, persists audit record, but does NOT validate task existence, resolve the target operator, mutate any records, or notify the target operator. **Not aligned** with this spec or architecture.md. | Lines 360–376: "Scenario: /handoff transfers oversight to another operator" — tests full oversight transfer with task validation, operator resolution via `IOperatorRegistry`, `TaskOversight` record creation, bidirectional notification, and audit. **Aligned** with this spec and architecture.md. | **This spec, architecture.md, and e2e-scenarios.md are aligned on full oversight transfer.** Implementation-plan.md is the outlier: it implements a V1 stub. **Implementation-plan.md should reconcile** by upgrading `HandoffCommandHandler` from V1 stub to full transfer per architecture.md §5.5. |
...
| P95 latency scoping (HC-4) | < 2 s, scoped to first-attempt, non-rate-limited sends; requires operator agreement | §10.4 (lines 628–634): narrows to first-attempt, non-rate-limited sends; flags operator agreement requirement. **Aligned** with this spec. | Not specified | Burst test asserts P95 < 2 s for first-attempt successes | **Aligned** across all docs. The scoping is a deliberate acceptance-test interpretation that requires product/operator confirmation. |
...
| DefaultAction model (HC-3) | Shared `AgentQuestion` **includes** nullable `DefaultAction` (`string?`) per architecture.md §3.1. Telegram connector reads it at render time and denormalizes into `PendingQuestionRecord.DefaultActionId`. | §3.1 (lines 167–184): explicitly includes `DefaultAction` as a first-class shared field in the `AgentQuestion` field table (line 180). Describes the full default-action flow (line 184). **Aligned** with this spec. | Stage 1.2 (line 36): defines `AgentQuestion` **without** `DefaultAction`, stating the story requirement is satisfied connector-side via metadata. Stage 2.4 (line 142) says "determine the proposed default action from agent/command context metadata (not from an `AgentQuestion` property)". Stage 3.5 (line 266) says "the shared `AgentQuestion` model does not carry this property." These **contradict** architecture.md §3.1 and this spec (implementation-plan.md incorrectly cites architecture.md §3.1 as support for excluding `DefaultAction`). | Timeout/default-action flow uses `PendingQuestionRecord.DefaultActionId` — **functionally aligned** with this spec's timeout flow regardless of where `DefaultAction` originates. | **This spec aligns with architecture.md §3.1** (authoritative for shared model definitions). Implementation-plan.md is the outlier: it excludes `DefaultAction` from `AgentQuestion` and incorrectly cites architecture.md §3.1 as the basis for this exclusion, when architecture.md §3.1 actually includes it. **Implementation-plan.md should reconcile** by adding `DefaultAction` to `AgentQuestion` as a shared property per architecture.md §3.1 line 180. E2e-scenarios.md is functionally aligned on the timeout/default-action flow. |
