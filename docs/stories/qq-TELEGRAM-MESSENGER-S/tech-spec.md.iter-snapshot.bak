...
| HC-4 | P95 outbound send latency **< 2 seconds** after event is queued. **Cross-document alignment — canonical contract per architecture.md §10.4 (lines 678–690).** The authoritative metric definitions are: **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from the **dequeue instant** (when a worker picks the message from the outbound queue) to Telegram Bot API returning HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** waiting behind a 429 rate-limit hold. By measuring from dequeue (not enqueue), the metric isolates the system's intrinsic per-message processing latency independently of queue depth. The **P95 ≤ 2 s** acceptance criterion applies to this metric. **`telegram.send.latency_ms`** (primary, all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram Bot API returning HTTP 200, measured for **all** messages regardless of attempt number, rate-limit holds, or queue dwell time — the all-inclusive monitoring metric for capacity planning and operational dashboards. **`telegram.send.queue_dwell_ms`** (diagnostic) = elapsed time from enqueue to dequeue — monitors queue backlog under burst conditions. Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends) and `telegram.send.rate_limited_wait_ms` (429 backoff duration). Per operator answer to `p95-latency-scope`: the P95 < 2 s target applies to first-attempt, non-rate-limited sends only; retried/rate-limited sends are tracked separately via the all-inclusive `telegram.send.latency_ms`. Under normal operating conditions (low queue depth, no rate-limit hits), queue dwell is negligible and `first_attempt_latency_ms` closely tracks `latency_ms`. Under burst (1000+ messages), the per-message processing time remains under 2 s; queue dwell increases but does not affect the acceptance metric. **Sibling-doc divergences (see Section 10):** e2e-scenarios.md uses reversed metric names (`telegram.send.latency_ms` as acceptance gate, `telegram.send.all_attempts_latency_ms` as all-inclusive); implementation-plan.md uses the correct canonical names but measures `first_attempt_latency_ms` from enqueue rather than dequeue. Both require editorial correction to match architecture.md §10.4. | Story requirement; architecture.md §10.4 is authoritative — `telegram.send.first_attempt_latency_ms` = dequeue to HTTP 200, first-attempt, non-rate-limited (P95 ≤ 2 s acceptance gate); `telegram.send.latency_ms` = enqueue to HTTP 200, all-inclusive primary monitoring (see Section 10) |
...
| P95 latency scoping (HC-4) | **Canonical contract per architecture.md §10.4.** `telegram.send.first_attempt_latency_ms` (acceptance gate) measures from **dequeue** to HTTP 200, first-attempt, non-rate-limited; P95 ≤ 2 s applies. `telegram.send.latency_ms` (primary, all-inclusive) measures from **enqueue** to HTTP 200 for all sends. `telegram.send.queue_dwell_ms` (diagnostic) tracks enqueue to dequeue. | §10.4 (lines 678–690): **authoritative source.** Defines `telegram.send.first_attempt_latency_ms` (acceptance gate) as "elapsed time from **dequeue instant** to HTTP 200, first-attempt, non-rate-limited." Defines `telegram.send.latency_ms` (primary, all-inclusive) as "elapsed time from `OutboundMessage.CreatedAt` (enqueue) to HTTP 200, all messages." Line 690: "The 2-second P95 target applies to `telegram.send.first_attempt_latency_ms`." **This spec's HC-4 is aligned with architecture.md §10.4 on both naming and measurement point.** | **Naming aligned; measurement-point divergence.** Stage 4.1 (line 367) and Stage 7.2 (line 553) now use the correct canonical names (`telegram.send.first_attempt_latency_ms` = acceptance gate, `telegram.send.latency_ms` = all-inclusive) — matching architecture.md. However, line 367 measures `first_attempt_latency_ms` from `OutboundMessage.CreatedAt` (**enqueue**), not from the **dequeue instant** as architecture.md §10.4 specifies. This measurement-point divergence requires editorial correction: the acceptance gate metric should measure from dequeue to HTTP 200. | **Naming divergence.** E2e-scenarios.md uses **reversed metric names** compared to architecture.md: lines 264–268 define `telegram.send.latency_ms` as the first-attempt, non-rate-limited acceptance gate; lines 269–272 define `telegram.send.all_attempts_latency_ms` as the all-inclusive diagnostic. Lines 278, 280, 292 use `telegram.send.latency_ms` as the acceptance gate metric. Lines 621–622 (metrics table) repeat this convention. Footer line 683 confirms e2e-scenarios.md treats `telegram.send.latency_ms` as acceptance gate. Architecture.md §10.4 line 719 flags this: "e2e-scenarios scenario text calls `telegram.send.latency_ms` the acceptance gate — contradicting its own footer relative to architecture.md's convention." E2e-scenarios.md also measures from enqueue (not dequeue). Both the naming and measurement point require editorial correction to match architecture.md §10.4. | **Architecture.md §10.4 is authoritative.** This spec's HC-4 is aligned with architecture.md on naming (`first_attempt_latency_ms` = acceptance gate, `latency_ms` = all-inclusive) and measurement point (dequeue to HTTP 200 for acceptance gate). **Two sibling divergences remain:** (1) e2e-scenarios.md uses reversed metric names (`latency_ms` = acceptance gate, `all_attempts_latency_ms` = all-inclusive) and enqueue-based measurement; (2) implementation-plan.md uses correct names but enqueue-based measurement for the acceptance gate. Both require editorial correction before implementation. All documents agree on the **behavioral contract**: P95 ≤ 2 s applies to first-attempt, non-rate-limited sends; all-inclusive latency is tracked separately for capacity planning. |
