...
Feature: Durable outbound message queue with retry and dead-letter
  ...
  Scenario: Transient Telegram API failure triggers retry
    ...
    And telegram.send.first_attempt_latency_ms (acceptance gate) is NOT recorded for this message because it did not succeed on first attempt without rate-limiting (per architecture.md §10.4: this metric covers only first-attempt, non-rate-limited successes measured from dequeue to HTTP 200; P95 ≤ 2s acceptance criterion applies to this metric)
    And telegram.send.latency_ms (primary, all-inclusive) records elapsed time from OutboundMessage.CreatedAt (enqueue) to final HTTP 200, including the rate-limit wait (per architecture.md §10.4: all-inclusive metric covering ALL messages regardless of attempt number or rate-limit holds — used for capacity planning)
  ...
  Scenario: P95 send latency under 2 seconds (normal load)
    ...
    # = elapsed time from dequeue instant to Telegram API HTTP 200, measured ONLY for
    # messages that succeed on first attempt without rate-limit waits — the P95 ≤ 2s
    # acceptance criterion applies to this metric (per operator answer to p95-metric-scope:
    # first-attempt, non-rate-limited only).
    # telegram.send.latency_ms (primary, all-inclusive) = elapsed time from
    ...
    And telegram.send.first_attempt_latency_ms (acceptance gate) is emitted for every send (per architecture.md §10.4: dequeue to HTTP 200, first-attempt, non-rate-limited sends only)
    And telegram.send.latency_ms (primary, all-inclusive) is also emitted for all 100 messages (per architecture.md §10.4: enqueue to HTTP 200, all messages regardless of attempt number or rate-limit holds)
    ...
    And P95 telegram.send.first_attempt_latency_ms across ALL 100 messages (all severities) is under 2 seconds (per operator answer to p95-metric-scope: the P95 ≤ 2s acceptance criterion applies to first-attempt, non-rate-limited sends only, measured from dequeue to HTTP 200)
    And P99 telegram.send.first_attempt_latency_ms across ALL 100 messages is under 3 seconds
    And under these test conditions (no retries, no rate-limiting, low queue depth), telegram.send.queue_dwell_ms is negligible and telegram.send.latency_ms closely tracks telegram.send.first_attempt_latency_ms plus queue dwell
  ...
  Scenario: Burst of 1000+ agent alerts without message loss
    ...
    And telegram.send.first_attempt_latency_ms (acceptance gate) is emitted for messages that succeed on first attempt without rate-limiting (per architecture.md §10.4: dequeue to HTTP 200, first-attempt, non-rate-limited sends only)
    And telegram.send.latency_ms (primary, all-inclusive) is emitted for all 1000 messages regardless of attempt number or rate-limit holds (per architecture.md §10.4: enqueue to HTTP 200 — capacity planning)
    ...
    And P95 telegram.send.first_attempt_latency_ms across ALL severities (Critical, High, Normal, Low) is under 2 seconds (per architecture.md §10.4: the acceptance metric measures dequeue-to-HTTP-200 per-message processing time, which holds for all severities regardless of queue backlog)
    And queue dwell time (telegram.send.queue_dwell_ms) increases under burst as expected but does not affect the acceptance metric (per architecture.md §10.4: dequeue-based measurement excludes queue wait)
...
Feature: Observability integration
  ...
  Scenario: Metrics are emitted for queue depth and latency
    ...
    Then the following metrics are emitted via OpenTelemetry:
      | Metric                                   | Type      | Source                                                      |
      | telegram.send.first_attempt_latency_ms   | histogram | architecture.md §10.4: acceptance gate (dequeue to HTTP 200, first-attempt, non-rate-limited — P95 ≤ 2s) |
      | telegram.send.latency_ms                 | histogram | architecture.md §10.4: primary all-inclusive (enqueue to HTTP 200, all sends regardless of attempt or rate-limit — capacity planning) |
...
_Document generated for story qq:TELEGRAM-MESSENGER-S, iteration 24._
...
_Metric naming: Per architecture.md §10.4 (canonical source), the latency metric contract is: **`telegram.send.first_attempt_latency_ms`** (acceptance gate) = elapsed time from **dequeue instant** to Telegram Bot API HTTP 200, measured **only** for messages that succeed on their **first delivery attempt** and are **not** waiting behind a 429 rate-limit hold — the **P95 ≤ 2s** acceptance criterion applies to this metric. **`telegram.send.latency_ms`** (primary, all-inclusive) = elapsed time from `OutboundMessage.CreatedAt` (enqueue) to HTTP 200, measured for **all** messages regardless of attempt number or rate-limit holds — for capacity planning and operational dashboards. **`telegram.send.queue_dwell_ms`** (diagnostic) = elapsed time from enqueue to dequeue — queue backlog monitoring. Additional diagnostics: `telegram.send.retry_latency_ms` (retried sends) and `telegram.send.rate_limited_wait_ms` (429 backoff duration). Backpressure dead-letter counter: `telegram.messages.backpressure_dlq` (per architecture.md §10.4 — canonical name). Queue depth and error counters: `telegram.queue.depth`, `telegram.dlq.depth`, `telegram.errors` (per implementation-plan.md Stage 6.1)._
_P95 metric scope: Per operator answer to p95-metric-scope, the P95 ≤ 2s acceptance criterion applies to `telegram.send.first_attempt_latency_ms` — first-attempt, non-rate-limited sends only, measured from **dequeue** to HTTP 200 (per architecture.md §10.4). By measuring from dequeue (not enqueue), the metric captures per-message processing latency independently of queue depth. Under normal operating conditions (no rate-limit hits, low queue depth), queue dwell is negligible and the acceptance metric closely tracks end-to-end latency for all severities. Under burst (1000+ messages), queue dwell increases (tracked via `telegram.send.queue_dwell_ms`) but does not affect the acceptance metric because it measures from dequeue. The P95 ≤ 2s target holds for **all severities** (per architecture.md §10.4) because each message's dequeue-to-200 processing time is well under 2s regardless of queue backlog. The all-inclusive `telegram.send.latency_ms` (enqueue to 200) will show higher values for burst-delayed messages — this is expected and tracked for capacity planning._
