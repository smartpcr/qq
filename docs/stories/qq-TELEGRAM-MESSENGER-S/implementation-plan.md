...
- [ ] Create `AgentQuestion` record in Abstractions with properties: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions` (list of `HumanAction`), `ExpiresAt`, `CorrelationId` — per architecture.md §3.1 lines 166-182, the shared `AgentQuestion` model does **not** include a `DefaultAction` property; the story requirement for "proposed default action" is satisfied connector-side: each messenger connector determines the default action from agent/command context metadata and stores it in connector-specific records (e.g., `PendingQuestionRecord.DefaultActionId` for Telegram); this keeps the shared model stable across all messenger connectors
...
- [ ] Implement `SendQuestionAsync` to render `AgentQuestion` as a rich Telegram message: include `Title`, `Body` (full context), `Severity` badge, `ExpiresAt` timeout countdown; render `AllowedActions` as Telegram `InlineKeyboardMarkup` buttons with callback data encoding `QuestionId:ActionId`; determine the proposed default action from agent/command context metadata (not from an `AgentQuestion` property — per architecture.md §3.1 lines 166-182, `DefaultAction` is connector-side metadata) and, when present, display its label in the message body (e.g., "Default action if no response: Approve") and store the `ActionId` into `PendingQuestionRecord.DefaultActionId` (Stage 3.5) for efficient timeout polling
...
- [ ] Implement `HandoffCommandHandler` as a **V1 stub** (per architecture.md §5.5 line 481 "V1 stub; full transfer deferred" and e2e-scenarios.md lines 359-372): (1) validates `/handoff TASK-ID @operator-alias` syntax (two arguments: task ID and operator alias); if invalid (zero or one argument), returns usage help "Usage: `/handoff TASK-ID @operator-alias`"; (2) does **NOT** validate task existence, resolve the target operator via `IOperatorRegistry`, mutate `OperatorBinding`, or notify the target operator — full oversight transfer requires a `TaskOversight` entity not yet defined (architecture.md §5.5 line 491); (3) replies with a stub acknowledgement: "🔜 Handoff for TASK-ID to @operator-alias noted — full transfer will be implemented in a future iteration"; (4) persists an audit record with task ID, target alias, requesting operator, timestamp, and `CorrelationId` for traceability
...
- [ ] Scenario: Handoff stub acknowledged — Given an authorized user "operator-1" sends `/handoff TASK-099 @operator-2`, When `HandoffCommandHandler` processes it, Then the bot replies with "🔜 Handoff for TASK-099 to @operator-2 noted — full transfer will be implemented in a future iteration" and an audit record is persisted with task ID "TASK-099", target alias "@operator-2", requesting operator "operator-1", timestamp, and CorrelationId
- [ ] Scenario: Handoff stub does not mutate state — Given an authorized user sends `/handoff TASK-099 @operator-2`, When `HandoffCommandHandler` processes it, Then no `OperatorBinding` records are created, updated, or deleted, no notification is sent to operator-2, and no task existence validation occurs
...
- [ ] Create `PendingQuestionRecord` entity with fields: `QuestionId`, `AgentQuestion` (serialized), `TelegramChatId`, `TelegramMessageId`, `StoredAt`, `ExpiresAt`, `DefaultActionId` (nullable `string` — determined by the Telegram connector from agent/command context metadata at question-send time, per architecture.md §3.1 lines 166-182; the shared `AgentQuestion` model does not carry this property; stored here so that `QuestionTimeoutService` can poll for expired questions and resolve the default via `IDistributedCache` without re-fetching the full context), `Status` (enum: `Pending`, `Answered`, `AwaitingComment`, `TimedOut`), `CorrelationId`
