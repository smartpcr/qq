...
- [ ] Create `OutboundQueueProcessor` as a `BackgroundService` with configurable concurrency (`OutboundQueue:ProcessorConcurrency`, default 10 workers per architecture.md §10.4); each worker independently dequeues the highest-severity pending message (recording the **dequeue instant** as `DequeuedAt`), transitions to `Sending`, sends via `TelegramMessageSender`, and transitions to `Sent` on success or `Failed` on error; on every successful send that succeeds on its **first delivery attempt** without rate-limit waits, emit `telegram.send.first_attempt_latency_ms` (acceptance gate metric per architecture.md §10.4: elapsed time from **dequeue instant** (`DequeuedAt`) to Telegram Bot API HTTP 200 — isolates intrinsic per-message processing latency independent of queue dwell; the **P95 ≤ 2 s** acceptance criterion applies to this metric, per operator answer to `p95-latency-scope`); on every successful send regardless of attempt number or rate-limit holds, emit `telegram.send.latency_ms` (primary, all-inclusive metric per architecture.md §10.4: measures elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to final HTTP 200 including queue dwell, retries, and rate-limit waits — used for capacity planning and operational dashboards); on every dequeue, emit `telegram.send.queue_dwell_ms` (diagnostic metric per architecture.md §10.4: elapsed time from `OutboundMessage.CreatedAt` (enqueue) to `DequeuedAt` (dequeue) — monitors queue backlog under burst conditions); under burst conditions (100+ agents), the 10 concurrent workers combined with severity-priority dequeue ensure Critical/High first-attempt sends meet the P95 ≤ 2s target because per-message processing time (dequeue → format → HTTP round-trip) is well under 2s regardless of queue depth
...
- [ ] Scenario: P95 latency measured accurately — Given the PERF001 test, When 100 messages are sent through the outbound pipeline (all succeeding on first attempt without rate-limiting), Then `telegram.send.first_attempt_latency_ms` (acceptance gate per architecture.md §10.4: measured from **dequeue instant** to HTTP 200, first-attempt, non-rate-limited sends only) is emitted for every send with P95 ≤ 2000ms; `telegram.send.latency_ms` (primary, all-inclusive per architecture.md §10.4: enqueue-to-HTTP-200) is also emitted; `telegram.send.queue_dwell_ms` (diagnostic per architecture.md §10.4: enqueue-to-dequeue) is also emitted; under low-queue-depth test conditions, queue dwell is negligible so `first_attempt_latency_ms` (dequeue-to-200) closely tracks `latency_ms` (enqueue-to-200)
