...
- [ ] Implement the deduplication stage: on entry, call `IDeduplicationService.IsProcessedAsync(event.EventId)` — if already processed, short-circuit with `PipelineResult { Handled = true }` and skip all downstream stages; otherwise call `MarkProcessedAsync` and continue
...
- [ ] **Degraded burst behavior (documented, not acceptance-gated):** Under sustained burst (100+ agents, 1000+ messages), priority queuing ensures Critical/High messages are dequeued first with minimal queue dwell, keeping their enqueue-to-200 `first_attempt_latency_ms` within the 2-second P95 window. Normal/Low severity messages may exceed the 2-second P95 under sustained burst due to accumulated queue dwell — this is expected degraded behavior per architecture.md §10.4, not a failure of the acceptance gate. The `all_attempts_latency_ms` and `queue_dwell_ms` diagnostics provide visibility into burst-induced latency
...
> **Cross-document metric alignment note.** This plan follows architecture.md §10.4 (lines 697–709) as the canonical source for all metric names and measurement points. Architecture.md defines: `telegram.send.first_attempt_latency_ms` = **enqueue** (`OutboundMessage.CreatedAt`) to HTTP 200 (acceptance gate, P95 ≤ 2s); `telegram.send.all_attempts_latency_ms` = enqueue to HTTP 200 all-inclusive (capacity planning); `telegram.send.queue_dwell_ms` = enqueue to dequeue (diagnostic). **Known sibling-document divergences:** tech-spec.md HC-4 and Section 10 define `first_attempt_latency_ms` as **dequeue**-to-HTTP-200 and name the all-inclusive metric `telegram.send.latency_ms`; e2e-scenarios.md similarly uses dequeue-based measurement and `latency_ms` naming in several scenarios while its footer (line 716) claims alignment with architecture.md. These divergences are editorial — all four documents agree on the substantive contract: (1) `first_attempt_latency_ms` is the acceptance gate, (2) P95 ≤ 2s applies to first-attempt non-rate-limited sends only, (3) a separate all-inclusive metric captures all sends for capacity planning. **Reconciliation recommendation:** tech-spec.md and e2e-scenarios.md should be updated to use the architecture.md canonical definitions (enqueue-based measurement, `all_attempts_latency_ms` naming) before implementation begins.
