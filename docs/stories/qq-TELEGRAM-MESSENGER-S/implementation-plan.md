...
- [ ] Create `IUserAuthorizationService` interface in Abstractions with method: `AuthorizeAsync(string externalUserId, string chatId, string? commandName, CancellationToken)` returning `AuthorizationResult` — accepts an optional `commandName` parameter so the authorization service can distinguish `/start` onboarding (Tier 1: allowlist check, creates `OperatorBinding` if user is in `AllowedUserIds`) from all other commands (Tier 2: requires an existing `OperatorBinding` via `IOperatorRegistry.IsAuthorizedAsync`); when `commandName` is `"start"`, the service performs Tier 1 allowlist-based authorization; for all other commands, the service performs Tier 2 binding-based authorization
- [ ] Create `AuthorizationResult` record with properties: `IsAuthorized`, `OperatorId`, `TenantId`, `WorkspaceId`, `DenialReason`; create `AuthorizedOperator` record in Abstractions with properties: `OperatorId`, `TenantId`, `WorkspaceId`, `Roles` (list of string), `TelegramUserId`, `TelegramChatId` — represents a resolved authorized identity passed to command handlers
...
- [ ] Compose the pipeline as a sequential chain: deduplication check (via `IDeduplicationService`) → command parsing (via `ICommandParser`, producing `ParsedCommand`) → allowlist gate (via `IUserAuthorizationService.AuthorizeAsync(userId, chatId, parsedCommand.CommandName, ct)`, producing `AuthorizedOperator`; the `commandName` parameter enables the auth service to apply Tier 1 allowlist-based `/start` onboarding versus Tier 2 binding-based authorization for all other commands) → routing by `EventType`: `Command` events pass `ParsedCommand` and `AuthorizedOperator` to `ICommandRouter.RouteAsync`, `CallbackResponse` events go to `ICallbackHandler`, `TextReply` events check for pending `RequiresComment` prompts (via `IPendingQuestionStore`, defined in Stage 1.3 Abstractions) before falling through to default handling
...
- [ ] Implement two-tier authorization per architecture.md §7.1: Tier 1 (onboarding) checks `TelegramOptions.AllowedUserIds` via `IOptions<TelegramOptions>` when `commandName == "start"` — if the user's Telegram ID is not in the list, `/start` is rejected and no `OperatorBinding` is created; Tier 2 (runtime) applies when `commandName` is anything other than `"start"` — checks `OperatorBinding` records via `IOperatorRegistry.IsAuthorizedAsync(userId, chatId)` on every inbound command — if no matching binding exists for the (userId, chatId) pair, the command is rejected; the `commandName` parameter (added to `IUserAuthorizationService.AuthorizeAsync` in Stage 1.3) enables this distinction without requiring separate pipeline branches
...
- [ ] Write test `PERF001_P95SendLatencyUnder2Seconds`: enqueue 100 outbound messages, configure WireMock to respond with 200 in under 50ms (ensuring all succeed on first attempt without rate-limiting), measure the elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram API HTTP 200 (send-completion) for each message; assert that `telegram.send.first_attempt_latency_ms` (acceptance gate metric per architecture.md §10.4: first-attempt, non-rate-limited sends — the P95 ≤ 2s acceptance gate, per operator answer to `p95-latency-scope`) is emitted for every send; assert that `telegram.send.latency_ms` (primary, all-inclusive metric per architecture.md §10.4) is also emitted; the P95 ≤ 2s acceptance gate assertion uses `telegram.send.first_attempt_latency_ms` with threshold 2000ms; under these test conditions (no retries, no rate-limiting), both metrics have identical values
...
- [ ] Scenario: P95 latency measured accurately — Given the PERF001 test, When 100 messages are sent through the outbound pipeline (all succeeding on first attempt without rate-limiting), Then `telegram.send.first_attempt_latency_ms` (acceptance gate, first-attempt/non-rate-limited per architecture.md §10.4) is emitted for every send, and `telegram.send.latency_ms` (primary, all-inclusive per architecture.md §10.4) is also emitted; the P95 ≤ 2s acceptance gate assertion uses `telegram.send.first_attempt_latency_ms` with threshold 2000ms; under these test conditions both metrics have identical values since all sends are first-attempt and non-rate-limited
