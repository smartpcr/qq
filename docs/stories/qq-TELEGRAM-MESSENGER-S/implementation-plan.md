...
- [ ] Route `AgentStatusUpdateEvent` events: convert to a `MessengerMessage` with `Severity=Normal`, resolve the oversighting operator via `ITaskOversightRepository` (interface defined in Stage 1.3 Core, concrete implementation provided later by Stage 3.2 Persistence; at this stage a stub/no-op implementation is registered in DI that returns null, causing status updates to broadcast to all active operators until oversight records exist), and call `IMessengerConnector.SendMessageAsync`
...
- [ ] Create `OutboundQueueProcessor` as a `BackgroundService` with configurable concurrency (`OutboundQueue:ProcessorConcurrency`, default 10 workers per architecture.md §10.4); each worker independently dequeues the highest-severity pending message, transitions to `Sending`, sends via `TelegramMessageSender`, and transitions to `Sent` on success or `Failed` on error; on every successful send, emit `telegram.send.latency_ms` (primary, all-inclusive metric per e2e-scenarios.md line 645 and architecture.md §10.4: measures elapsed time from `OutboundMessage.CreatedAt` to Telegram API HTTP 200 for **all** messages regardless of attempt number or rate-limit holds); additionally, for sends that succeed on first attempt without rate-limit waits, also emit `telegram.send.first_attempt_latency_ms` (diagnostic metric for capacity planning and acceptance gate — per operator answer to `p95-latency-scope`, the P95 ≤ 2s acceptance criterion applies to this first-attempt, non-rate-limited metric only); under burst conditions (100+ agents), the 10 concurrent workers combined with severity-priority dequeue ensure Critical/High first-attempt sends meet the P95 ≤ 2s target
...
- [ ] Write test `PERF001_P95SendLatencyUnder2Seconds`: enqueue 100 outbound messages, configure WireMock to respond with 200 in under 50ms (ensuring all succeed on first attempt without rate-limiting), measure the elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram API HTTP 200 (send-completion) for each message; assert that `telegram.send.latency_ms` (the primary all-inclusive metric per e2e-scenarios.md line 645 and architecture.md §10.4) is emitted for every send; assert that `telegram.send.first_attempt_latency_ms` (diagnostic metric for first-attempt, non-rate-limited successes) is also emitted; the P95 ≤ 2s acceptance gate applies to `telegram.send.first_attempt_latency_ms` (per operator answer to `p95-latency-scope`: first-attempt, non-rate-limited only); under these test conditions (no retries, no rate-limiting), both metrics have identical values
