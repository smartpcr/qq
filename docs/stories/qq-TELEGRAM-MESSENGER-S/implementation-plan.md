...
- [ ] Create `AgentQuestion` record in Abstractions with properties: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions` (list of `HumanAction`), `ExpiresAt`, `CorrelationId` — the shared `AgentQuestion` does NOT include a `DefaultAction` property (per architecture.md §3.1 and tech-spec HC-3); the "proposed default action" requirement is satisfied Telegram-side via `PendingQuestionRecord.DefaultActionId`, keeping the shared model stable across all messenger connectors
...
- [ ] Implement `SendQuestionAsync` to render `AgentQuestion` as a rich Telegram message: include `Title`, `Body` (full context), `Severity` badge, `ExpiresAt` timeout countdown; render `AllowedActions` as Telegram `InlineKeyboardMarkup` buttons with callback data encoding `QuestionId:ActionId`; determine the proposed default action from the agent/command context at render time (the shared `AgentQuestion` does NOT carry `DefaultAction` per architecture.md §3.1) and, when a default is determined, display its label in the message body (e.g., "Default action if no response: Approve") and store the `ActionId` in `PendingQuestionRecord.DefaultActionId` (Stage 3.5) for timeout handling
...
- [ ] Implement `HandoffCommandHandler` as a **V1 stub** (per tech-spec D-4 cross-doc alignment: "V1 stub — syntax validation and stub ack only; does not perform any transfer, reassignment, or `OperatorRegistry` mutation"): validates `/handoff TASK-ID @operator-alias` syntax (two arguments: task ID and operator alias); if syntax is invalid, returns usage help "Usage: `/handoff TASK-ID @operator-alias`"; if syntax is valid, returns a stub acknowledgement message ("🔜 Handoff for TASK-ID to @operator-alias noted — full transfer will be implemented in a future iteration"); persists an audit record with the handoff request details (task ID, requested target alias, requesting operator, timestamp, `CorrelationId`) for future replay; does NOT validate task existence, does NOT resolve the target operator via `IOperatorRegistry`, does NOT update any `OperatorBinding`, and does NOT notify the target operator
...
- [ ] Scenario: Handoff stub ack — Given an authorized user sends `/handoff TASK-099 @operator-2`, When `HandoffCommandHandler` processes it, Then the bot responds with a stub acknowledgement ("🔜 Handoff for TASK-099 to @operator-2 noted — full transfer will be implemented in a future iteration") and an audit record is persisted with the request details
- [ ] Scenario: Handoff stub does not mutate registry — Given an authorized user sends `/handoff TASK-099 @operator-2`, When `HandoffCommandHandler` processes it, Then no `OperatorBinding` records are created, updated, or deleted, and no notification is sent to @operator-2
...
- [ ] Create `PendingQuestionRecord` entity with fields: `QuestionId`, `AgentQuestion` (serialized), `TelegramChatId`, `TelegramMessageId`, `StoredAt`, `ExpiresAt`, `DefaultActionId` (nullable `string` — set by the Telegram connector at question-send time based on the agent/command context's proposed default action, NOT copied from `AgentQuestion` which does not carry this property per architecture.md §3.1; enables `QuestionTimeoutService` to poll for expired questions and resolve the default via `IDistributedCache` without re-fetching the full `AgentQuestion`), `Status` (enum: `Pending`, `Answered`, `AwaitingComment`, `TimedOut`), `CorrelationId`
