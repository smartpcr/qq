...
- [ ] Create `InboundUpdate` record in Abstractions with fields: `UpdateId` (long, PK), `RawPayload` (string), `ReceivedAt` (DateTimeOffset), `ProcessedAt` (DateTimeOffset?), `IdempotencyStatus` (enum: `Received`, `Processing`, `Completed`, `Failed`) — defined here so `IInboundUpdateStore` (Stage 2.4) can reference it in Abstractions without depending on the Persistence project; EF Core entity configuration is also provided by Stage 2.4
...
- [ ] Compose the pipeline as a sequential chain: deduplication check (via `IDeduplicationService`) → command parsing (via `ICommandParser`, producing `ParsedCommand`) → authorization gate (via `IUserAuthorizationService.AuthorizeAsync(userId, chatId, parsedCommand.CommandName, ct)`, returning `AuthorizationResult` with `Bindings` list; the `commandName` parameter enables the auth service to apply Tier 1 allowlist-based `/start` onboarding versus Tier 2 binding-based authorization for all other commands; if `Bindings.Count == 0`, reject; if `Bindings.Count == 1`, construct `AuthorizedOperator` directly; if `Bindings.Count > 1`, present workspace disambiguation via inline keyboard per e2e-scenarios.md multi-workspace flow) → role enforcement (check `AuthorizedOperator.Roles` against command role requirements per architecture.md §9) → routing by `EventType`: `Command` events pass `ParsedCommand` and `AuthorizedOperator` to `ICommandRouter.RouteAsync`, `CallbackResponse` events go to `ICallbackHandler`, `TextReply` events check for pending `RequiresComment` prompts (via `IPendingQuestionStore`, defined in Stage 1.3 Abstractions) before falling through to default handling
...
- [ ] Create `IInboundUpdateStore` interface in Abstractions with methods: `PersistAsync(InboundUpdate, CancellationToken)` returning `bool` (false if duplicate), `MarkProcessingAsync(long updateId, CancellationToken)`, `MarkCompletedAsync(long updateId, CancellationToken)`, `MarkFailedAsync(long updateId, CancellationToken)`, `GetUnprocessedAsync(CancellationToken)` returning `IReadOnlyList<InboundUpdate>` (records with `IdempotencyStatus = Received` or `Processing`) — `InboundUpdate` is defined in Stage 1.2 Abstractions, so this interface has no cross-project dependency
...
- [ ] Register a stub/no-op `IOperatorRegistry` implementation in DI (`StubOperatorRegistry`) that returns a configurable set of hardcoded `OperatorBinding` entries for development and testing; this stub is replaced by the concrete `PersistentOperatorRegistry` in Stage 3.4; the stub enables this stage to resolve tenant IDs and target chat IDs without depending on Phase 3 persistence
- [ ] Register a stub/no-op `ITaskOversightRepository` implementation in DI (`StubTaskOversightRepository`) that returns `null` for all `GetByTaskIdAsync` lookups; this stub is replaced by the concrete `PersistentTaskOversightRepository` in Stage 3.2; when the stub returns `null`, status updates broadcast to all active operators (via `IOperatorRegistry.GetAllBindingsAsync`) rather than routing to a specific oversighting operator
...
### Implementation Steps
- [ ] Create `OperatorRegistry` implementing `IOperatorRegistry` (defined in Stage 1.3 Core) in Core that maps Telegram `chatId`/`userId` to `OperatorBinding` records; implements all five interface methods: `GetBindingsAsync` (returns `IReadOnlyList<OperatorBinding>` — all active bindings for the user/chat pair, one per workspace, supporting multi-workspace disambiguation per architecture.md §4.3), `GetAllBindingsAsync`, `GetByAliasAsync`, `RegisterAsync`, `IsAuthorizedAsync`
...
- [ ] Implement latency metric emission in `OutboundQueueProcessor`: on every dequeue, emit `telegram.send.queue_dwell_ms` (diagnostic metric per architecture.md §10.4: elapsed time from `OutboundMessage.CreatedAt` (enqueue) to dequeue instant — monitors queue backlog under burst conditions); on every successful first-attempt send without rate-limit waits, emit `telegram.send.first_attempt_latency_ms` (acceptance gate metric per architecture.md §10.4: elapsed time from **dequeue instant** (`DequeuedAt`) to Telegram Bot API HTTP 200, first-attempt non-rate-limited sends only — the **P95 ≤ 2 s** acceptance criterion applies to this metric; under normal operating conditions queue dwell is negligible so this dequeue-to-200 metric effectively tracks the full enqueue-to-200 path); on every successful send regardless of attempt number or rate-limit holds, emit `telegram.send.latency_ms` (all-inclusive metric per architecture.md §10.4: elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to final HTTP 200 including queue dwell, retries, and rate-limit waits — the story's "after event is queued" language is fully represented by this metric, used for capacity planning and operational dashboards)
- [ ] Verify burst SLO behavior: under burst conditions (100+ agents), the 10 concurrent workers combined with severity-priority dequeue ensure Critical/High messages are dequeued first with minimal queue dwell, keeping their dequeue-to-HTTP-200 `first_attempt_latency_ms` within the 2-second P95 window; Normal/Low messages may exceed P95 under sustained burst per architecture.md §10.4
...
- [ ] Scenario: P95 latency measured accurately — Given the PERF001 test, When 100 messages are sent through the outbound pipeline (all succeeding on first attempt without rate-limiting), Then `telegram.send.first_attempt_latency_ms` (acceptance gate per architecture.md §10.4: measured from **dequeue instant** (`DequeuedAt`) to HTTP 200, first-attempt, non-rate-limited sends only) is emitted for every send with P95 ≤ 2000ms; `telegram.send.latency_ms` (all-inclusive per architecture.md §10.4: enqueue-to-HTTP-200, all sends regardless of attempt or rate-limit) is also emitted; `telegram.send.queue_dwell_ms` (diagnostic per architecture.md §10.4: enqueue-to-dequeue) is also emitted; under low-queue-depth test conditions, queue dwell is negligible so `first_attempt_latency_ms` + `queue_dwell_ms` ≈ `latency_ms` (the difference between the metrics is measurement start point and population scope)
