...
- [ ] Create `AgentQuestion` record in Abstractions with properties: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions` (list of `HumanAction`), `ExpiresAt`, `CorrelationId` — the shared model does **not** include a `DefaultAction` property; the proposed default action is carried as sidecar metadata via `ProposedDefaultActionId` in `AgentQuestionEnvelope` (see below); this follows architecture.md §3.1 (lines 167–184) which defines `AgentQuestion` without `DefaultAction` and `AgentQuestionEnvelope` with `ProposedDefaultActionId`; **cross-doc note:** e2e-scenarios.md declares this divergence UNRESOLVED (line 57: "Cross-document divergence on where DefaultAction lives is UNRESOLVED") — the e2e scenarios intentionally test the behavioral contract (proposed default is obtained, denormalized, and applied on timeout) without depending on which modeling approach is adopted (line 79: "carried on the model or envelope"); this plan adopts the envelope/sidecar approach per architecture.md §3.1 as the concrete implementation choice; the behavioral contract tested by e2e-scenarios.md is satisfied under either approach
...
- [ ] Create `OutboundQueueProcessor` as a `BackgroundService` with configurable concurrency (`OutboundQueue:ProcessorConcurrency`, default 10 workers per architecture.md §10.4); each worker independently dequeues the highest-severity pending message, transitions to `Sending`, sends via `TelegramMessageSender`, and transitions to `Sent` on success or `Failed` on error; on successful first-attempt send (no prior retries, no rate-limit wait), emit `telegram.send.latency_ms` (primary metric, per e2e-scenarios.md and tech-spec.md HC-4: first-attempt, non-rate-limited successes only — P95 ≤ 2s acceptance criterion applies to this metric); for all sends regardless of attempt number or rate-limit holds, emit `telegram.send.all_attempts_latency_ms` (diagnostic metric for capacity planning); under burst conditions (100+ agents), the 10 concurrent workers combined with severity-priority dequeue ensure Critical/High first-attempt sends meet the P95 ≤ 2s target; under extreme burst conditions, Normal/Low severity messages may queue-delay beyond 2 seconds per the severity-scoped SLO; **cross-doc note:** architecture.md §10.4 defines `telegram.send.latency_ms` as the all-inclusive metric, while e2e-scenarios.md and tech-spec.md HC-4 define it as first-attempt, non-rate-limited only; this plan follows the e2e-scenarios.md/tech-spec definition as it directly governs acceptance validation
...
- [ ] Write test `PERF001_P95SendLatencyUnder2Seconds`: enqueue 100 outbound messages, configure WireMock to respond with 200 in under 50ms (ensuring all succeed on first attempt without rate-limiting), measure the elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram API HTTP 200 (send-completion) for each message — this is `telegram.send.latency_ms`, the primary metric per e2e-scenarios.md and tech-spec.md HC-4, covering only first-attempt, non-rate-limited successes — and assert that the 95th percentile is under 2 seconds; also emit a diagnostic `telegram.send.all_attempts_latency_ms` histogram covering all messages regardless of attempt number or rate-limit holds for capacity planning
...
- [ ] Scenario: P95 latency measured accurately — Given the PERF001 test, When 100 messages are sent through the outbound pipeline (all succeeding on first attempt without rate-limiting), Then a P95 histogram is computed using `telegram.send.latency_ms` — the primary metric covering first-attempt, non-rate-limited successes per e2e-scenarios.md and tech-spec.md HC-4 — and the assertion threshold is 2000ms; a separate `telegram.send.all_attempts_latency_ms` diagnostic histogram is also emitted for all-inclusive capacity planning
