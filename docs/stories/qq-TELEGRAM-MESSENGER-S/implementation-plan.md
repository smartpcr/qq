...
- [ ] Create `AgentQuestion` record in Abstractions with properties: `QuestionId`, `AgentId`, `TaskId`, `Title`, `Body`, `Severity`, `AllowedActions` (list of `HumanAction`), `ExpiresAt`, `DefaultAction` (nullable `string` — the `HumanAction.ActionId` of the proposed default action to apply on timeout; when present, all messenger connectors display and honour the default; when `null`, the question expires with a `__timeout__` action value), `CorrelationId` — per architecture.md §3.1, `DefaultAction` is a shared first-class property on `AgentQuestion` so that all messenger connectors can display and honour the proposed default without connector-specific derivation logic
...
- [ ] Implement `SendQuestionAsync` to render `AgentQuestion` as a rich Telegram message: include `Title`, `Body` (full context), `Severity` badge, `ExpiresAt` timeout countdown; render `AllowedActions` as Telegram `InlineKeyboardMarkup` buttons with callback data encoding `QuestionId:ActionId`; read the proposed default action from `AgentQuestion.DefaultAction` (a shared first-class nullable property per architecture.md §3.1) and, when present, display its label in the message body (e.g., "Default action if no response: Approve") and copy the `ActionId` into `PendingQuestionRecord.DefaultActionId` (Stage 3.5) as a denormalized value for efficient timeout polling
...
- [ ] Implement `HandoffCommandHandler` performing **full oversight transfer** (per tech-spec D-4 Decided and e2e-scenarios lines 359–390): (1) validates `/handoff TASK-ID @operator-alias` syntax (two arguments: task ID and operator alias); if invalid, returns usage help "Usage: `/handoff TASK-ID @operator-alias`"; (2) validates that the specified task exists and the sending operator currently has oversight, via `ISwarmCommandBus`; if the task does not exist, returns an error ("❌ Task TASK-ID not found"); (3) resolves the target operator (`@operator-alias`) via `IOperatorRegistry.GetByAliasAsync` (defined in Stage 1.3); if the target is not registered, returns an error ("❌ Operator @operator-alias is not registered"); (4) transfers oversight by publishing a `SwarmCommand` with `CommandType=HandoffTransfer` containing source operator, target operator, and task ID; (5) notifies the source operator with confirmation ("✅ Oversight of TASK-ID transferred to @operator-alias") and the target operator with a handoff notification including task context; (6) persists an audit record with handoff details (task ID, source operator, target operator, timestamp, `CorrelationId`)
...
- [ ] Scenario: Handoff transfers oversight — Given an authorized user "operator-1" sends `/handoff TASK-099 @operator-2` and TASK-099 exists with operator-1 having oversight and @operator-2 is registered, When `HandoffCommandHandler` processes it, Then a `SwarmCommand` with `CommandType=HandoffTransfer` is published, operator-1 receives confirmation ("✅ Oversight of TASK-099 transferred to @operator-2"), operator-2 receives a handoff notification with task context, and an audit record is persisted
- [ ] Scenario: Handoff with invalid task ID is rejected — Given an authorized user sends `/handoff NONEXISTENT @operator-2` and task NONEXISTENT does not exist, When `HandoffCommandHandler` processes it, Then the bot responds with "❌ Task NONEXISTENT not found" and no transfer occurs
- [ ] Scenario: Handoff with unregistered target operator is rejected — Given an authorized user sends `/handoff TASK-099 @unknown-user` and @unknown-user is not registered in `IOperatorRegistry`, When `HandoffCommandHandler` processes it, Then the bot responds with "❌ Operator @unknown-user is not registered" and no transfer occurs
...
- [ ] Create `PendingQuestionRecord` entity with fields: `QuestionId`, `AgentQuestion` (serialized), `TelegramChatId`, `TelegramMessageId`, `StoredAt`, `ExpiresAt`, `DefaultActionId` (nullable `string` — denormalized copy of `AgentQuestion.DefaultAction`, stored at question-send time by `TelegramMessageSender` so that `QuestionTimeoutService` can poll for expired questions and resolve the default via `IDistributedCache` without re-fetching the full `AgentQuestion`; per architecture.md §3.1 this is a denormalization of the shared `AgentQuestion.DefaultAction` property), `Status` (enum: `Pending`, `Answered`, `AwaitingComment`, `TimedOut`), `CorrelationId`
