...
- [ ] Create `OutboundQueueProcessor` as a `BackgroundService` with configurable concurrency (`OutboundQueue:ProcessorConcurrency`, default 10 workers per architecture.md §10.4); each worker independently dequeues the highest-severity pending message, transitions to `Sending`, sends via `TelegramMessageSender`, and transitions to `Sent` on success or `Failed` on error; on every successful send that succeeds on its **first delivery attempt** without rate-limit waits, emit `telegram.send.latency_ms` (primary metric per architecture.md §10.4: first-attempt, non-rate-limited sends from `OutboundMessage.CreatedAt` to HTTP 200 — the **P95 ≤ 2 s** acceptance criterion applies to this metric, per operator answer to `p95-latency-scope`); on every successful send regardless of attempt number or rate-limit holds, emit `telegram.send.all_attempts_latency_ms` (diagnostic, all-inclusive metric per architecture.md §10.4: measures elapsed time from `OutboundMessage.CreatedAt` to final HTTP 200 including retries and rate-limit waits — used for capacity planning and operational dashboards); under burst conditions (100+ agents), the 10 concurrent workers combined with severity-priority dequeue ensure Critical/High first-attempt sends meet the P95 ≤ 2s target
...
- [ ] Write test `PERF001_P95SendLatencyUnder2Seconds`: enqueue 100 outbound messages, configure WireMock to respond with 200 in under 50ms (ensuring all succeed on first attempt without rate-limiting), measure the elapsed time from `OutboundMessage.CreatedAt` (enqueue instant) to Telegram API HTTP 200 (send-completion) for each message; assert that `telegram.send.latency_ms` (primary metric per architecture.md §10.4: first-attempt, non-rate-limited sends — the P95 ≤ 2s acceptance gate, per operator answer to `p95-latency-scope`) is emitted for every send; assert that `telegram.send.all_attempts_latency_ms` (diagnostic, all-inclusive metric per architecture.md §10.4) is also emitted; the P95 ≤ 2s acceptance gate assertion uses `telegram.send.latency_ms` with threshold 2000ms; under these test conditions (no retries, no rate-limiting), both metrics have identical values
...
- [ ] Scenario: P95 latency measured accurately — Given the PERF001 test, When 100 messages are sent through the outbound pipeline (all succeeding on first attempt without rate-limiting), Then `telegram.send.latency_ms` (primary, first-attempt/non-rate-limited per architecture.md §10.4) is emitted for every send, and `telegram.send.all_attempts_latency_ms` (diagnostic, all-inclusive per architecture.md §10.4) is also emitted; the P95 ≤ 2s acceptance gate assertion uses `telegram.send.latency_ms` with threshold 2000ms; under these test conditions both metrics have identical values since all sends are first-attempt and non-rate-limited
