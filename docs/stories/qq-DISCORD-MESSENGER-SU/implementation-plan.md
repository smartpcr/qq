...
- [ ] Create `GuildBinding` record in Abstractions with properties: `Id` (Guid), `GuildId` (ulong), `ChannelId` (ulong), `ChannelPurpose` (ChannelPurpose), `TenantId` (string), `WorkspaceId` (string), `AllowedRoleIds` (ulong[]), `CommandRestrictions` (IReadOnlyDictionary of string to ulong[]?), `RegisteredAt` (DateTimeOffset), `IsActive` (bool) -- shared model used by Core's AuthorizationResult, Discord's IGuildRegistry, and Persistence's EF entity configuration; aligns with architecture.md Section 3.1 GuildBinding definition
...
- [ ] Create `AuthorizationResult` record in Core with properties: `IsAllowed` (bool), `DenialReason` (string?), `ResolvedBinding` (GuildBinding?) -- return type for IUserAuthorizationService per architecture.md Section 4.5; uses GuildBinding from Abstractions (valid dependency: Core -> Abstractions)
...
- [ ] Configure `GuildBinding` as an EF entity in Persistence using Fluent API: map all properties from the shared Abstractions record (`Id` as Guid PK, `GuildId`, `ChannelId`, `ChannelPurpose`, `TenantId`, `WorkspaceId`, `AllowedRoleIds`, `CommandRestrictions`, `RegisteredAt`, `IsActive`), with UNIQUE(GuildId, ChannelId, WorkspaceId) and index on (GuildId, ChannelPurpose) -- per architecture.md Section 3.1; uses the shared GuildBinding type directly as the EF entity (Persistence -> Abstractions dependency)
- [ ] Create `OutboundMessage` entity in Persistence mapping all fields from the shared OutboundMessage record including `MessageId` (Guid PK), `IdempotencyKey` (string, unique index), `ChatId` (long), `Severity` (int), `Status` (enum), `SourceType` (enum), `Payload` (string), `SourceEnvelopeJson` (string?), `SourceId` (string?), `AttemptCount` (int), `MaxAttempts` (int, default 5), `NextRetryAt` (DateTimeOffset?), `PlatformMessageId` (long?), `CorrelationId` (string), `CreatedAt` (DateTimeOffset), `SentAt` (DateTimeOffset?), `ErrorDetail` (string?) -- all fields per architecture.md Section 3.2; SourceId, NextRetryAt, CreatedAt, SentAt, and ErrorDetail are required by retry logic (Stage 2.3) and Gap B recovery (Stage 5.3)
...
- [ ] Scenario: GuildBinding unique constraint -- Given a GuildBinding entity for (GuildId=1, ChannelId=2, WorkspaceId="ws-main"), When a duplicate is inserted, Then DbUpdateException is thrown
...
- [ ] Implement Discord-specific authorization logic inside the implementation class: (1) parse platformGroupId and chatId back to ulong, (2) resolve GuildBinding via IGuildRegistry.GetBindingAsync(guildId, channelId), (3) validate binding exists and IsActive=true, (4) check if CommandRestrictions has an override for the commandName -- if so use those role IDs, otherwise use AllowedRoleIds, (5) validate user has at least one required role
...
### Implementation Steps
- [ ] Implement `IGuildRegistry` in Discord project with methods: `GetBindingAsync(ulong guildId, ulong channelId, CancellationToken)` returning GuildBinding or null, `GetAlertChannelAsync(ulong guildId, CancellationToken)` returning the Alert-purpose GuildBinding, `GetWorkstreamChannelsAsync(ulong guildId, string workspaceId, CancellationToken)`, `IsAuthorizedChannelAsync(ulong guildId, ulong channelId, CancellationToken)`, `RegisterAsync(GuildBinding, CancellationToken)` -- per architecture.md Section 4.3; all return shared GuildBinding type from Abstractions
- [ ] Implement `GuildRegistry` class that loads GuildBinding entities from database on startup, caches with configurable TTL (default 5 minutes), and returns the shared GuildBinding type directly
...
- [ ] Implement configuration seeding: on first startup if no GuildBinding entity exists for configured GuildId, create bindings from DiscordOptions.GuildBindings configuration array

### Dependencies
- phase-security-and-reliability/stage-authorization-and-role-enforcement

### Test Scenarios
- [ ] Scenario: Control channel binding lookup -- Given a GuildBinding with ChannelPurpose=Control for channelId=111, When GetBindingAsync is called with guildId and channelId=111, Then the Control GuildBinding is returned
- [ ] Scenario: Alert channel routing -- Given a GuildBinding with ChannelPurpose=Alert for a guild, When GetAlertChannelAsync is called, Then the alert GuildBinding is returned with the correct ChannelId
