...
- Gateway Intent declaration: `Guilds`, `GuildMessages`, `MessageContent`, `GuildMessageReactions`. The `GuildMessageReactions` intent is included per architecture.md for tracking operator emoji reactions on agent embeds (e.g., acknowledgement signals) even though reactions are not parsed as command input (see Section 3, "Message reactions as input" exclusion).
...
| A-6 | The `MessageContent` privileged intent is enabled for the bot application in the Discord Developer Portal (single-guild deployment, toggle only -- no formal verification program required). | Without this intent, the bot cannot read message content in `GuildMessages` events. Slash commands, component interactions, and all core bot functionality operate without it, but message-content-based features (future extensibility) are unavailable. The bot does NOT refuse to start if this intent is missing -- it operates with reduced capability (see constraint in Section 5.1). |
