...
- **Durable command inbox:** Every inbound interaction is persisted as a `DiscordInteractionRecord` (including full `RawPayload`) before `DeferAsync()` acknowledges the interaction (architecture.md Section 4.2, Section 5.1, Section 8.2). `DeferAsync()` is called immediately after the durable persist, within the 3-second interaction deadline. Authorization, deduplication, and command processing then proceed asynchronously after the ACK. If the UNIQUE constraint on `InteractionId` rejects the INSERT, the interaction is a duplicate and is dropped without acknowledgement. On crash, the `InteractionRecoverySweep` background service re-processes records with `IdempotencyStatus` in `Received`, `Processing`, or `Failed` (where `AttemptCount < MaxRetries`) on startup and every 60 seconds.
...
- **Ephemeral rejections:** Unauthorized command attempts receive an ephemeral error message visible only to the unauthorized user, preventing information leakage. Because the architecture requires `DeferAsync()` before authorization (see Section 2.7), the rejection is delivered as `FollowupAsync(text, ephemeral: true)` -- a separate follow-up message that is independently ephemeral regardless of the original defer's visibility. The non-ephemeral deferred "thinking" indicator is then cleaned up via `DeleteOriginalResponseAsync()`. This satisfies both the architecture's persist-then-defer-then-auth ordering and the security requirement for private rejections (see architecture.md Section 5.1 key invariant 3).
...
|---|---|---|
| **3-second interaction response deadline** | All slash command and component interactions must be acknowledged within 3 seconds or they fail visibly for the user. | Persist `DiscordInteractionRecord` then call `DeferAsync()` immediately (architecture.md Section 8.2). Authorization and command processing proceed asynchronously after the ACK. Unauthorized users receive an ephemeral follow-up via `FollowupAsync(text, ephemeral: true)` (see Section 2.10). |
