...
| **Performance** | P95 Adaptive Card delivery < 3 seconds after queue pickup. Connector recovery < 30 seconds. Concurrency target per §7 Assumption 8. |
...
| **Teams-specific contracts** | `IConversationReferenceStore` is defined in `AgentSwarm.Messaging.Teams` (not in the shared abstractions) because conversation reference shapes are platform-specific. `implementation-plan.md` Stage 1.2 places the interface definition step in the shared connector-interface stage for convenience of ordering, but the assembly owner is `AgentSwarm.Messaging.Teams`. **Cross-doc alignment — verified:** All sibling docs use the dual identity-key model. `implementation-plan.md` §1.2 (line 33) defines the interface contract and §4.1 (line 243) implements `GetByAadObjectIdAsync` and `GetByInternalUserIdAsync` with `AadObjectId` and `InternalUserId` fields. `architecture.md` §4.2 (line 551) confirms alignment. No legacy single-key references remain. |
...
| Constraint | Source |
|------------|--------|
| C# / .NET 8+ | Epic-level mandate (story description specifies C# SDKs; .NET 8 is the project's supported LTS baseline and the team's standard runtime). |
...
| `Microsoft.Bot.Connector.Teams` (≥ 4.22) | Story description requirement ("Microsoft.Bot.Connector.Teams" listed under required C# libraries). Provides Teams-specific model types such as `TeamsChannelData`, `TeamInfo`, and `TeamsChannelAccount`. |
...
| **Concurrent users** | The story description does not specify a concurrency requirement. The assumed target of 1000+ concurrent users (see §7 Assumption 8) drives architectural decisions around rate limiting and queue throughput but is not a hard constraint sourced from the operator. |
