...
Feature: Adaptive Card Approvals
  ...
  Scenario: User sends bare approve with multiple pending questions — disambiguation card returned
    ...
    And the disambiguation card is a read-only listing — it contains NO action buttons (per implementation-plan.md §3.2: the card lists QuestionId, Title, and CreatedAt so the user can re-issue the command with an explicit questionId)
    ...
    # card per implementation-plan.md §3.2. The user must re-issue the command with an explicit
    # questionId (e.g., "approve Q-801") to resolve a specific question. The disambiguation card
    # itself has no action buttons — it is informational only. This prevents accidental resolution
...
Feature: Reliability — Retry and Recovery
  ...
  Scenario: Persistent failure exhausts retries and dead-letters
    ...
    And an audit record is persisted with Outcome "DeadLettered" (per tech-spec.md §4.3 canonical Outcome vocabulary: Success | Rejected | Failed | DeadLettered) and EventType "ProactiveNotification"
...
Feature: Teams Message Actions (Message Extensions)
  ...
  Scenario: User invokes message action to forward a message to an agent
    ...
    And MessageExtensionHandler extracts the source message context and dispatches via ICommandDispatcher.DispatchAsync (which internally delegates to CommandParser for text parsing) with the forwarded content
  ...
  Scenario: Message action uses direct submit (fetchTask false) to forward a message
    ...
    And the bot dispatches the forwarded message text via ICommandDispatcher.DispatchAsync (which internally delegates to CommandParser for text parsing)
...
| Approval event envelope | ALL approval/rejection scenarios (both Adaptive Card actions AND text-command paths) explicitly assert that the `HumanDecisionEvent` payload is wrapped in a `MessengerEvent` with `EventType = "Decision"` (the `DecisionEvent` subtype) before enqueuing via `IInboundEventPublisher.PublishAsync` | architecture.md §3.1 defines `DecisionEvent` subtype with `EventType = "Decision"` carrying `HumanDecisionEvent` payload; `implementation-plan.md` §1.1 defines `DecisionEvent` with `EventType = "Decision"` | **Resolved** — all Adaptive Card action scenarios AND all text-command approve/reject scenarios now explicitly assert the `MessengerEvent`/`Decision` envelope wrapping the `HumanDecisionEvent` payload, ensuring the orchestrator receives a consistent DecisionEvent subtype regardless of whether the decision arrived via card action or text command. |
| Card update/delete path | Update and delete scenarios use `ITeamsCardManager.UpdateCardAsync` / `ITeamsCardManager.DeleteCardAsync` as the call-site contract, with `TeamsMessengerConnector` internally rehydrating `TeamsCardState` and delegating to `ProactiveNotifier` for the underlying Bot Framework SDK calls | architecture.md §4.1.1 defines `ITeamsCardManager` with `UpdateCardAsync(questionId, action, ct)` and `DeleteCardAsync(questionId, ct)`; §6.5 shows the full sequence: Orchestrator → ITeamsCardManager → TeamsMessengerConnector → CardStateStore → ProactiveNotifier → Teams | **Resolved** — update/delete scenarios now test through the `ITeamsCardManager` contract surface rather than asserting direct Bot Framework SDK calls. The internal `UpdateActivityAsync`/`DeleteActivityAsync` calls are documented as implementation details within `TeamsMessengerConnector`, not as the QA contract boundary. |
...
| `message-action-ux` | **Direct submit (no task module popup).** | `architecture.md` §2.15 (`MessageExtensionHandler`) defines the message-action flow as direct submit via `composeExtension/submitAction`. The scenarios in this document already assume direct submit. This is the simpler UX and aligns with the sibling architecture. |
...
| `forged-jwt-pipeline-contract` | **Terminological, not behavioral.** "Before any application code or middleware runs" in architecture.md/tech-spec.md refers to Bot Framework `IMiddleware` and bot handler, not ASP.NET Core HTTP middleware. QA follows the 5-step pipeline: TenantValidationMiddleware → RateLimitMiddleware → CloudAdapter (JWT) → Bot Framework IMiddleware → bot handler. | `implementation-plan.md` §2.1 is the authoritative source for the concrete middleware registration order. All four sibling documents agree on the actual execution sequence. The phrase "before any application code or middleware runs" is shorthand for "before any Bot Framework middleware or bot handler runs" — ASP.NET Core HTTP middleware (TenantValidationMiddleware, RateLimitMiddleware) runs before CloudAdapter by design to enable HTTP-level status code control (403/429). No behavioral change needed; phrasing difference only. |
