...
| `EventType` | `string` | Yes | Describes the event category: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error`. The canonical set contains exactly six values. Message actions log as `CommandReceived` (not a separate event type) because message actions are a command submission mechanism, consistent with `architecture.md` §3.2 line 432, §6.4 line 943, `e2e-scenarios.md` line 773, and `implementation-plan.md` §1.3 and §5.2. |
