...
| **Teams-specific contracts** | `IConversationReferenceStore` is defined in `AgentSwarm.Messaging.Teams` (not in the shared abstractions) because conversation reference shapes are platform-specific. `implementation-plan.md` Stage 2.1 (ASP.NET Core Bot Host) is the canonical location for the interface definition step — the interface is created in the `AgentSwarm.Messaging.Teams` project which is itself created in Stage 2.1. All sibling docs use the dual identity-key model with `GetByAadObjectIdAsync` and `GetByInternalUserIdAsync` — see `implementation-plan.md` §2.1 and §4.1, `architecture.md` §4.2 `IConversationReferenceStore` interface definition. |
...
>
> **Cross-doc note on `architecture.md` §10.3:** The architecture doc's error handling table at §10.3 lists four rejection rows that mirror this matrix exactly: (1) "Invalid Bot Framework JWT (pre-application)" → HTTP 401, no application audit; (2) "Tenant not in allow-list" → HTTP 403 + `SecurityRejection` audit; (3) "Unmapped user identity" → HTTP 200 + card + `SecurityRejection` audit; (4) "Insufficient RBAC role" → HTTP 200 + card + `SecurityRejection` audit. Invalid JWT rejections are covered by infrastructure-level logging (Azure Front Door WAF logs, API gateway access logs) per operator decision `invalid-jwt-audit`. Both docs are aligned as of iteration 23.
