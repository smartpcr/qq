...
| `EventType` | `string` | Yes | Describes the event category: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `MessageActionReceived`, `Error`. Message actions log as `MessageActionReceived` (distinct from `CommandReceived`) to distinguish the message-action submission mechanism from direct command input, consistent with `architecture.md` §3.2 line 432 and §6.4 line 938. |
