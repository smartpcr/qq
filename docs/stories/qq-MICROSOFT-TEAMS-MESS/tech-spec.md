...
> The canonical **audit** `EventType` values are: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error`. The canonical set contains exactly six values. Message actions (Teams message-extension submissions) log as `CommandReceived` because a message action is a command submission mechanism, not a semantically distinct event category — the `Source` field on the domain `MessengerEvent` distinguishes message-action-originated commands (`Source = MessageAction`) from direct text input. This is consistent with `architecture.md` §3.2 line 432 (six canonical values, message actions log as `CommandReceived`), `architecture.md` §6.4 line 947 (audit entry of type `CommandReceived` for message actions), `implementation-plan.md` §1.3 line 48 (six canonical values, message actions log as `CommandReceived`), `implementation-plan.md` §3.4 line 206 (audit `EventType = "CommandReceived"` for message actions), and `implementation-plan.md` §5.2 line 291 (six canonical values, message actions log as `CommandReceived`).
>
> **Remaining sibling inconsistency:** `e2e-scenarios.md` line 773 asserts `EventType "MessageActionReceived"` and line 905 says "message actions audit as MessageActionReceived" with seven values. These contradict this spec and the architecture/implementation-plan siblings. The `e2e-scenarios.md` sibling agent should fix these lines to use `CommandReceived` and six values in its next iteration.
>
...
| `EventType` | `string` | Yes | Describes the event category: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error`. The canonical set contains exactly six values. Message actions (Teams message-extension submissions) log as `CommandReceived` because a message action is a command submission mechanism, not a semantically distinct event category. The `Source` field on the domain `MessengerEvent` distinguishes message-action-originated commands from direct text input. Consistent with `architecture.md` §3.2 line 432 (six values, message actions as `CommandReceived`), `architecture.md` §6.4 line 947 (audit `CommandReceived` for message actions), `implementation-plan.md` §1.3 line 48 and §5.2 line 291 (six values, message actions as `CommandReceived`). |
