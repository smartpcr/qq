...
Feature: Adaptive Card Approvals
  ...
  Scenario: Human approves through Adaptive Card action
    ...
    And CardActionHandler atomically transitions AgentQuestion.Status from "Open" to "Resolved" via IAgentQuestionStore.UpdateStatusAsync (architecture.md §2.6, §3.1 — first-writer-wins durable transition)
...
Feature: Adaptive Card — Incident Summary and Release Gates
  ...
  Scenario: Release gate approval with multiple approvers (separate questions per approver)
    ...
    And the orchestrator creates two separate AgentQuestion records:
      ...
      | Q-gate-a   | alice@contoso.com      | Open   |
      | Q-gate-b   | bob@contoso.com        | Open   |
    And each AgentQuestion follows the standard single-decision lifecycle (first-writer-wins per architecture §3.2)
