...
- [ ] Implement `Startup`/`Program.cs` registering `CloudAdapter` (from `Microsoft.Bot.Builder.Integration.AspNet.Core`) as the bot adapter with middleware pipeline in this exact order: `TelemetryMiddleware` → `TenantValidationMiddleware` → `ActivityDeduplicationMiddleware` → `RateLimitMiddleware` (per `architecture.md` §2.3). Register `IBot`, health check endpoints (`/health`, `/ready`), and OpenTelemetry tracing.
