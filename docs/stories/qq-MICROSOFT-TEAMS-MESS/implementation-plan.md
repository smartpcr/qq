...
- [ ] Define and implement `IAgentQuestionStore` interface with methods: `SaveAsync(AgentQuestion question, CancellationToken ct)`, `GetByIdAsync(string questionId, CancellationToken ct)`, `UpdateStatusAsync(string questionId, string status, CancellationToken ct)`. This store persists the full `AgentQuestion` payload (including `AllowedActions`) so that `CardActionHandler` (Stage 3.2) and `OnAdaptiveCardInvokeAsync` (Stage 2.2) can resolve the originating question by `QuestionId` and validate that the submitted `ActionValue` is in the question's `AllowedActions` list. Backed by a SQL table `AgentQuestions` with columns: `QuestionId` (PK), `AgentId`, `TaskId`, `TargetUserId`, `TargetChannelId`, `Title`, `Body`, `Severity`, `AllowedActionsJson` (serialized), `ExpiresAt`, `CorrelationId`, `Status` (Open/Resolved/Expired), `CreatedAt`, `ResolvedAt`.
