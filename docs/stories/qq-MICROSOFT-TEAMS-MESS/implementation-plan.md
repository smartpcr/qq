...
- [ ] Override `OnAdaptiveCardInvokeAsync` to process Adaptive Card `Action.Submit` invoke activities, extract `ActionId` and optional comment from `Activity.Value`, resolve the originating `AgentQuestion` via `IAgentQuestionStore.GetByIdAsync(questionId)` (store defined in Stage 3.3), validate that the submitted `ActionValue` is in the question's `AllowedActions` list, produce a `HumanDecisionEvent`, and return an `AdaptiveCardInvokeResponse`.
...
- [ ] Implement `ApproveCommandHandler` and `RejectCommandHandler` that resolve the referenced `AgentQuestion` via `IAgentQuestionStore.GetByIdAsync(questionId)` (store defined in Stage 3.3), validate the action against `AllowedActions`, and emit a `HumanDecisionEvent`.
