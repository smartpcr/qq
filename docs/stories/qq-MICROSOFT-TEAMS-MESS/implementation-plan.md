...
- [ ] Implement `TenantValidationMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that reads the `AllowedTenantIds` list from `TeamsMessagingOptions` and rejects activities from tenants not in the list by returning HTTP 403 and short-circuiting the pipeline. Log a `SecurityRejection` audit entry for blocked tenants.
...
### Implementation Steps
- [ ] Enhance `TenantValidationMiddleware` (created in Stage 2.1) with audit logging: emit a `SecurityRejection` audit entry with `Outcome = "Rejected"` and `Action = "UnauthorizedTenantRejected"` for each blocked tenant, and return an Adaptive Card explaining the tenant restriction. The middleware itself was implemented and registered in Stage 2.1; this step adds compliance-grade logging and user-facing rejection cards.
