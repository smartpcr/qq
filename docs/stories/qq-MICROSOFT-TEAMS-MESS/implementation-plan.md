...
- [ ] Define `IConversationReferenceStore` interface in project `AgentSwarm.Messaging.Teams` (assembly owner per `tech-spec.md` §2.1 and `architecture.md` §2.8 — conversation reference shapes are Teams/Bot-Framework-specific, not shared abstractions) with methods: `SaveOrUpdateAsync`, `GetAsync`, `GetAllActiveAsync`, `GetByUserIdAsync`, `GetByChannelIdAsync`, `MarkInactiveAsync`, `IsActiveAsync`, `DeleteAsync`. This interface is defined here in Stage 1.2 for ordering convenience, but the file lives in the `AgentSwarm.Messaging.Teams` project created in Stage 2.1.
...
- [ ] Define and implement `IAgentQuestionStore` interface — the concrete question-state contract that persists the full `AgentQuestion` payload including allowed actions. Methods: `SaveAsync(AgentQuestion question, CancellationToken ct)`, `GetByIdAsync(string questionId, CancellationToken ct)` returning the full `AgentQuestion` with all fields including `AllowedActions`, `UpdateStatusAsync(string questionId, string status, CancellationToken ct)`. This is the store that `CardActionHandler` (Stage 3.2), `OnAdaptiveCardInvokeAsync` (Stage 2.2), `ApproveCommandHandler`, and `RejectCommandHandler` call to resolve the originating question by `QuestionId` and validate that the submitted `ActionValue` is in the question's `AllowedActions` list. Backed by a SQL table `AgentQuestions` with columns: `QuestionId` (PK), `AgentId`, `TaskId`, `TargetUserId`, `TargetChannelId`, `Title`, `Body`, `Severity`, `AllowedActionsJson` (serialized JSON of `IReadOnlyList<HumanAction>`), `ExpiresAt`, `CorrelationId`, `Status` (Open/Resolved/Expired), `CreatedAt`, `ResolvedAt`. The `AllowedActionsJson` column is deserialized on read so callers receive the typed `AllowedActions` list.
