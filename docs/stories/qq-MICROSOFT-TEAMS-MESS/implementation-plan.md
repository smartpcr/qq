...
- [ ] Define `IIdentityResolver` interface with method `ResolveAsync(string aadObjectId)` returning an internal user identity record, and `IUserAuthorizationService` interface with method `AuthorizeAsync(string tenantId, string userId, string command)` returning an authorization result, both in `AgentSwarm.Messaging.Abstractions`. These minimal contracts allow Stage 2.2 to gate conversation reference capture on authorization without depending on the concrete RBAC implementation (Stage 5.1). Default-deny stub implementations are registered in DI at startup: `IIdentityResolver` rejects unmapped users immediately and `IUserAuthorizationService` enforces a minimal policy allowing only recognized users.
