...
- [ ] Implement `IdentityResolver : IIdentityResolver` (concrete implementation of the interface defined in Stage 1.2) that maps the Teams `Activity.From.AadObjectId` (Entra AAD object ID) to an internal user identity record; reject unmapped users with HTTP 200 + Adaptive Card explaining access denial and how to request access (per the two-tier rejection model in `tech-spec.md` §4.2). This replaces the default-deny stub registered at startup.
