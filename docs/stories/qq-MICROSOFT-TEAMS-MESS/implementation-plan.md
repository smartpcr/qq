...
- [ ] Implement `UserAuthorizationService : IUserAuthorizationService` (concrete implementation of the interface defined in Stage 1.2) with full role-scoped RBAC logic that checks permissions based on `RbacOptions` configuration, replacing the default-deny stub registered at startup.
