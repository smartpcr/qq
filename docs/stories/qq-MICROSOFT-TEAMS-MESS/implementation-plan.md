...
- [ ] Define `IConversationReferenceStore` interface in project `AgentSwarm.Messaging.Teams` (assembly owner per `tech-spec.md` §2.1 and `architecture.md` §2.8 — conversation reference shapes are Teams/Bot-Framework-specific, not shared abstractions) with methods: `SaveOrUpdateAsync`, `GetAsync`, `GetAllActiveAsync`, `GetByAadObjectIdAsync(string tenantId, string aadObjectId)` (lookup by Teams-native AAD identity key, used at install and message receipt to find/update references), `GetByInternalUserIdAsync(string tenantId, string internalUserId)` (lookup by orchestrator identity key, used for proactive routing when `AgentQuestion.TargetUserId` is set), `GetByChannelIdAsync(string tenantId, string channelId)`, `MarkInactiveAsync`, `IsActiveAsync`, `DeleteAsync`. The dual-key model (separate `AadObjectId` and `InternalUserId` fields per `architecture.md` §TeamsConversationReference lines 383-384) supports two lookup paths: Teams-side operations use `AadObjectId` (captured from `Activity.From.AadObjectId`); orchestrator-side proactive delivery uses `InternalUserId` (populated when `IIdentityResolver` first maps the AAD object ID to an internal identity). This interface is defined here in Stage 1.2 for ordering convenience, but the file lives in the `AgentSwarm.Messaging.Teams` project created in Stage 2.1.
