...
- [ ] Override `OnTeamsMembersAddedAsync` to capture and persist the conversation reference on bot installation. Note: install lifecycle events are app-level events (not user commands) and are subject to tenant validation only (via `TenantValidationMiddleware` in the bot adapter pipeline, created in Stage 2.1), NOT user-level identity resolution or authorization. This is a deliberate design distinction: install events establish the bot's ability to send proactive messages to a conversation scope, while user-level auth (via `IIdentityResolver`/`IUserAuthorizationService`) gates command execution in `OnMessageActivityAsync`. The captured install reference may be updated or superseded when an authorized user later sends a message and passes identity/auth checks.
...
- [ ] Override `OnInstallationUpdateActivityAsync` to handle Teams app install/uninstall lifecycle and log audit entries. On install, capture and persist the conversation reference (subject to tenant validation only, same as `OnTeamsMembersAddedAsync` — see note above on the tenant-only vs. user-level auth distinction for install lifecycle events). On uninstall, mark conversation references inactive rather than removing them.
