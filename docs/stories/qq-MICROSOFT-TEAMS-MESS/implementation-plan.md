...
- [ ] Define and implement `SqlAgentQuestionStore : IAgentQuestionStore` — the concrete implementation of the `IAgentQuestionStore` interface (defined in Stage 1.2) that persists the full `AgentQuestion` payload including allowed actions. Backed by a SQL table `AgentQuestions` with columns: `QuestionId` (PK), `AgentId`, `TaskId`, `TargetUserId`, `TargetChannelId`, `Title`, `Body`, `Severity`, `AllowedActionsJson` (serialized JSON of `IReadOnlyList<HumanAction>`), `ExpiresAt`, `CorrelationId`, `Status` (Open/Resolved/Expired), `CreatedAt`, `ResolvedAt`. The `AllowedActionsJson` column is deserialized on read so callers receive the typed `AllowedActions` list. This is the store that `CardActionHandler` (Stage 3.2), `OnAdaptiveCardInvokeAsync` (Stage 2.2), `ApproveCommandHandler`, and `RejectCommandHandler` call to resolve the originating question by `QuestionId` and validate that the submitted `ActionValue` is in the question's `AllowedActions` list.
