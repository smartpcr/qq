...
> **Cross-doc naming — `CommandParser` (architecture) vs `ICommandDispatcher` / `CommandDispatcher` (implementation-plan):** This architecture models the parsing concern as `CommandParser` — a focused component that converts raw text into a `ParsedCommand`. The sibling `implementation-plan.md` §1.2 line 35 defines `ICommandDispatcher` / `CommandDispatcher` which combines parsing with handler routing (resolve the matching `ICommandHandler` from DI and invoke it). These are the **same logical component at different granularities**: this architecture separates parsing from dispatch to clarify the data flow, while the implementation plan merges them into a single dispatcher class for implementation convenience. At implementation time, `CommandDispatcher` will contain a `CommandParser` internally (or inline the parsing logic) and add the handler-routing step. The `TeamsSwarmActivityHandler` key-override at §2.4 maps `OnMessageActivityAsync` → `CommandParser`; in implementation this call site becomes `ICommandDispatcher.DispatchAsync`, which parses first, then routes.
...
> **Cross-doc naming — `AdaptiveCardRenderer` / `IAdaptiveCardRenderer` (architecture) vs `AdaptiveCardBuilder` (implementation-plan):** This architecture uses the name `AdaptiveCardRenderer` (and interface `IAdaptiveCardRenderer` at §4.6) for the component that builds Adaptive Card JSON. The sibling `implementation-plan.md` §3.1 line 154 calls the same component `AdaptiveCardBuilder`. These refer to the **same component** — a single class that renders `AgentQuestion` and other domain objects to Adaptive Card JSON using the AdaptiveCards NuGet package. At implementation time, either name may be used; if the team adopts `AdaptiveCardBuilder` (the implementation plan's name), the DI registration at §10.2 becomes `services.AddSingleton<IAdaptiveCardRenderer, AdaptiveCardBuilder>()` — the interface `IAdaptiveCardRenderer` remains the contract surface regardless of the concrete class name.
>
> **Method-name harmonization — `AdaptiveCardBuilder.RenderQuestion(agentQuestion)` (implementation-plan shorthand) ≡ `IAdaptiveCardRenderer.RenderQuestionCard(question)` (canonical contract):** `implementation-plan.md` §3.1 line 172 and §4.2 line 298 use the shorthand `AdaptiveCardBuilder.RenderQuestion(agentQuestion)` when describing how `TeamsMessengerConnector.SendQuestionAsync` and `TeamsProactiveNotifier` render an `AgentQuestion` as an Adaptive Card. The canonical method on the interface (§4.6) is named **`RenderQuestionCard(AgentQuestion question)`** — the `*Card` suffix mirrors the sibling renderer methods (`RenderStatusCard`, `RenderIncidentCard`, `RenderReleaseGateCard`, `RenderDecisionConfirmationCard`) and is a deliberate harmonization across the renderer surface. Implementations therefore call `IAdaptiveCardRenderer.RenderQuestionCard(question)` (the canonical surface) wherever the implementation plan reads `AdaptiveCardBuilder.RenderQuestion(agentQuestion)`; both phrasings refer to the same method on the same interface. This avoids a naming-contract drift between the requirements text and the compiled API: the architecture document defines the contract surface, the implementation plan uses a more conversational shorthand, and the code adheres to the architecture's canonical name. The DI registration above (`services.AddSingleton<IAdaptiveCardRenderer, AdaptiveCardBuilder>()`) is what makes `AdaptiveCardBuilder.RenderQuestionCard(...)` resolve through the `IAdaptiveCardRenderer` interface at call sites such as `TeamsProactiveNotifier.SendQuestionCoreAsync` and `TeamsMessengerConnector.SendQuestionAsync`.
...
| **Responsibility** | Append-only, immutable audit trail. Logs every inbound command, outbound message, card action, proactive notification, security rejection, and error. Each entry carries all canonical fields from `tech-spec.md` §4.3: `CorrelationId`, `Timestamp`, `ActorId`, `ActorType`, `TenantId`, `AgentId` (first-class field — present on all events tied to an agent task, including human-originated actions; see `tech-spec.md` §4.3 line 139), `TaskId`, `ConversationId`, `EventType`, `Action`, `Outcome`, and `PayloadJson`. Suitable for enterprise compliance review. |
...
> **Field mapping note:** `ActorId` identifies *who performed the action* (AAD object ID for users, agent ID for agent-originated events), disambiguated by `ActorType`. `AgentId` identifies *which agent's task/question is involved* — it is a **first-class field** per `tech-spec.md` §4.3 canonical schema, not derived from or replaced by `ActorId`/`ActorType`. For agent-originated events, both `ActorId` and `AgentId` hold the same agent ID. For human actions on an agent's question (e.g., `approve`), `ActorId` = user's AAD object ID, `ActorType = User`, and `AgentId` = the associated agent's ID. This ensures the agent is always recorded regardless of who initiated the action.
>
...
TeamsCardState *──────1 TeamsConversationReference
      │                        │
      │ QuestionId             │ (TenantId, AadObjectId) or
│                        │ (TenantId, InternalUserId) or
│                        │ (TenantId, ChannelId)
      ▼                        │
HumanDecisionEvent             │
      │                        │
...
>
> **Cross-doc consistency — verified aligned:** `implementation-plan.md` §1.2 (line 33) and §4.1 (lines 237–242) now use the dual identity-key model with `GetByAadObjectIdAsync`, `GetByInternalUserIdAsync`, `AadObjectId`, and `InternalUserId` — aligned with this architecture's `IConversationReferenceStore` interface above.
...
```text
Orchestrator    TeamsMessengerConnector    AgentQuestionStore    OutboxRetryEngine    ProactiveNotifier    ConvRefStore    AdaptiveCardRenderer    CardStateStore    Teams
     │                    │                      │                     │                     │                  │                  │                    │              │
     │── SendQuestion ───>│                      │                     │                     │                  │                  │                    │              │
     │   (AgentQuestion)  │── SaveAsync ────────>│                     │                     │                  │                  │                    │              │
     │                    │<── persisted (Open) ──│                     │                     │                  │                  │                    │              │
     │                    │── enqueue ───────────────────────────────>│                     │                  │                  │                    │              │
     │                    │                      │                     │── dequeue ─────────>│                  │                  │                    │              │
     │                    │                      │                     │                     │── lookup ref ───>│                  │                    │              │
...
2. `TeamsMessengerConnector` calls `IAgentQuestionStore.SaveAsync(agentQuestion)` to persist the question with `Status = Open` before any rendering or enqueueing. This ensures the question is durably stored so that subsequent card actions (§6.3) can resolve it via `GetByIdAsync` for action validation and idempotent duplicate handling.
3. `TeamsMessengerConnector` creates an `OutboxEntry` and persists it via `IMessageOutbox.EnqueueAsync`.
4. The outbox background worker dequeues the entry and delegates to `ProactiveNotifier`.
5. `ProactiveNotifier` resolves the target from the `OutboxEntry.Destination` field (which was derived from `AgentQuestion.TargetUserId` or `AgentQuestion.TargetChannelId` — see §3.1 routing derivation note). It looks up the `ConversationReference` for the target user via `IConversationReferenceStore.GetByInternalUserIdAsync(tenantId, targetUserId)` (or `GetByChannelIdAsync` for channel-scoped questions).
6. **Inactive-installation pre-check:** If the reference is not found, or if `IsActive == false` (indicating the user uninstalled the bot), `ProactiveNotifier` skips the Bot Framework call entirely, moves the outbox entry to the dead-letter queue via `IMessageOutbox.DeadLetterAsync`, and logs an audit entry (`EventType: Error`, `Outcome: Failed`, reason: inactive/missing reference). This pre-check avoids unnecessary Bot Framework calls for known-uninstalled users (aligned with `implementation-plan.md` §5.1 `InstallationStateGate` and `e2e-scenarios.md` §Proactive Messaging lines 121–129).
7. `AdaptiveCardRenderer.RenderQuestionCard` builds the Adaptive Card with action buttons.
8. `ProactiveNotifier` calls `CloudAdapter.ContinueConversationAsync` with the conversation reference, sending the card as an `Activity`.
9. Teams returns the `activityId` of the sent message.
10. `CardStateStore.SaveAsync` persists the `TeamsCardState` with the `activityId` (needed for future update/delete).
11. `OutboxRetryEngine` marks the entry as `Sent` (via `IMessageOutbox.AcknowledgeAsync`).
12. If delivery fails with a transient error (HTTP 429, 500, 502, 503, 504), the engine schedules a retry per `tech-spec.md` §4.4 (exponential backoff with ±25% jitter; `Retry-After` override for 429). If delivery fails with HTTP 403 or 404 (stale reference — user removed from tenant without an uninstall event), `ProactiveNotifier` marks the reference as inactive, dead-letters the message, and logs an audit entry (aligned with `e2e-scenarios.md` §Proactive Messaging lines 131–143).
