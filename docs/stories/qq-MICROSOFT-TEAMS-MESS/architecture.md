...
| `EventType` | `string` | Yes | `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error` — exactly the six canonical values from `tech-spec.md` §4.3 (compliance-audit-schema). Message actions (Teams message-extension submissions) log as `CommandReceived` because a message action is a command submission mechanism, not a semantically distinct event category — consistent with `tech-spec.md` §4.3 line 136. The `Source` field on the domain `MessengerEvent` (§3.1) distinguishes message-action-originated commands (`Source = MessageAction`) from direct input (`Source = null`). |
...
6. An audit entry of type `CommandReceived` is logged (message actions log as `CommandReceived` per `tech-spec.md` §4.3 line 136 — the canonical audit set contains exactly six values and treats message actions as a command submission mechanism, not a separate event category). The `Source = MessageAction` field on the domain `MessengerEvent` distinguishes the submission mechanism for downstream processing.
