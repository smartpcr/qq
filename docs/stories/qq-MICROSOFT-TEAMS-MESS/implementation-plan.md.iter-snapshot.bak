...
- [ ] Define `IAuditLogger` interface with method `LogAsync(AuditEntry, CancellationToken)` where `AuditEntry` includes all canonical fields from `tech-spec.md` §4.3 (Canonical Audit Record Schema): `Timestamp` (DateTimeOffset), `CorrelationId` (string), `EventType` (string — exactly six canonical values: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error`; message actions (Teams message-extension submissions) log as `CommandReceived` because a message action is a command submission mechanism — the `Source` field on the domain `MessengerEvent` distinguishes message-action-originated commands from direct text input, per `tech-spec.md` §4.3 Canonical Audit Record Schema), `ActorId` (string — AAD object ID for users, agent ID for agents), `ActorType` (string — `User` or `Agent`), `TenantId` (string), `TaskId` (string, optional), `ConversationId` (string, optional), `Action` (string), `PayloadJson` (string — sanitized JSON payload), `Outcome` (string — `Success`, `Rejected`, `Failed`, `DeadLettered`).
- [ ] Define `IMessageStore` interfacefor persisting inbound/outbound messages with methods: `SaveInboundAsync`, `SaveOutboundAsync`, `GetByCorrelationIdAsync`.
...
- [ ] Override `OnMessageActivityAsync` to strip `@mention` markup from incoming message text before parsing commands: use `Activity.RemoveMentionText(Activity.Recipient.Id)` (or equivalent `TurnContext.Activity` helper) to normalize channel messages where Teams prepends `@AgentBot` to the text. This ensures command parsing works identically in personal chat and team channels (per `e2e-scenarios.md` §Message Actions lines 38-40 which require stripping `@AgentBot` before parsing). After normalization, parse incoming text commands (`agent ask`, `agent status`, `approve`, `reject`, `escalate`, `pause`, `resume`) and route to a command dispatcher.
- [ ] In `OnMessageActivityAsync`, after the command dispatcher confirms the user is identity-resolved and authorized (via `IIdentityResolver.ResolveAsync` and `IUserAuthorizationService.AuthorizeAsync` from Stage 5.1 — these services are registered in DI from the start with a default-deny posture — `IIdentityResolver` rejects unmapped users immediately and `IUserAuthorizationService` enforces a minimal RBAC policy allowing only recognized users to execute commands; Stage 5.1 adds the full role-scoped RBAC configuration), extract the `ConversationReference` from the incoming `Activity` via `Activity.GetConversationReference()` and call `IConversationReferenceStore.SaveOrUpdateAsync` to persist or update it. This ensures conversation references are captured only for authorized users on first interaction (not for unauthorized or unmapped users), satisfying the story requirement that references are stored for authorized users/channels. This also satisfies `e2e-scenarios.md` §Conversation Reference Persistence scenario "Conversation reference is stored on first interaction" (lines 252-256).
...
- [ ] After extracting and dispatching the message-action payload, log an audit entry with `EventType = "CommandReceived"` via `IAuditLogger.LogAsync`, recording the acting user's AAD object ID, tenant ID, the source message text, and the resulting correlation/task ID. Message actions log as `CommandReceived` because a message action is a command submission mechanism, not a semantically distinct event category — the `Source` field on the domain `MessengerEvent` distinguishes message-action-originated commands (`Source = MessageAction`) from direct text input (per `tech-spec.md` §4.3 Canonical Audit Record Schema, which defines exactly six canonical `EventType` values).
...
### Implementation Steps
- [ ] Implement `SqlAuditLogger : IAuditLogger` writing to an append-only `AuditLog` table with columns matching the canonical schema from `tech-spec.md` §4.3 (Canonical Audit Record Schema): `Id` (surrogate PK), `Timestamp` (DateTimeOffset), `CorrelationId` (string), `EventType` (string — exactly six canonical values: `CommandReceived`, `MessageSent`, `CardActionReceived`, `SecurityRejection`, `ProactiveNotification`, `Error`; message actions (Teams message-extension submissions) log as `CommandReceived` because a message action is a command submission mechanism — the `Source` field on the domain `MessengerEvent` distinguishes message-action-originated commands from direct text input, per `tech-spec.md` §4.3 Canonical Audit Record Schema), `ActorId` (string — AAD object ID or agent ID), `ActorType` (string — `User` or `Agent`), `TenantId` (string), `TaskId` (string, nullable), `ConversationId` (string, nullable), `Action` (string), `PayloadJson` (string — sanitized JSON), `Outcome` (string — `Success`, `Rejected`, `Failed`, `DeadLettered`), `Checksum` (SHA-256 of row content for tamper detection).
