...
- [ ] Define `IIdentityResolver` interface with method `ResolveAsync(string aadObjectId)` returning an internal user identity record, and `IUserAuthorizationService` interface with method `AuthorizeAsync(string tenantId, string userId, string command)` returning an authorization result, both in `AgentSwarm.Messaging.Abstractions` (interface assembly per `architecture.md` §2.17 Interface field: "defined in `AgentSwarm.Messaging.Abstractions`; implementations in `AgentSwarm.Messaging.Teams`", §2.18 Interface field: same split, and §7 assembly table which lists both interfaces under `AgentSwarm.Messaging.Abstractions`). These minimal contracts allow Stage 2.2 to gate conversation reference capture on authorization without depending on the concrete RBAC implementation (Stage 5.1).
- [ ] Implement `DefaultDenyIdentityResolver : IIdentityResolver` stub in `AgentSwarm.Messaging.Abstractions` that returns `null` for all AAD object IDs (rejecting unmapped users immediately). Implement `DefaultDenyAuthorizationService : IUserAuthorizationService` stub in `AgentSwarm.Messaging.Abstractions` that returns `IsAuthorized = false` for all requests (enforcing a minimal deny-all policy). These stubs are registered in DI at startup (Stage 2.1) so that Stage 2.2's `TeamsSwarmActivityHandler` can inject and call these interfaces before the full RBAC implementations exist (Stage 5.1 replaces them).
...
- [ ] Implement `Startup`/`Program.cs` registering `CloudAdapter` (from `Microsoft.Bot.Builder.Integration.AspNet.Core`) as the bot adapter with middleware pipeline in this exact order: `TelemetryMiddleware` → `TenantValidationMiddleware` → `ActivityDeduplicationMiddleware` → `RateLimitMiddleware` (per `architecture.md` §2.3). Register `IBot`, health check endpoints (`/health`, `/ready`), and OpenTelemetry tracing. Register `DefaultDenyIdentityResolver` as `IIdentityResolver` and `DefaultDenyAuthorizationService` as `IUserAuthorizationService` in DI (stub implementations created in Stage 1.2) so that `TeamsSwarmActivityHandler` (Stage 2.2) can inject these interfaces before the full RBAC implementations replace them in Stage 5.1.
- [ ] Implement `TenantValidationMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that reads the `AllowedTenantIds` list from `TeamsMessagingOptions` and rejects activities from tenants not in the list by returning HTTP 403 and short-circuiting the pipeline. Log a `SecurityRejection` audit entry for blocked tenants. This middleware is created and registered here in Stage 2.1 because the bot adapter pipeline (previous step) requires it at startup.
- [ ] Implement `TelemetryMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that records OpenTelemetry spans for every inbound activity with attributes: `activity.type`, `activity.id`, `tenant.id`, `conversation.id`, and `correlation.id`. This middleware runs first in the pipeline to ensure all downstream processing is captured in the trace.
- [ ] Implement `ActivityDeduplicationMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that suppresses duplicate inbound webhook deliveries by checking `Activity.Id` (or `Activity.ReplyToId` for invoke activities) against an `IActivityIdStore`. Position in the pipeline after `TenantValidationMiddleware` and before `RateLimitMiddleware`. Use a configurable TTL (default: 10 minutes, aligned with `architecture.md` §2.16 `IActivityIdStore` default TTL). Short-circuit duplicates with HTTP 200 and log a deduplication event. This is distinct from Stage 6.2's domain-level idempotency check which operates on `(QuestionId, UserId)` pairs in `CardActionHandler`.
- [ ] Implement `RateLimitMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that applies per-tenant inbound rate limiting using a sliding-window counter (default: 100 activities per minute per tenant, configurable via `TeamsMessagingOptions`). When the rate limit is exceeded, short-circuit with HTTP 429 and a `Retry-After` header. Log a rate-limit event for monitoring.
