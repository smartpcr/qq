...
- [ ] Implement `TenantValidationMiddleware : IMiddleware` in `AgentSwarm.Messaging.Teams.Middleware` that reads the `AllowedTenantIds` list from `TeamsMessagingOptions` and rejects activities from tenants not in the list by returning HTTP 403 and short-circuiting the pipeline with no bot response or card (per `tech-spec.md` §4.2). Log a structured warning via `ILogger` for blocked tenants (basic operational logging only; formal `IAuditLogger` integration is added in Stage 5.1 when the compliance audit infrastructure is available).
...
### Implementation Steps
- [ ] Enhance `TenantValidationMiddleware` (created in Stage 2.1) with compliance-grade audit logging: emit a `SecurityRejection` audit entry via `IAuditLogger` with `Outcome = "Rejected"` and `Action = "UnauthorizedTenantRejected"` for each blocked tenant. The middleware continues to return HTTP 403 with no bot response or Adaptive Card (per `tech-spec.md` §4.2: tenant allow-list failures are rejected at the middleware layer before the bot handler runs, so there is no conversation context in which to send a card). Stage 2.1 created the middleware with basic `ILogger` structured logging; this step replaces that with formal `IAuditLogger` integration for compliance-grade audit trail.
