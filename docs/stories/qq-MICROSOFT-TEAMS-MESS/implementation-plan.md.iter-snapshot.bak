...
- [ ] Implement `Startup`/`Program.cs` registering `CloudAdapter` (from `Microsoft.Bot.Builder.Integration.AspNet.Core`) as the bot adapter with middleware pipeline in this exact order: `TelemetryMiddleware` → `TenantValidationMiddleware` → `ActivityDeduplicationMiddleware` → `RateLimitMiddleware`. This is the canonical four-middleware pipeline. `architecture.md` §2.3 diagram (line 39) shows the abbreviated form `Telemetry → TenantValidation → Dedup → Rate` — this is the same four-middleware pipeline using short names and is now aligned with this plan (both documents list four middleware in the same order; no middleware is omitted). All four middleware classes are created in the preceding steps within this stage. Register `IBot`, health check endpoints (`/health`, `/ready`), and OpenTelemetry tracing. Register `DefaultDenyIdentityResolver` as `IIdentityResolver` and `DefaultDenyAuthorizationService` as `IUserAuthorizationService` in DI (stub implementations created in Stage 1.2, available via Phase 2's dependency on Phase 1) so that `TeamsSwarmActivityHandler` (Stage 2.2) can inject these interfaces before the full RBAC implementations replace them in Stage 5.1.
...
- [ ] Override `OnMessageActivityAsync` to strip `@mention` markup from incoming message text before parsing commands: use `Activity.RemoveMentionText(Activity.Recipient.Id)` (or equivalent `TurnContext.Activity` helper) to normalize channel messages where Teams prepends `@AgentBot` to the text. This ensures command parsing works identically in personal chat and team channels (per `e2e-scenarios.md` §Message Actions lines 38-40 which require stripping `@AgentBot` before parsing). After normalization, parse incoming text commands (`agent ask`, `agent status`, `approve`, `reject`, `escalate`, `pause`, `resume`) and route to the injected `ICommandDispatcher` (interface defined in Stage 1.2; concrete `CommandDispatcher` implementation is provided in Stage 3.2).
